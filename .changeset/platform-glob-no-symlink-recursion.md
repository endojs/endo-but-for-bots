...
`followSymlinks` is reachable from every layer that exposes the search surface, not only from a direct `@endo/platform` consumer: `EndoMount`'s `glob`, `grep`, and `glorp` accept it, and the `glob`/`grep` agent tools take it as an argument. On `glorp` it reaches the enumeration half only — the scan half receives an explicit path array, and a named path is followed regardless.
