...
`@endo/platform`: `globPaths` gains a `sorted` option (default `true`, glob's UTF-16 sorted-path contract). With `sorted: false` the engine yields matched paths in walk order as it discovers them — no global-sort barrier before the first batch — over the same single walker, confinement, and denial filtering. The `sorted: false` walk sorts each directory's own entries locally before descending, so walk order is deterministic and platform-independent (not dependent on `readDirectory`'s underlying order) while still avoiding the whole-tree global sort; the eager `sorted: true` path skips this per-directory sort, since its final global sort makes the intra-directory order unobservable. `streamGlob` drives the walk this way (`sorted: false`) so its output is incremental and demand-bounded; eager `glob()` keeps `sorted: true`.
