...
# codes:
#
#   npm run lint:sh                     # default severity warning
#   npm run lint:sh --severity=error    # tighter gate
#   npm run lint:sh -- -e SC2086        # exclude a code
#
...
# it installed can still run the rest of `npm run lint` locally. CI installs
