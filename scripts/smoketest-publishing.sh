...
# registry. Runs the full `pnpm release:npm` flow (which internally
# calls `pnpm pack:all` + `npm publish` per tarball) pointed at a
...
# Isolate npm / pnpm state to a per-run HOME so the smoketest cannot
...
# prompt the user before downloading the pinned pnpm version.
...
echo "smoketest-publishing: running 'pnpm release:npm'"
(
  cd "$ROOT"
  npm_config_registry="$REGISTRY_URL" pnpm release:npm
)
