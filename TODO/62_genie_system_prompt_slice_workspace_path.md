...
1. [ ] Plumb a slice-awareness signal into `makePiAgent` and
...
2. [ ] Wire the prompt addition from both call sites:
...
3. [ ] Update the `bash` / `exec` / `git` tool descriptions (in
...
4. [ ] Add a regression test that builds a genie agent with a fake
