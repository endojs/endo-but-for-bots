...
[#902](https://github.com/endojs/endo-but-for-bots/pull/902) add the Shell,
...
mode-selected, except that writable Git (`readWrite` or `historyRewrite`)
requires a writable filesystem grant: the native Git backend writes the same
working tree at the OS level, so `fs: 'readOnly'` combined with a writable
Git mode is rejected at provisioning time rather than left as an authority
union.
...
Remote policy normalization is owned by `@endo/exo-git`. Fetch-refspec order is
preserved, and `defaultPullRef` may select the concrete mapping used by an
unqualified pull.
...
| [PR #902](https://github.com/endojs/endo-but-for-bots/pull/902) | **Shell, HTTP, and GitRemote capability globals.** Adds their extractors, globals, and generated declarations; the workspace/fs global and its generated declarations predate #902 on `llm`. |
