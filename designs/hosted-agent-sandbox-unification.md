...
Create `makeFsMounter` and its 9P bridges locally in each dedicated native session
controller; the shared mounter currently imports disposable workspace/config filesystem
formulas, so resolving the sandbox factory's bind paths alone does not fix its lifetime.
...
The current mounter/bridge cleanup does not yet prove this ordering: it suppresses
some cleanup failures and treats socket closure as completion of filesystem effects.
