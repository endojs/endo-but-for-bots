...
2. **The full verified interface inventory is larger than the three headline
   ...
   Design Decision 8 keeps most of them out of the _catalog_ deliberately;
   the appendix exists so the _catalogue_ of what currently exists is
...
does move toward co-location, but via a layered guard _tier_, not a single
...
1. **Storage-shape vocabulary, not mechanism.** It names _what a value of the
   filesystem IS_ (the Readable / Snapshot / Mutable lattice over Blob /
   Tree), deliberately leaving _how bytes move_ to the caps that implement it.
...
3. **Structural attenuation, not behavioral.** `readOnly()` returns a cap that
   _lacks_ the mutation methods (platform-fs Decision 4), rather than a cap
...
3. **A backing seam (`FsBackend`), not per-backing reimplementation.** The
   ...
   the retire/merge must carry across intact — it is _why_ new backings are
   cheap.
...
**How the reconciliation honors both:** the catalog names the _intersection_
...
3. **Snapshot return type.**
   ...
   These are _not_ the same shape and not interchangeable; the catalog must pick one.
4. **`readOnly()` location.**
...
| `writeText`     | `writeText(content: string) → Promise<void>` (Mutable Blob)                                 | Whole-text convenience on a blob, peer of `writeBytes` / `append`. One signature only (per the same-name-implies-same-signature invariant, review finding 3). Writing text to a _named tree entry_ is not a second `writeText`; it is `write(path, content)`, whose `string` value is UTF-8 text.                                                                                                                                                                                                                         |
...
`create(name) → OpenFile` returns a _writer the caller must drive_ — a
...
| Method     | Signature                                                                   | Returns                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `readOnly` | `readOnly() → ReadableBlob \| ReadableTree`                                 | Structural attenuation (per platform-fs Decision 4). Returned cap has only the Readable methods; mutation methods are _absent_, not _throwing_.                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Method     | Signature                                            | Returns                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------- | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `snapshot` | `snapshot() → Promise<SnapshotBlob \| SnapshotTree>` | Capture the cap's current state into the host's `SnapshotStore` and return the content-addressed cap. The catalog adopts platform-fs's `SnapshotBlob` / `SnapshotTree` shape (with `sha256()` method), **not** endo-fs's `BlobRef \| null` shape. See [Design Decisions](#design-decisions) Decision 3 for why. Backings that cannot cheaply produce a snapshot return a rejected promise with a structured error naming the gap (not `null`). **endo-fs's existing `File.snapshot() → BlobRef \| null` is a _different_ method that stays on the extended surface** (it optimizes a peer-CAS cache-hit short-circuit and is allowed to return `null`); a backing satisfies the _catalog_ `snapshot` only through the `BlobRef → SnapshotBlob` adapter (endo-fs ROADMAP F6, **not yet built**), so the matrix marks endo-fs `snapshot` as `I*` pending that adapter (review finding 7). |
...
| `identify` / `locate` / `followNameChanges`-on-formula-system-paths | Formula-system concepts (per platform-fs Decision: stops-at-filesystem-boundary). `followNameChanges` in the catalog is the _filesystem_ sense, not the formula-system sense.                                                                                                    |
...
over the same _job_ as `lookup`, so it gets no separate conformance row — only
...
Per platform-fs Decision 4, attenuation is _structural_: where a method is **A**bsent, the cap simply does not have the method.
...
dispatches on `__getMethodNames__()`, which returns method _names_ with no
...
3. **Backing-aware messaging.** The tooltip names the gap in user-meaningful terms, not in method-name terms. "Cannot edit: this is an immutable snapshot" is better than "writeText not in **getMethodNames**".
...
2. **endo-fs is being retired into platform/fs** (D4 below). A
   ...
   about to delete. Big-bang renames _and_ re-homes in one cutover.
...
The load-bearing design consideration this must confront (it was the _reason_
...
guard; `sloppy: true` means "permit _additional, unchecked_ methods," not
"layer a second validated guard on top." Worse, the two guards _contradict_ on
...
can satisfy both. "Two tiers" would resolve the divergence by _abandoning_
...
`@endo/platform/fs/extended`'s `Directory` / `File` are a _superset_
...
(below) is sound. `sloppy: true` is retained only for _forward_ evolution
...
implement the _catalog_ `ReadableTree` / `Directory` guards (the narrow ones),
...
1. Land the cap-FS surface as a new `@endo/platform/fs/extended` entry (move
   ...
   "vocabulary _plus_ an opt-in extended engine behind a separate entry,"
   not "a fatter vocabulary."
2. Make `@endo/endo-fs` a **thin re-export shim** of the new location in the
   _same_ commit, so its dependents keep resolving (no dangling imports). The
...
- **[platform-fs.md](platform-fs.md)** § _Relationship to existing interfaces_: already does part of the reconciliation; this design extends to mount / scratch / CAS / endo-fs / name hub. platform-fs's "stops at the filesystem boundary" discipline is preserved.
- **[daemon-mount.md](daemon-mount.md)** § _Exo Interface_: the existing live `MountInterface` is the catalog's reference shape for method names and groupings. No name changes on `MountInterface`; only `subView` and `followNameChanges` land as new methods.
...
- **[filesystem-watchers.md](filesystem-watchers.md)** (PR #277): landing `followNameChanges` on `MountInterface` is the catalog's _observe_ row.
- **[exo-zip-package.md](exo-zip-package.md)** § _reuse-platform-interface-not-daemon-interface_: the precedent for the catalog living in `packages/platform/src/fs/interfaces.js` rather than a daemon-specific shape.
...
1. **Each method name is chosen on merit and recorded — not adopted verbatim.**
   ...
   it is the _best_ name for the job — `has`, `list`, `lookup`, `write`,
   ...
   The difference from verbatim adoption is the _discipline_: where two
   ...
   - The in-session re-root method is named **`subView`**, not `subDir`
     ...
     `provideSubMount` and because it re-roots a _view_, not necessarily a
     directory.
...
3. **`snapshot()` returns `SnapshotBlob` / `SnapshotTree`, not `BlobRef \| null`.**
   ...
   - It is content-addressed _and_ a full Readable surface (`text`, `streamBase64`, `json`, `has`, `list`, `lookup`) on one cap, matching the viewer's needs without a second hop.
...
4. **`readOnly()` is a cap method, not (only) a composer.**
   Returning a `ReadableBlob` / `ReadableTree` from `cap.readOnly()` is the catalog's per-cap attenuation. endo-fs's top-level `readOnly(fs)` composer is a _separate_ affordance for whole-filesystem attenuation and remains on endo-fs.
   Rationale: chat-side attenuation is one cap at a time (an editor attenuates _this_ file before passing it to a preview pane). The composer is for filesystem-construction time.
...
5. **`subView` lands as a new method on Mutable Tree, ReadableTree (where it makes sense), and `MountInterface`.**
   ...
   as "a re-rooted _view_," which is what it is — a transient attenuator, not
   a new directory.
   ...
   _plus_ a confinement-root shift to `path.join(mountRoot, 'a', 'b')`. The
...
8. **The catalog is silent about `OpenFile`, `Cursor`, `Lock`, `Xattrs`, `NodeWatcher`.**
   ...
   The clean line: the catalog covers what the _filesystem viewer_ needs; the cap-FS surface (range I/O, advisory locks, kernel-style watches, xattrs) stays on endo-fs for the 9P / FUSE / OS-bridge consumers.
...
_Not_ automatic verbatim adoption of MountInterface (the design's original
pick).

- _Consideration for verbatim:_ zero rename cost for daemon + platform/fs.
- _Consideration for best-on-merit:_ a catalog whose every name is defensible
  on its own terms, not inherited as an accident of which surface happened to
  name the method first.
- _Resolution:_ keep the existing name where it is the best name (the large
...
- _Consideration:_ a sync surface is safe only for backings that never cross
  ...
  _under_ the Promise. See [Sync versus async surface](#sync-versus-async-surface).

**D3 — Migration: big-bang, not gradual.** Overrides the design's original
gradual pick.

- _Consideration against big-bang (originally decisive):_ the standing
  per-package / per-commit migration preference; gradual keeps each step
  small and revertible.
- _Considerations that make big-bang acceptable here:_ endo-fs is a single
...
- _Cost to manage:_ one large reviewed cutover instead of several small ones;
...
- _Consideration against merge (originally decisive):_ endo-fs's DESIGN.md
...
- _Resolution & how the consideration is honored:_ merge into a new
  ...
  cap-FS methods, all explicitly declared. This is only possible _because_ the
...
- _Consideration:_ `subDir` collided (in name) with the formula-bearing
  `provideSubMount`, and the two have different lifetimes (transient exo vs
  persisted formula). `subView` names the _view_ attenuation and removes the
...
empty stream.** Overrides the design's original _absent_ pick.

- _Consideration against (originally decisive):_ the no-content-no-method
  principle — a method that can never do anything is noise, and consumers can
  check `__getMethodNames__()` before subscribing.
- _Consideration for (chosen):_ polymorphic consumers can call
  ...
  capability.
- _Cost to manage:_ the conformance matrix CAS cell flips **A → I (empty
...
- _Known limitation (distinguishability):_ an immediately-closed stream is
  ...
  _through this reference_," and a caller that needs to tell "immutable by
...
- _Consideration:_ `@endo/exo-stream` is the established substrate endo-fs
...
_librarian_ / _scholar_ tasks, not design blockers; to be filed as journal
...
_Resolution:_ catalog `lookup` is the path-array form everywhere; endo-fs's
...
writer). _Resolution:_ they stay distinct methods; a writer-only backing
...
discovery mis-dispatches if a name can carry two signatures. _Resolution:_
...
with the _catalog_ signature. Changed: [What the viewer reads](#what-the-viewer-reads).
...
than layering it; and the two guards contradict on `lookup`. _Resolution:_
...
_Resolution:_ the cap-FS engine lands strictly behind the
...
_Resolution:_ standardize on a `ReaderRef` (remotable async-iterator ref),
...
null` contract** and depended on an unbuilt adapter. _Resolution:_ endo-fs's
`File.snapshot → BlobRef | null` is a _separate_ extended method; the catalog
...
per-subdirectory confinement** (`..` reaches the mount root). _Resolution
(implemented):_ rather than defer, `EndoMount.subView(path)` was added — it
...
_Resolution:_ re-sequenced so `@endo/endo-fs` becomes a re-export shim in the
...
`list()` issued `2 + 2n` _serial_ round-trips (per-entry `lookup` +
...
> appendix were captured _before_ the `fs-interface-consolidation` work landed,
...
The catalog (above) is deliberately the _intersection vocabulary_ a
...
- that `FilePowers` is the host-authority floor every backing ultimately
  stands on, and is intentionally _not_ a cap (no `M.interface`).
