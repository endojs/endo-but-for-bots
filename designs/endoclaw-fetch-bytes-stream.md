...
the API. Same-vat consumers, which is how connectors compose per
[endoclaw-oauth](endoclaw-oauth.md) § The Connector Contract, are ungated
today; only the CapTP hop is gated, on upstream marshal support for
`byteArray` ([endojs/endo#2248](https://github.com/endojs/endo/issues/2248)),
with the existing `@endo/exo-stream` base64 byte readers as the documented
interim for remote streaming.
...
RFC 822 message import and export outside the JSON envelope. The gap was
noted and deferred in the endoclaw-oauth Connector Contract revision
(endojs/endo-but-for-bots#621) with the directive to plan `bytes()` and
`stream()` gated on passable-byte-array progress. This design is that plan.
...
- **Phases 1 and 2 below do not wait.** The connector composition is same-vat
  by contract (the host closes the connector over the `OAuth` exo's `fetch`;
  no CapTP hop per request), so a `ByteArray` returned by `bytes()` or carried
  as a request body never reaches marshal. Everything a Drive or Gmail
  connector needs is buildable today.
...
`maxResponseBytes` with a `truncated` flag. `bytes()` returns that
...
mid-stream breaks the reader the same way (`code: 'revoked'`). In streaming
mode `text()`, `json()`, and `bytes()` throw a structured `'body-streaming'`
error. In buffered mode `stream()` still works, yielding the single buffered
body as one chunk then done, so a consumer can be written uniformly against
...
graph LR
  ...
  R[Remote agent] -.->|"CapTP: gated on endo#2248; interim: exo-stream base64"| O
...
| [endoclaw-oauth](endoclaw-oauth.md) (revision in endojs/endo-but-for-bots#621) | Consumer layer; its `FetchOptions` / `FetchResponse` widen in lockstep; its Connector Contract's same-vat composition is what makes phases 1 and 2 ungated |
...
5. **Remote flip.** When endojs/endo#2248 lands and the `llm` branch picks it
   up, the remote surface works unchanged; retire the base64 bridge per
...
> `json()` only and `FetchOptions.body` is `string` only — the gap noted and
> deferred in `designs/endoclaw-oauth.md` (Capability Shape section) and owned
> by `designs/endoclaw-network-fetch.md`.
>
