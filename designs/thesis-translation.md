...
- **Figures.** The dissertation's figures are redrawn as mermaid diagrams
  ...
  example, not Yee's. Reviewing this design the author **resolved that none
  of these four are a separate-permission problem**: they are part of the
  thesis under the joint understanding at the time it was written, not
  separate work merely referred to by the thesis, so Miller's diagram grant
  covers them exactly as it covers his own figures — no separate permission
  from Ka-Ping Yee is required
  ([endojs/endo-but-for-bots#631 review](https://github.com/endojs/endo-but-for-bots/pull/631#pullrequestreview-4650784770),
  2026-07-08). They therefore get the same treatment as Miller's own
  figures: the mermaid redraw is tried first, and reproducing the original
  figure is a permitted fallback where the redraw disappoints. Where a figure
  resists mermaid, the chapter describes it and links the PDF page.
...
- The acknowledgements note the dissertation borrows from co-authored
  ...
  16.1, 17.1) were initially read as *otherwise attributed* and outside the
  grant, but the author **resolved that they are not a separate-permission
  problem**: they are part of the thesis under the joint understanding at
  the time it was written, not separate work merely referred to by the
  thesis, so Miller's diagram grant covers them exactly as it covers his own
  figures — no separate permission from Ka-Ping Yee is required
  ([endojs/endo-but-for-bots#631 review](https://github.com/endojs/endo-but-for-bots/pull/631#pullrequestreview-4650784770),
  2026-07-08). The handling therefore mirrors § Docs structure, Figures:
  redraw first, reproduce the original as a permitted fallback where the
  redraw disappoints, describe-with-PDF-link only where a figure resists
  mermaid.
...
8. **Mermaid redraws as the first figure experiment**, tool-native and
   ...
   17.1) get the same treatment: the author resolved they are part of the
   thesis and covered by his diagram grant, not separate work needing
   Ka-Ping Yee's permission, so they are redrawn first and reproduced from
   the original where the redraw disappoints, exactly like Miller's own
   figures. Considered and rejected: treating them as *otherwise attributed*
   third-party work gated on a separate permission from Ka-Ping Yee — the
   author held they were part of the thesis under the joint understanding at
...
7. **Ka-Ping Yee figure permission. — RESOLVED.** The design answered the
   author's "which diagrams?" question — the Yee figures are Figures 14.2,
   14.3, 16.1, and 17.1 (named in the thesis acknowledgements) — and the
   author then resolved that none of these four are a separate-permission
   problem: they are part of the thesis under the joint understanding at the
   time it was written, not separate work merely referred to by the thesis,
   so Miller's diagram grant covers them exactly as it covers his own figures
   ([endojs/endo-but-for-bots#631 review](https://github.com/endojs/endo-but-for-bots/pull/631#pullrequestreview-4650784770),
   2026-07-08). No separate permission from Ka-Ping Yee is required; the
   phase-4 builder treats all four like Miller's own figures (redraw first,
   reproduce the original where the redraw disappoints). Figure 9.3 (the
   membrane, Marc Stiegler's example) is a distinct attribution and was not
   in scope of this resolution.
