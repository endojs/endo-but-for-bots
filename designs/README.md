...
| [gateway-oauth-redirect](gateway-oauth-redirect.md) | 2026-07-10 | 2026-07-10 | Proposed |
| [gateway-oauth-aws](gateway-oauth-aws.md) | 2026-07-10 | 2026-07-10 | Proposed |
| [gateway-oauth-cloudflare](gateway-oauth-cloudflare.md) | 2026-07-10 | 2026-07-10 | Proposed |
| [gateway-oauth-netlify](gateway-oauth-netlify.md) | 2026-07-10 | 2026-07-10 | Proposed |
...
flowchart TD
    ...
    subgraph Remote Access
        ...
        goredir[gateway-oauth-redirect]
        goaws[gateway-oauth-aws]
        gocf[gateway-oauth-cloudflare]
        gonl[gateway-oauth-netlify]
        gauth --> gpkg
        onoise --> gpkg
        gpkg --> ddock
        fbund --> ddock
...
| [gateway-oauth-redirect](gateway-oauth-redirect.md) | Proposed | Shared contract for the remote first-mint redirect: public `/oauth/callback` route, `state` as single-use claim ticket, `RedirectRelay` seam, custody invariants (verifier and client secret never leave the daemon; the relay sees only `{ code, state }`). Resolves [endoclaw-oauth](endoclaw-oauth.md) Open Question 1 (maintainer directive on PR #621); serves M7's agent-side OAuth mints over M5 hosting. |
| [gateway-oauth-aws](gateway-oauth-aws.md) | Proposed | AWS Endo Gateway narrative: direct ingress; the callback is one more path on the [gateway-aws-deployment](gateway-aws-deployment.md) ALB/ACM/Route53 listener; multi-tenant client-registration options for the [gateway-aws-attuned](gateway-aws-attuned.md) variant. |
| [gateway-oauth-cloudflare](gateway-oauth-cloudflare.md) | Proposed | CloudFlare Endo Gateway narrative: Cloudflare Tunnel ingress onto the daemon's own gateway listener (primary; daemon can live behind NAT); Worker + Durable Object mailbox variant for hosts that cannot run a connector. |
| [gateway-oauth-netlify](gateway-oauth-netlify.md) | Proposed | Netlify Endo Gateway narrative: dead-drop mailbox (callback + claim functions over Netlify Blobs in strong-consistency mode, hash-keyed; the daemon polls outbound with `state` as the claim ticket). |
...
for each; add roughly one week for the gateway OAuth redirect relay
quartet's implementation (the `RedirectRelay` seam and mailbox poller,
then per-provider routes as their hosting substrates land); the
gateway-package, packaging-ci, and AWS stack designs are
...
| gateway-oauth-redirect | S-M | 2-3 days | 5 | `RedirectRelay` seam, loopback re-expression, provider-neutral mailbox poller |
| gateway-oauth-aws | S | 1-2 days | 5 | Callback route on the AWS gateway listener; WAF/log notes; multi-tenant registrar deferred |
| gateway-oauth-cloudflare | S-M | 2-3 days | 5 | Tunnel recipe (documentation) + Worker/Durable Object mailbox variant |
| gateway-oauth-netlify | S-M | 2-3 days | 5 | Relay site (two functions + Blobs) and deploy recipe |
...
| M5: Public Hosting & Billing (was M7) | 4 in-flight on PR #356 stack (`gateway-package` counted under M3; `gateway-packaging-ci`, `gateway-aws-deployment`, `gateway-aws-attuned` counted here) + 4 Proposed (the gateway OAuth redirect relay quartet: `gateway-oauth-redirect` + the AWS / CloudFlare / Netlify narratives) + 3 design gaps (`gateway-oauth-bonding`, `gateway-key-recovery`, `gateway-stripe-adapter`) | 5-7 weeks design + impl | merge cadence of PRs #343 and #356 |
