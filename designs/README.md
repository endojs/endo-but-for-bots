...
[session-sandbox-substrate](session-sandbox-substrate.md) to M3, defining the
daemon-owned durable session substrate that binds one canonical workspace mount
to a sandbox filesystem backend, sandbox-backed Git, npm/pnpm/Yarn execution,
public egress with private/LAN blocking, restart reconstruction, and
process-scoped Git credential injection; summary, dependency graph, M3 row,
...
[session-sandbox-substrate](session-sandbox-substrate.md) (added 2026-08-06;
one durable daemon formula owns the live sandbox handle, canonical workspace
mount, session-wide public egress with private/LAN blocking, a sandbox
filesystem backend, sandbox-backed local and remote Git, process-scoped
credentials, and a borrowed npm/pnpm/Yarn backend),
...
| [session-sandbox-substrate](session-sandbox-substrate.md) | 2026-08-06 | — | Proposed |
...
flowchart TD
    ...
    subgraph Capability System
        ...
        ssub[session-sandbox-substrate<br/><i>PROPOSED</i>]
...
| session-sandbox-substrate | Proposed | One daemon-owned durable substrate for execution-capable code-mode sessions: a reconstructed `SandboxHandle`, one canonical `/workspace` mount, a sandbox `FsBackend`, conforming public egress with private/LAN blocking, sandbox-backed local and remote Git with process-scoped credentials, and a borrowed npm/pnpm/Yarn backend. Review-only sessions remain on the cheaper host path; v1 provisions a new execution session instead of widening a retained one. |
...
| session-sandbox-substrate | L | 4-6 weeks | 3 | Eight reviewable cuts: sandbox lifecycle/streams, private egress and secret descriptors, durable daemon formula plus sandbox `FsBackend`, sandbox local Git and clone, Git remotes, borrowed npm/pnpm/Yarn backend, code-mode grants, and an end-to-end restart scenario. Builds on the existing sandbox, platform filesystem, mount, Git, remote, agent-tools, and agentry seams. |
