...
| [endoclaw-fetch-bytes-stream](endoclaw-fetch-bytes-stream.md) | 2026-07-10 | 2026-07-10 | Proposed |
...
flowchart TD
    ...
    subgraph Agent Capabilities
        ...
        ebytes[endoclaw-fetch-bytes-stream]
...
| endoclaw-fetch-bytes-stream | Proposed | `bytes()` / `stream()` binary media on the confined fetch surface (both directions); serves Drive/Gmail media in connectors; same-vat use ungated, remote CapTP hop gated on passable byte arrays ([endojs/endo#2248](https://github.com/endojs/endo/issues/2248)) |
...
| endoclaw-fetch-bytes-stream | S-M | 2-3 days | 7 | `bytes()` + `ByteArray` bodies on `@endo/http-confine` / `@endo/exo-http-client` (phases 1-2, ungated), streaming download (phase 3); remote CapTP hop gated upstream ([endojs/endo#2248](https://github.com/endojs/endo/issues/2248)) |
...
| M7: Weblets & Integrations (was M3) | 13 (`familiar-unified-weblet-server`, `familiar-chat-weblet-hosting`, `cli-store-verb-text-modes`, `cli-edit-verb`, `daemon-weblet-application`, `exo-zip-package`, `endoclaw-oauth`, `endoclaw-fetch-bytes-stream`, `exo-google-sheets`, `endoclaw-proactive-messages`, `endoclaw-notifications`, `endoclaw-webhooks`, `endoclaw-voice`) | 6-8 weeks | 8-11 weeks |
