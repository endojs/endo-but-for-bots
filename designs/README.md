...
[endo-ertp-migration](endo-ertp-migration.md) (added 2026-07-17;
recasts ERTP — Amount, Brand, Issuer, Mint, Purse, Payment, and the
AmountMath / AssetKind value algebra — as a standalone Endo package
`@endo/ertp` built on `@endo/pass-style` / `@endo/marshal` /
`@endo/patterns` / `@endo/exo` / `@endo/far`, decoupled from vats /
Zoe / SwingSet: AmountMath delegates its set/copyBag algebra to the
`@endo/patterns` key algebra, the remotable facets become `@endo/exo`
classes with `M.interface` guards, and durability rides an injected
Zone power (agoric supplies `@agoric/zone`, the Endo daemon supplies a
vfs / exo-state zone) instead of SwingSet baggage; folds in the
long-dormant "ERTP v2" backlog surveyed from the agoric-sdk issue
corpus (durability and AmountMath-refactor halves satisfied; NFT-store
performance, shared rights, and Want-Patterns deferred to a Phase-4
sibling design), with a re-export migration path that preserves brand
identity and an interop story; assigned to M10; summary table, M10
row, and this note synced),
...
| [endo-ertp-migration](endo-ertp-migration.md) | 2026-07-17 | 2026-07-17 | Not Started |
...
| endo-ertp-migration | Not Started | **Strategic / ecosystem:** recast ERTP (Amount / Brand / Issuer / Mint / Purse / Payment / AmountMath / AssetKind) as the standalone Endo package `@endo/ertp`, decoupled from vats / Zoe / SwingSet — AmountMath over the `@endo/patterns` key algebra, remotable facets as `@endo/exo` classes with `M.interface` guards, durability via an injected Zone power (agoric's `@agoric/zone` or a daemon vfs / exo-state zone) instead of SwingSet baggage. Folds in the dormant "ERTP v2" backlog surveyed from the agoric-sdk issue corpus (durability + AmountMath-refactor satisfied; NFT-store perf, shared rights, Want-Patterns deferred to a Phase-4 sibling). Migration is by re-export (`@agoric/ertp` adopts `@endo/ertp`, preserving brand identity), not a parallel fork. Phases 0-3 independent of the M10 gateway substrate. Not on the hosted-Gateway critical path |
...
| M10: Confinement & Ecosystem (was M5) | 7 (`endo-posix-sandbox`, `daemon-capability-persona`, `daemon-capability-bank`, `endoclaw-browser`, `endoclaw-channel-bridges`, `endoclaw-skill-registry`, `endo-ertp-migration`) | 14-20 weeks | 16-22 weeks |
