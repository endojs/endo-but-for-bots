...
| [ironhorse-panic](ironhorse-panic.md) | 2026-08-17 | — | Proposed |
...
| ironhorse-panic | Proposed | Names, formalizes, and extends the **panic**: an uncatchable, unrecoverable vat/worker termination no `try`/`catch`, promise handler, or engine path can intercept. Scope-first finding: `Halt::StackOverflow` and `Halt::MeterAbort` are *already* panics (abort-to-host, built on XS `fxAbort`), so the panic half is reclassification, not new machinery. Formal category: keep the diagnostic `Halt` variants, add an `is_panic()` grouping plus a `Halt::Panic(PanicKind)` for net-new sources (a caught Rust panic, the Coda's reference error), and surface a three-way `CrankOutcome` (`Committed`/`Uncaught`/`Panicked`) at the prospective `-e ironhorse` `Machine` seam. Message embargo: **surveyed the live crank path** and found there is **no commit point** and **no embargo** today (outbound leaves synchronously mid-crank via `send_frame` in `worker_io.rs`; a meter-aborted worker just dies via `process_meter_report`->`unregister` with its sent messages irrevocable; no delivery transcript, only coarse snapshot suspend/resume). So the embargo + crank-commit boundary + transcript replay are net-new and deferred to a named follow-on design (`message-embargo-and-crank-commit`), building on the `send_frame` chokepoint, the crank-start/end markers, and ironhorse-snapshot-store-seam's `HeapStore::commit`. Retry composes `debugWorker` suspend/resume; "fixed" is enumerated (new-snapshot code fix, config-change retry, external-condition retry). Debugger: a panic is categorically uncatchable (never consults `jumps`), distinct from an uncaught throw (`Halt::Throw` + `jumps.is_empty()`), so it gets its own `<panic>` wire message orthogonal to `setExceptionBreakMode`, and when a debugger is attached it stops the world at the panic site. Coda: off-by-default `Machine` construction option that makes the `XS_CODE_GET_LOCAL` / variable-lookup reference-error sites panic instead of throw (snapshot captures the PC at the fault pre-unwind); it is the opposite setting of the debugger design's `raise`-helper engine-unwind switch, so the two coexist as one mode toggle. |
