...
[http-adapter-pipeline](http-adapter-pipeline.md) (added 2026-08-15 as a
follow-up to the PR #286 `endo http mk` Phase 1 approval; it elaborates the
...
| http-adapter-pipeline | M-L | 4-6 days | 3 | Pass-style adapter pipeline over the Phase 1 `endo http` client: pre-flight/onion split plus five concerns (metering, fees, rate, retry, breaker; fees funds the meter stage rather than being a stage of its own) as exo facets, the `ChargeAccount`/`MeterReservation` interfaces, seven new controller verbs, and the byte-cap<->metering reconciliation. Phased (3, 3.5, 3.6, 4) and each phase independently shippable; design-only for now (Proposed) |
