...
one canonical sandbox mount, confined filesystem and Git backends,
operation-scoped credential descriptors, borrowed package-manager ownership,
sandbox-only package-code execution, enforced public egress, lifecycle and
orphan cleanup, restart reconstruction, and bwrap/Podman conformance without
holding the base endo-pi milestone; summary, dependency graph, M10 row,
estimate, totals, and timeline synced.*

*Layered on 2026-08-06: added
[session-execution-capabilities](session-execution-capabilities.md) to M3,
defining the daemon-owned durable session and its backend-independent workspace,
Git, package-manager, and Web grants; the safe first milestone uses one
canonical workspace, fixed-argv no-lifecycle dependency installation, mediated
public Web research, brokered HTTPS Git, explicit credentials, and restart
reconstruction without making a sandbox a prerequisite; summary, dependency
graph, M3 row, estimate, totals, and timeline synced.*
...
[session-sandbox-backend](session-sandbox-backend.md) (added 2026-08-06; an
optional bwrap/Podman backend over the durable session, with one `/workspace`,
confined filesystem and Git execution, descriptor-only credentials, borrowed
package management, guarded untrusted package code, enforced public egress,
and leak-free restart reconstruction),
...
| [session-execution-capabilities](session-execution-capabilities.md) | 2026-08-06 | 2026-08-06 | Proposed |
| [session-sandbox-backend](session-sandbox-backend.md) | 2026-08-06 | 2026-08-06 | Proposed |
...
**Current totals (2026-08-06):** 44 Complete/Implemented, 26 In Progress,
41 Not Started, 38 Proposed, 2 Active, 7 Reference, 2 Deprecated,
4 Superseded (164 designs).
This supersedes the counts in the historical summaries above.
The addition of
[session-execution-capabilities](session-execution-capabilities.md)
increments Proposed and the design count by one from the direct summary-table
baseline of 162.
The stacked addition of [session-sandbox-backend](session-sandbox-backend.md)
increments both once more without changing the base milestone.
The same table recount corrects existing prose drift from the older 150-design
snapshot.
...
flowchart TD
    ...
    subgraph Capability System
        ...
        secap[session-execution-capabilities<br/><i>PROPOSED</i>]
        sback[session-sandbox-backend<br/><i>PROPOSED</i>]
...
| session-execution-capabilities | Proposed | One durable daemon-owned session identity with independently granted workspace, Git, package-manager, and Web capabilities. The first useful endo-pi milestone converges on one canonical mount, adds fixed-argv no-lifecycle npm/pnpm/Yarn installation, replaces ambient Web fetch with mediated fetch/search and full SSRF checks, and brokers public HTTPS Git clone/fetch before operation-scoped authenticated fetch/push. A sandbox is an optional later backend, never public session identity or a release gate. |
| session-sandbox-backend | Proposed | Optional bwrap/Podman execution backend over the durable session contracts: one canonical `/workspace`, a race-free filesystem bridge, sandbox-backed local and remote Git, descriptor-only operation credentials, a borrowed package-manager backend, and explicit sandbox-only lifecycle/script/binary execution. Drivers must enforce public egress filtering, serialized lifecycle, kill-before-EOF output caps, orphan cleanup, restart failure, and the same cross-driver conformance suite. |
...
| session-sandbox-backend | Proposed | Optional execution backend over `session-execution-capabilities`: one canonical sandbox workspace, confined filesystem/Git/package code, descriptor-only credentials, enforced filtered egress, restart reconstruction, and bwrap/Podman lifecycle conformance. It is not the durable session identity or a gate on the base release. |
...
**Estimated duration (1 dev):** 12-18 weeks
...
| session-execution-capabilities | XL | 8-12 weeks across parallel lanes | 3 | Compose the landed portable package-manager facets with the #1011 coordinator, open projection, registry convergence, loopback broker, broker-configured `@endo/package-manager`, and trusted-grant stacks; complete the four-pull-request Web stack; add exact-origin Git CONNECT brokering; add durable write-ahead operation receipts and effect reconciliation; qualify sandbox `broker-only` networking for untrusted native processes; and run strict consumer-independent acceptance. |
...
| session-sandbox-backend | L | 4-6 weeks | 10 | Seven optional cuts over the base session: leak-free lifecycle and streams, real filtered egress and secret descriptors, private reconstruction plus filesystem bridge, sandbox Git and clone, credentialed remotes, borrowed package management plus guarded project code, and cross-driver end-to-end acceptance. Builds on `endo-posix-sandbox` and does not hold the M3 usability path. |
...
| M10: Confinement & Ecosystem (was M5) | 7 (`endo-posix-sandbox`, `session-sandbox-backend`, `daemon-capability-persona`, `daemon-capability-bank`, `endoclaw-browser`, `endoclaw-channel-bridges`, `endoclaw-skill-registry`) | 18-26 weeks | 20-28 weeks |
| M11: Rust Daemon (`endor`) (was M6) | 5 (`endor-git-bindings`, `endor-registry-proxy-worker`, `endor-tui`, `endor-bus-tui`, `endor-native-zip-xs`) | 14-21 weeks | 16-23 weeks |
| **Total remaining** | **63** + 7 M5 rows (4 in-flight + 3 design gaps) + 1 M6 own-work row | **~68-94 weeks** + M5 4-6 weeks + M6 ~2 weeks | **~80-110 weeks** |
...
gantt
    ...
    section Milestone 3
    Remote Access & Tools         :m3, after m2, 15w
    ...
    Confinement & Ecosystem       :m10, after m9, 26w
...
| M10: Confinement & Ecosystem (was M5) | 18-26 weeks | 54-73 weeks | Late June to late October 2027 |
| M11: Rust Daemon (`endor`) (was M6) | 14-21 weeks | 68-94 weeks | Q4 2027 to Q1 2028 (research-heavy; may run in parallel) |
