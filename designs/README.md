...
| [ironhorse-computron-benchmark-baseline](ironhorse-computron-benchmark-baseline.md) | 2026-09-16 | — | Proposed |
...
flowchart TD
    ...
    subgraph Ironhorse Snapshot Schema
        ...
        ihbaseline[ironhorse-computron-benchmark-baseline<br/><i>PROPOSED</i>]
        ihengine --> ihbaseline
...
| ironhorse-computron-benchmark-baseline | Proposed | Re-expresses the computron range constraints that PR #1282 relaxed as benchmark-established, self-referential baselines: a per-load cost model `C = coef*f(n)+b` fit from deterministic computrons, gated on PR CI by exact pins plus a growth-envelope, and validated nightly against measured CPU time (growth-class + fidelity). Models polynomial-in-input built-ins (named-property insertion, string/regexp/collection ops). Doctrine-aligned (own CPU time, never XS); sibling build job executes it. |
