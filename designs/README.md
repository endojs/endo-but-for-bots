...
flowchart TD
    ...
    %% hardening adapter, not a hard prerequisite (endo-claude.md DD6).
    dsand -.-> eclaude
...
- **The confined in-guest agent** — an agent running *inside* a guest,
  ...
  *is* an Endo guest's inference engine, its Claude tool surface replaced by
  only that one guest's facet (OS confinement is optional hardening, not the
  acceptance boundary). Tool-surface-substitution core in flight as PR
