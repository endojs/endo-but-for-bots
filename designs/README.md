...
| [ironhorse-ses-compartment-equivalence](ironhorse-ses-compartment-equivalence.md) | 2026-09-15 | — | Proposed |
...
| [ironhorse-ses-compartment-equivalence](ironhorse-ses-compartment-equivalence.md) | Proposed | Three-way equivalence between SES's `lockdown`/`Compartment`, XS's native implementation (`xsLockdown.c`, `fx_Compartment` in `xsModule.c`) and IronHorse's, written as the handoff for Phase 4 of the sequencing design. Measured rather than assumed: `ironhorse-262/tests/stage4_ses_boot.rs` dual-runs the daemon's own boot sequence against the XS oracle with census cranks bracketing each source. The headline result resizes the gap. Only `Compartment` is an XS realm intrinsic; `lockdown`, `harden`, `petrify` and `mutabilities` are embedder-installed globals that `fxCreateMachine` never binds, and the daemon's xsnap installs none of them (`ffi.rs:274` declares `fx_lockdown` and never calls it) — so against the daemon's XS, rather than against the oracle's test shim, the missing name is `Compartment` alone. `ses_boot.js` is not the SES shim either: 70 KB of `@endo/harden` + `@endo/env-options` + `@endo/eventual-send`, whose only `globalThis` write is `HandledPromise`. The document reads `fx_lockdown`'s five steps as the specification a native route would implement — idempotence, removing the evaluators from the locked realm, the compartment-global template, taming Date/Math into that template only, and the transitive harden — and finds IronHorse already does the last and widest of the five. Adds no scope: it is the equivalence detail behind `ironhorse-engine` roadmap stage 4, already counted there. |
...
| ironhorse-ses-compartment-equivalence | — | not estimated; a handoff, not an implementation target | M11 | The document's own first question is whether the daemon needs guest-visible SES at all or only the Rust-level compartment API exposed to `rust/endo`, and the two answers differ by more than an order of magnitude. Sizing it before that is answered would price the wrong project. What it does do is shrink the input to the XL `ironhorse-daemon-acceptance-sequencing` row above: the SES bar's dominant unknown was the `ses_boot.js` bundling call, and that is now settled — 70 KB, generated by `@endo/compartment-mapper`'s `makeBundle`, carrying no `lockdown`, and generated in the `test-ironhorse-oracle` lane as of this change. No milestone total or timeline change. |
