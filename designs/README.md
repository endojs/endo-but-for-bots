...
design),
[endor-npm-runtime-identity](endor-npm-runtime-identity.md) (added
2026-07-28; the condition-set and builtin policy the npm-via-CAS
registry proxy defers: which environment the endor archive runtime
professes to an npm package, separating build selection (the
`exports` condition set) from module provision (endowments and any
`node:` exit modules), so that shimming `node:crypto` never implies
activating the `node` condition; proposes flavor plus `endo` plus
`default` as the standing set, `browser` opt-in pending a corpus
experiment, web-standard globals preferred over builtin shims, and a
powerless-only `node:` allowlist that excludes `Buffer`; precipitated
by the nanoid@5 probe in the 2026-07-27 press reports and awaiting a
maintainer ruling, so milestone-bin and dependency-graph placement
are left to the roadmap keeper alongside its unbinned parent
...
| [endor-npm-runtime-identity](endor-npm-runtime-identity.md) | 2026-07-28 | — | Proposed |
...
**Current totals (2026-07-29, post-`endor-npm-runtime-identity` rebase):** 42 Complete/Implemented, 23 In Progress, 39 Not Started, 33 Proposed, 2 Active, 7 Reference, 2 Deprecated, 3 Superseded (151 designs). This supersedes the counts in the historical summaries above. The 2026-07-29 rebase adds [pass-style-promise](pass-style-promise.md) (Proposed) and [endor-npm-runtime-identity](endor-npm-runtime-identity.md) (Proposed, the condition-set and builtin policy note for the npm-via-CAS registry proxy), so Proposed goes 31 → 33 and the design count 149 → 151.
