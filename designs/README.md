...
*Layered on 2026-08-06 and revised on 2026-08-17:
...
one private owner and a lazy pool of exact-authority incarnations over selected
mount grants, confined Git and package execution, workload-specific broker-only
networking, operation-scoped Git credential descriptors, bounded lifecycle and
orphan cleanup, conservative restart outcomes, and profile-specific driver
conformance. The design evaluates direct `EndoMount` and the adjacent #971 9P
projection before introducing another filesystem protocol, and it does not hold
the frozen-installer or mediated-Web milestones; summary, dependency graph, M10
row, estimate, totals, and timeline synced.*

*Layered on 2026-08-06: added
[session-execution-capabilities](session-execution-capabilities.md) to M3,
defining the daemon-owned durable session and its backend-independent mount,
Git, package-manager, and Web grants; the first milestone uses a named authority
graph, fixed-argv no-lifecycle dependency installation, mediated
...
revised 2026-08-17; an optional confined backend over the durable authority
graph, with lazy exact-authority incarnations, compatibility and named mount
projection, sandbox Git and package execution, broker-only profiles,
...
| [session-sandbox-backend](session-sandbox-backend.md) | 2026-08-06 | 2026-08-17 | Proposed |
...
| session-sandbox-backend | Proposed | Optional confined execution backend over the generic authority graph. One owner lazily pools exact-authority sandbox incarnations keyed by selected mount set, modes, rootfs, network posture, and policy version. Compatibility `/workspace` and named `/mnt/<name>` projections, sandbox-backed Git and package execution, broker-only profiles, descriptor-scoped credentials, bounded lifecycle, conservative restart outcomes, and driver-specific conformance remain tracked work. The design reuses daemon `EndoMount` and evaluates the adjacent #971 9P projection before adding another filesystem protocol. |
...
| session-sandbox-backend | Proposed | Optional confined execution backend over `session-execution-capabilities`: lazy exact-authority incarnations over selected mount grants, confined Git and package code, broker-only networking, descriptor-scoped Git credentials, bounded lifecycle, explicit backend migration, conservative restart outcomes, and profile-specific conformance. It is not durable session identity or a gate on the base release. |
...
| session-sandbox-backend | XL | 8-12 weeks | 10 | Optional confined backend over the generic authority graph: #954 lifecycle, versioned binding and lazy exact-authority pool, direct-mount versus #971 9P projection decision, one reference driver's broker-only profiles, narrow Git credential descriptors, sandbox Git and remotes, borrowed package/project execution, mutating-effect records, and profile-specific acceptance. Additional drivers qualify independently. Does not hold the M3 frozen-installer or mediated-Web path. |
...
| M10: Confinement & Ecosystem (was M5) | 7 (`endo-posix-sandbox`, `session-sandbox-backend`, `daemon-capability-persona`, `daemon-capability-bank`, `endoclaw-browser`, `endoclaw-channel-bridges`, `endoclaw-skill-registry`) | 22-32 weeks | 24-34 weeks |
...
| **Total remaining** | **65** + 7 M5 rows (4 in-flight + 3 design gaps) + 2 M6 own-work rows | **~77-107 weeks** + M5 4-6 weeks + M6 ~3-3.5 weeks | **~89-123 weeks** |
...
| M10: Confinement & Ecosystem (was M5) | 22-32 weeks | 67-94 weeks | Early September 2027 to late March 2028 |
| M11: Rust Daemon (`endor`) (was M6) | 15-22 weeks | 82-116 weeks | Q1 to Q3 2028 (research-heavy; may run in parallel) |
