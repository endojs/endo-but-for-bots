...
*Last updated: 2026-07-20 (added [ocapn-structural-protocol-core](ocapn-structural-protocol-core.md) to M4 (Networking): a three-tier refactor of `@endo/ocapn` — pure wire codecs over a narrow `ReferenceIO` seam, a structural `makeOcapnSessionCore` that speaks the protocol in slot descriptors without minting presences (the comms-vat/kernel tier the CBOR encoding's out-of-band-slots design goal anticipates), and the existing presence layer re-expressed as its default client — plus a `handoffs: false` point-to-point profile that omits 3PHO (trimmed bootstrap, no gift table, codec branch removed, session core constructed without session-graph powers); summary table, M4 row, dependency graph, and per-design estimate synced; milestone totals and Gantt deferred to the next journalist cycle.
...
[ocapn-structural-protocol-core](ocapn-structural-protocol-core.md)
(added 2026-07-20; splits `@endo/ocapn` into pure wire codecs over a
fifteen-method `ReferenceIO` seam, a sans-invoke structural session
core (`makeOcapnSessionCore`) exposing slot descriptors for
comms-vat/kernel consumers, and the presence layer
(Remotable/HandledPromise/E()) as its default client; hoists handoff
issuance/redemption out of codec read/write — which today performs
network I/O during (de)serialization — into message-boundary
transforms; adds a `handoffs: false` point-to-point profile with a
`fetch`-only bootstrap, no gift table, the `third-party:handoff`
codec branch omitted, and the session core constructed without
`provideSession`/`getActiveSession` session-graph powers),
...
| [ocapn-structural-protocol-core](ocapn-structural-protocol-core.md) | 2026-07-20 | 2026-07-20 | Proposed |
...
flowchart TD
    ...
    subgraph OCapN
        ...
        ostruct[ocapn-structural-protocol-core]
        onet --> otcp --> onoise
        orev --> onoise
        dnet --> onoise
        onoise --> okey
...
| ocapn-structural-protocol-core | Proposed | Three-tier `@endo/ocapn` refactor: pure codecs over a `ReferenceIO` seam, structural `makeOcapnSessionCore` (slot descriptors, no presences) for comms-vat consumers, presence layer as default client; plus a `handoffs: false` point-to-point profile without 3PHO |
...
| ocapn-structural-protocol-core | M-L | 1.5 weeks | 4 | Layered refactor: `ReferenceIO` extraction, handoff hoist to message boundary, structural session core, `handoffs: false` profile; four behavior-gated phases |
