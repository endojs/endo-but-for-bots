...
| [daemon-agent-tools](daemon-agent-tools.md) | 2026-03-02 | 2026-07-06 | In Progress |
...
| daemon-agent-tools | In Progress | Capability-layer map and build sequence for the M3 "Claw-like coding capabilities" pillar, reconciled 2026-07-06 against the landed mount/git trio. Filesystem and local-git tool groups shipped (`Filesystem` over `mountAsFilesystem` / `Git.filesystemAt`; `Git` via `provideGit`; tools via `@endo/agent-tools` #523/#524). Remaining phased work: the mount-derived `Shell` capability (`provideShell` over a writable `EndoMount`, `Spawner`-seam engine, sandbox-slice destination), list/edit/stat file tools, `makeGitRemoteTool` push tier, and provisioning wiring |
...
| daemon-agent-tools | M-L | 1.5 weeks | 3 | Reconciled 2026-07-06: fs read + git tool wrappers already landed via `endo-agent-tools` (#523/#524); remaining bulk is the `Shell` capability (`provideShell` formula + `makeShellTool` + hardening tests), the list/edit/stat file tools, `makeGitRemoteTool`, and provisioning wiring — roughly M of the original M-L remains |
