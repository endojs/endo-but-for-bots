...
*Last updated: 2026-07-21 (added [mount-denied-segments-cli](mount-denied-segments-cli.md): the deferred CLI surface for PR #650's `deniedSegments` mount-creation option, with explicit omitted/custom/empty states, help, and end-to-end enforcement coverage; summary, M3 roadmap, dependency graph, and estimate rows synced.
Layered on 2026-07-19 (revised [ocapn-noise-key-only-session-boundary](ocapn-noise-key-only-session-boundary.md) per review: OCapN, the relay, and the Noise listener are independent components. An application injects a network adapter, such as Noise over WebSocket, into OCapN; the relay depends on neither. The Node controller exo supplies relay configuration and `SIGHUP` replacement while remaining loosely coupled.
...
| [mount-denied-segments-cli](mount-denied-segments-cli.md) | 2026-07-21 | 2026-07-21 | Proposed |
...
flowchart TD
    ...
    subgraph Capability System
        ...
        mdeny[mount-denied-segments-cli]
...
| mount-denied-segments-cli | Proposed | Deferred CLI follow-up for PR #650's `deniedSegments` creation option: custom, omitted, and explicit-empty sets through external and scratch mount commands |
...
| mount-denied-segments-cli | S | 1 day | 3 | CLI plumbing, help, and end-to-end enforcement tests for the daemon-side deny-set override in PR #650 |
...
| M3: Remote Access & Tools (was M1) | 19 (`gateway-package`, `daemon-docker-selfhost`, `daemon-agent-tools`, `endo-agent-tools`, `agentry-agent-builder`, `agentry-git-verb-gaps`, `agentry-git-eval-scenarios`, `daemon-mount`, `mount-denied-segments-cli`, `daemon-worker-import-from-mount`, `registry-capability`, `mvs-resolver`, `snapshot-mapper`, `filesystem-watchers`, `daemon-locator-terminology`, `daemon-rename-to-manager`, `daemon-xs-worker-snapshot`, `endo-reminder` (was `endoclaw-timer`), `endo-fetch` (was `endoclaw-network-fetch`)) | 9-13 weeks | 11-15 weeks |
