...
|--------|---------|---------|--------|
| [rust-module-lexer-consolidation](rust-module-lexer-consolidation.md) | 2026-08-17 | 2026-08-17 | Not Started |
...
| cli-edit-verb | In Progress | `endo edit` with hashline patches for AI agents; sibling of `cli-store-verb-text-modes` (PR #153); pure core landed in #796 |
...
| rust-module-lexer-consolidation | Not Started | Consolidates `entry_walk`'s bespoke ESM static-import scanner and `cjs_lexer.rs` onto one allocation-light, cursor-driven `scan` submodule modeled on `@endo/cjs-module-analyzer`, sheds the `cjs_lexer` `Vec<Token>` retention, and establishes a shared cross-language (JS `ava` + Rust) fixture corpus with a drift guard. Answers the #282 review asking that `entry_walk` reuse an existing lexer, honor the allocation constraint, and share tests with the cjs-module-lexer fork. The shared `scan` primitives land on `llm` first as the canonical copy; #282 rebases to consume them. |
...
| rust-module-lexer-consolidation | M | 5-7 days | 11 | Re-costed 2026-08-17: the phase sketch is heavier than the `endor-native-zip-xs` (S-M, single-language) comparable. Shared two-tier `scan` cursor-primitive submodule (stateless advancers + regex-vs-divide and depth-gate resolvers, each with a genuine behavior change to get right), re-express *both* `entry_walk` and `cjs_lexer` scanners held to byte-identical parity + shed `cjs_lexer` `Vec<Token>`, a new cross-language `ava`/Rust fixture corpus with drift guard, and a dedicated single-threaded allocation-counting integration binary with a custom global allocator. Design-only PR; implementation phases span #282's branch and `llm`. Not on the M11 critical path (endor-tui/bus-tui dominate). |
