...
| endor-git-bindings | M | 4-5 days | 11 | Shared `GitStore` contract, Rust `git2` wrapper, vendored-libgit2 FFI boundary, Endor-tree adapter, corruption coverage, and Zig cross-build checks. Minion Town supplies its own smart-HTTP and CAS/SQLite adapters against the same crate and fixtures. |
