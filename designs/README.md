...
of `@endo/workflow` charts (branch `claude/endo-workflow-system-r58hrd`,
pending convergence with this branch), so the prompt-enforced invariants
(push-before-pin, build-before-apply, confirm-before-apply) become chart
structure, operator approval becomes a mail form in the owner's own inbox,
and the deploy's own daemon restart is recovered by journal refold instead of
orphaning the flow. Presets grant durable revocable workflow *factories*
(`deploy-endo`, `change-nixos`) whose endowments are a new `deploy-performer`
caplet (settlement-shaped, opId-idempotent wrapper over the `NixosAdmin`
spool) and the owner's `@self` handle; realizes
[hosted-endo-self-update-loop](hosted-endo-self-update-loop.md) phases 5–6.
Summary table and totals synced.*
