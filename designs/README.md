...
[ironhorse-native-lockdown](ironhorse-native-lockdown.md) (added 2026-09-16;
the scope definition for a guest-callable `lockdown()` in `ironhorse-vm` —
...
and means the `ses-xs-parity` lockdown case still will not run on
`test262:ironhorse`, only start. Carries the measured starting state rather
than a reading of comments, and corrects two that described the unbound
`lockdown` as a `Halt::NotImplemented` scope fold when a reference is an
ordinary `ReferenceError`. Five open decisions gate the work, the first being
whether a guest `Compartment` is in after all),
...
| [ironhorse-native-lockdown](ironhorse-native-lockdown.md) | 2026-09-16 | 2026-09-16 | Proposed |
...
flowchart TD
    ...
    subgraph Ironhorse Snapshot Schema
        ...
        ihengine --> ihses
        ihw6 --> ihses
        ihlock[ironhorse-native-lockdown<br/><i>PROPOSED</i>]
        ihses --> ihlock
        ihengine --> ihlock
    end
...
| [ironhorse-native-lockdown](ironhorse-native-lockdown.md) | Proposed | The scope definition for a guest-callable `lockdown()` in `ironhorse-vm`: `xsLockdown.c`'s `fx_lockdown` transliterated, on the same oracle-locked terms as the rest of the port. States two boundaries explicitly, because both are the obvious thing to assume otherwise. It is **not** SES's `lockdown()` — that is a permits table, `removeUnpermittedIntrinsics`, override enablement, six taming modules and fifteen options against XS's five steps and no options — and it does **not** include a guest `Compartment`, which puts two of the five steps (the compartment-global template, and `Compartment.prototype`'s share of the constructor rewiring) out of scope and means the `ses-xs-parity` `Symbol.toStringTag-lockdown.js` case still will not pass on `test262:ironhorse`; what this work buys that lane is the ability to START, since `endot-ih -l` refuses today. The substance is step 2, the constructor rewiring: IronHorse has no `%ThrowTypeError%` to duplicate and no privileged slot-write path, and step 2 is what would close the measured hole where `({}).constructor.constructor('return 1+1')()` evaluates to `2` under `global_names: Some(vec![])`. Step 5 is already present and wider than XS's. Carries the measured starting state rather than a reading of comments, and corrects two comments that described the unbound `lockdown` as a `Halt::NotImplemented` scope fold when there is no dispatch and a reference is an ordinary `ReferenceError`. Five open decisions gate the work; binding the global also moves `Interp::boot_fingerprint`, so the `ironhorse-snapshot` golden identity fixtures regenerate and prior snapshots are refused at restore. Adds no scope — the native route is one half of the choice `ironhorse-engine` roadmap stage 4 already counts. |
...
| ironhorse-native-lockdown | — | not estimated; the definition gates the estimate | M11 | Sizing waits on the document's open decision 1 — whether a guest `Compartment` is in scope — which changes the project by more than the other four decisions together: `fx_lockdown` alone against `fx_lockdown` plus `fx_Compartment` (`xsModule.c:2864`). Two costs are known and should be carried into whatever estimate follows: a new inert-constructor native with a privileged slot-write path, and the `ironhorse-snapshot` golden-fixture regeneration that binding one global forces, whose precedent is PR #1279's ~200 digests across six fixtures. This is the native half of `ironhorse-engine` roadmap stage 4 and is already counted there; no milestone total or timeline change is assigned. |
