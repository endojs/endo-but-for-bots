...
[daemon-git-clone](daemon-git-clone.md) (added 2026-07-12; the design
record for the host-mediated repository bootstrap — `provideGitClone`
composing a writable, empty destination mount x a sealed
`GitRemoteEndpoint` into a fresh `Git` plus an origin-pre-bound
`GitRemote`, shipped via #538 — and the paired formula-owned
commit-identity boundary (`identity: { authorName, authorEmail }`,
guest-immutable, defaulting to `Endo <endo@invalid.local>`) in flight
as PR #706),
...
Open Work bootstrap item repointed 2026-07-12 at the now-written
[daemon-git-clone](daemon-git-clone.md);
...
| [daemon-git-remotes](daemon-git-remotes.md) | 2026-05-18 | 2026-07-12 | Proposed |
| [daemon-git-next-steps](daemon-git-next-steps.md) | 2026-05-27 | 2026-07-12 | Proposed |
| [daemon-git-clone](daemon-git-clone.md) | 2026-07-12 | 2026-07-12 | In Progress |
...
flowchart TD
    ...
    subgraph Capability System
        ...
        dgitclone[daemon-git-clone<br/><i>IN PROGRESS</i>]
...
| daemon-git-next-steps | Proposed | The version-controlled filesystem loop milestone over the canonical trio: north-star agent loop (provide workspace → read/list/edit → status/diff → commit → pull/push → inspect history via `filesystemAt(ref)`) and the content/versioning/network/historical-read/bulk-storage layer split. Open `- [ ]` work: worked bot-fork reference flow, `provideGitClone` + identity boundary (→ [daemon-git-clone](daemon-git-clone.md)), `tree(ref)`/`filesystemAt(ref)` reconciliation. Agent-tools layer deferred to #416 |
| daemon-git-clone | In Progress | Design record for the host-mediated repository bootstrap: `provideGitClone({ destMount, endpoint, identity? })` composes a writable, empty destination mount with a sealed `GitRemoteEndpoint` (non-extractable credential, opt-in `file:` transport, `https:`-only otherwise) into a fresh formula-backed `Git` plus an `origin`-pre-bound `GitRemote`; shipped via #538 (`gitClone` in `@endo/git`, `makeGitCloner` in `@endo/exo-git`, `provideGitClone` in `@endo/daemon`). Paired formula-owned commit-identity boundary (`identity: { authorName, authorEmail }`, guest-immutable, default `Endo <endo@invalid.local>`, `reword` preserves original authors) in flight as PR #706. Open questions: pet-name binding for the cloned pair, clone-time narrowing of the `origin` policy |
...
| daemon-git-clone | S | 1-2 days | 3 | Design record of shipped work: the `provideGitClone` bootstrap seam landed via #538 and the commit-identity boundary is PR #706. Remaining effort is landing #706 plus resolving the pet-name-binding and origin-policy-narrowing open questions |
...
| M3: Remote Access & Tools (was M1) | 19 (`gateway-package`, `daemon-docker-selfhost`, `daemon-agent-tools`, `endo-agent-tools`, `agentry-agent-builder`, `agentry-git-verb-gaps`, `agentry-git-eval-scenarios`, `daemon-git-clone`, `daemon-mount`, `daemon-worker-import-from-mount`, `registry-capability`, `mvs-resolver`, `snapshot-mapper`, `filesystem-watchers`, `daemon-locator-terminology`, `daemon-rename-to-manager`, `daemon-xs-worker-snapshot`, `endoclaw-timer`, `endoclaw-network-fetch`) | 9-13 weeks | 11-15 weeks |
