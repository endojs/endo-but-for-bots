...
| [endo-guest-stdio-mcp](endo-guest-stdio-mcp.md) | 2026-09-08 | 2026-09-08 | Not Started |
...
| **endo-guest-stdio-mcp** | Not Started | **Client-side bridge (arc [kriscendobot/garden#89](https://github.com/kriscendobot/garden/issues/89) item 5).** The stdio MCP server the confined `endo-claude` process reaches: a claude-spawned adapter (holds no facet fd) plus a harness-owned facet broker (resolves the 64-hex formula id to one guest facet at construction, holds the attenuated CapTP connection, pins the pruned `tools/list` snapshot, rejects any `tools/call` outside it). Owns the server side of `endo-claude`'s pinned-catalog contract and the server-hosting seam it names as a prerequisite; the neighbouring MCP designs (`endo-gateway-mcp`, `daemon-agent-tools`, and the minion.town trio) are all HTTP-plus-OAuth. Composes with the `@endo/agent-tools` MCP-adapter projection (`endo-agent-tools`, this milestone). Scoping is per-process, not per-bearer: the formula id never rides the wire, so the confined process holds no designator to forge. Fail-closed: empty/underivable catalog is a construction throw. Names honor the reconciled flat namespace from `kriscendobot/minion.town` PR #79. |
...
| endo-guest-stdio-mcp | S-M | 3-5 days | 3 | The stdio MCP server side of `endo-claude`: a claude-spawned stdio adapter (MCP framing loop, holds no facet fd) plus a harness-owned facet broker (formula-id resolution, facet attenuation, pinned pruned catalog, server-side dispatch check, harness-private channel). Design-only for now; the projection it hosts is the `@endo/agent-tools` MCP adapter (`endo-agent-tools`, prerequisite), so the net-new build bulk is the two-process seam and the fail-closed construction guards, not the projection. Estimate is the design's own build slice; the adapter-implementation prerequisite is counted under `endo-agent-tools` / `endo-claude` |
