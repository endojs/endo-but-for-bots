...
| endor-registry-transport | Proposed | Endor/XS transport backend behind the shared `RegistryBackend` shape: three Rust host functions plus a `makeRegistryXsPowers` adapter reusing the JS backend. Depends on registry-capability (M3). |
...
| M11: Rust Daemon (`endor`) (was M6) | 5 (`endor-git-bindings`, `endor-tui`, `endor-bus-tui`, `endor-native-zip-xs`, `endor-registry-transport`) | 14-20 weeks | 16-22 weeks |
| **Total remaining** | **61** + 7 M5 rows (4 in-flight + 3 design gaps) + 1 M6 own-work row | **~59-81 weeks** + M5 4-6 weeks + M6 ~2 weeks | **~71-97 weeks** |
