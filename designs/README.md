...
*Last updated: 2026-07-13 (added [ocapn-iroh-netlayer](ocapn-iroh-netlayer.md) to M4 (Networking), landing **Complete** with its implementation: an iroh 1.0 QUIC netlayer for `@endo/ocapn` shipped as `@endo/ocapn-iroh` — "dial keys, not IPs": the OCapN designator is the peer's iroh `EndpointId` (a 32-byte Ed25519 key) resolved through iroh discovery/relays to a mutually authenticated, encrypted QUIC connection carrying netstring-framed OCapN messages under the `ocapn/netstring/0` ALPN, with the standard `op:start-session` handshake run by the OCapN client, a datagram keep-alive mirroring the daemon iroh transport, an injectable binding surface so CI tests run against an in-memory mock (real two-endpoint integration gated behind `ENDO_IROH_INTEGRATION=1`), and two new `@endo/ocapn` subpath exports (`./client/types`, `./client/util`) for out-of-tree netlayers; summary table, M4 bucket, dependency graph, per-design estimate, and totals synced. Layered on 2026-07-19 (revised [ocapn-noise-key-only-session-boundary](ocapn-noise-key-only-session-boundary.md) per review: OCapN, the relay, and the Noise listener are independent components. An application injects a network adapter, such as Noise over WebSocket, into OCapN; the relay depends on neither. The Node controller exo supplies relay configuration and `SIGHUP` replacement while remaining loosely coupled. Summary and estimate rows synced.
...
iroh `EndpointId`, dialed through iroh discovery/relays into a
mutually authenticated encrypted QUIC connection carrying
netstring-framed OCapN messages under the `ocapn/netstring/0` ALPN;
session establishment stays the client's standard `op:start-session`
handshake; a datagram keep-alive mirrors the daemon iroh transport;
the `@number0/iroh` binding is optional and injectable, so the
end-to-end tests run against an in-memory mock in CI with a
real-endpoint integration test gated behind
