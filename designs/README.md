...
[propagator-cells](propagator-cells.md) (added 2026-06-25; a
Radul/Sussman propagator network at the Endo layer (`@endo/propagator`:
cells holding lattice-merged partial information, multidirectional
propagators, a run-to-quiescence scheduler, an injected `Lattice`
contract, hardened with explicit neighbor-graph teardown) lifted to
passable Exo objects (`@endo/exo-propagator`: `CellReader`/`CellWriter`/
`CellAdmin` facets, cross-vat eventual merge that stays monotone so
reorder/duplicate/loss converge — the idempotent-convergence member of
the change-propagation family beside *latest* and *changes*); generalizes
FRB `<->` two-way bindings; truth maintenance left as a seam; full
milestone / dependency-graph / Gantt integration deferred to a grooming
pass since the design is DRAFT and unscheduled),
...
|--------|---------|---------|--------|
| [propagator-cells](propagator-cells.md) | 2026-06-25 | 2026-06-25 | Proposed |
