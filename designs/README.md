...
Hofman's no-orthogonal-persistence worker model; recommends the **batch-flush
retention root** as a specialization of CapTP question/answer refcounting
(nothing new on the OCapN wire for the two-party case; multi-party deferred to
...
authoritative count is the **Current totals (2026-08-16)** line below: 150 ->
151, Proposed 32 -> 33 (the older, higher-count **Current totals (2026-07-22)**
line is a prior lineage the 2026-08-16 line already supersedes).*
...
[worker-retention-and-revocation](worker-retention-and-revocation.md) (added
2026-08-16, **Proposed**; a research / reassessment of the daemon's
kill-on-sweep worker revocation against SwingSet/`ocap-kernel`, E, and
Hofman's no-orthogonal-persistence worker model, recommending the
**batch-flush retention root** as a specialization of CapTP question/answer
refcounting — nothing new on the OCapN wire for the two-party case, multi-party
deferred behind the three-party-handoff embargo — positioning worker discipline
as a per-worker constraint on the incarnation formula and scoping (not solving)
durable pending-promises; no proposed path makes a formula identifier
guest-observable),
...
| [worker-retention-and-revocation](worker-retention-and-revocation.md) | 2026-08-16 | 2026-08-16 | Proposed |
...
flowchart TD
    ...
    subgraph OCapN
        ...
        onoise --> oortho
        oreconn --> oortho
        wretn[worker-retention-and-revocation<br/><i>PROPOSED</i>]
        oortho --> wretn
    end
...
| worker-retention-and-revocation | Proposed | **Research / reassessment** of the daemon's kill-on-sweep revocation stance against SwingSet/`ocap-kernel`, E, and Hofman's no-orthogonal-persistence worker model. Milestone home is M4 (Networking): its core, the **batch-flush retention root**, is CapTP question/answer retention and it builds directly on `ocapn-orthogonal-persistence`. Recommends building that root as a specialization of CapTP question/answer refcounting (a `question:<session>:<answer-pos>`-labeled edge refining the landed refcounted transient pin from single-operation to single-question lifetime), released on resolution plus the absence of any cross-peer import edge, bounded by session-abort plus a per-root lease over the whole retention interval plus per-session admission cap. Needs nothing from the OCapN wire for the two-party case, though it does need a new `@endo/captp` question-observation seam; defers multi-party (three-party-handoff embargo) explicitly. Takes positions on worker discipline (an explicit per-worker constraint on the incarnation formula, modeled as a discriminated union of coherent shapes) and durable pending-promises (scoped, not solved). No formula identifier is guest-observable on any proposed path (and it flags the pre-existing general-guest `identify`/`lookupById` exposure as a separate open question). |
...
| worker-retention-and-revocation | S | research doc | 4 | Research/reassessment (no build); lands recommendations on kill-the-worker revocation, worker discipline as constraint, durable pending-promises, and the batch-flush retention root (a `question:<session>:<answer-pos>`-labeled edge refining the landed refcounted transient pin from single-operation to single-question lifetime, released on resolution plus the absence of any cross-peer import edge). The buildable daemon-internal edge is a follow-up over `daemon-retention-paths` plus a new `@endo/captp` question-observation seam, and does not depend on the Proposed `makeRetainedValue` surface (that handle is Thread 2's guest-facing sugar); multi-party scope deferred behind three-party-handoff embargo |
