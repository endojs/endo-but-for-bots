...
| reminder-integration-chat | S-M | 3-4 days | 7 | Integrate `@endo/reminder` into Chat: reminder-courier caplet (`notify` → `send('@host', …)`), `setup-reminder.js` provisioning + attenuation, `/remind-every` chat-bar affordance; surfacing is free via `followMessages`. Shares the deployment-resolvable-specifier blocker with the Familiar/minion.town sibling plans |
