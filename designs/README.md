...
the review, which closed seven of the fifteen and collapsed the confinement
track that was to gate the SES bar into a single ungated Intl residue; decides
...
| ironhorse-daemon-acceptance-sequencing | XL | 6.5–9.5 developer weeks, critical path | M11 | Sum of the design's own per-phase sizes: M (typed error channel) + L (engine trait) + L (service adapters) + XL (SES bundle bar) + L (worker envelope) + S (reconciliation), with one ungated S (the Intl residue) outside the path. Dominated by the SES bar, which is `ironhorse-engine` roadmap stage 4 and is already counted there; this row sequences that scope rather than adding to it, so no milestone total or timeline change is assigned. |
