...
| [endo-guest-stdio-mcp](endo-guest-stdio-mcp.md) | 2026-09-08 | 2026-09-17 | Not Started |
...
| endo-guest-stdio-mcp | S | 2-4 days | 3 | The stdio MCP server side of `endo-claude`: a single claude-spawned process (MCP framing loop, env-threaded formula id, ordinary daemon-client resolution of the one guest facet, pinned pruned catalog, server-side dispatch check). Design-only for now; the projection it hosts is the `@endo/agent-tools` MCP adapter (`endo-agent-tools`, prerequisite), so the net-new build bulk is the config threading and the fail-closed construction guards, not the projection. Revised 2026-09-17 to drop the two-process broker / per-guest socket, which shrinks the build slice. Estimate is the design's own build slice; the adapter-implementation prerequisite is counted under `endo-agent-tools` / `endo-claude` |
...
| M4: Networking (was M2) | 11 (`ocapn-network-transport-separation`, `ocapn-tcp-for-test-extraction`, `ocapn-tcp-syrup-framing`, `cbor-frame`, `cbor-codec`, `slots-ocapn-op-lanes`, `ocapn-noise-cryptographic-review`, `daemon-agent-network-identity`, `thixotrope`, `thixotrope-on-cloudflare`, `thixotrope-on-cloudflare-addendum-single-vat-hub`) | 5-6 weeks; operation-lane reconciliation is already in flight | 6-8 weeks |
...
| **Total remaining** | **68** + 7 M5 rows (4 in-flight + 3 design gaps) + 2 M6 own-work rows | **~61-83 weeks** + M5 4-6 weeks + M6 ~3-3.5 weeks | **~74-101 weeks** |
