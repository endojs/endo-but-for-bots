...
| [exports-extensionless-migration](exports-extensionless-migration.md) | 2026-07-10 | 2026-07-10 | Not Started |
...
| exports-extensionless-migration | Not Started | Two-pass additive migration to extensionless `exports` subpaths (added 2026-07-10, reopening M2): pass 1 adds an extensionless sibling for every explicit `.js` exports key (235 keys, minor bump per package, zero break, `.js` keys retained with a removal-reserved-on-major changeset note); pass 2 (blocked on pass 1) rewrites ~967 eligible cross-package specifiers, leaving relative imports and the 22 wildcard-resolved sites on `.js`; idempotent codemods with CI-armed `--check` gates; each pass a parallel per-package-tranche orchestration |
...
with one additive row, `exports-extensionless-migration` (Not Started):
the extensionless-`exports` two-pass migration is exactly this bucket's
kind of substrate hygiene — not user-facing capability, cleans the
import-specifier surface every other milestone's commits touch.
...
| exports-extensionless-migration | M | 4-6 days | 2 | Two scripted, idempotent codemods plus CI gates and a dual-resolution probe; the effort is tranche-PR shepherding (pass 1: ~7 package.json tranches + arming child; pass 2, blocked on pass 1: specifier tranches + arming child), not the code |
...
| M2: Project Hygiene (was M½) | 1 (`exports-extensionless-migration`; reopened 2026-07-10 — original six rows Complete) | ~1 week | ~3 weeks |
...
| M2: Project Hygiene (was M½) | ~1 week (reopened 2026-07-10 with `exports-extensionless-migration`; original scope Complete 2026-06-15; runs in parallel with M3, no critical-path shift) | — | Opportunistic; does not move M3's target |
