...
three-party-handoff embargo); positions worker-type-as-constraint on the
incarnation formula; and scopes (does not solve) durable pending-promises. No
proposed path makes a formula identifier guest-observable. Summary table row and
M3 milestone row synced; not placed on the Gantt/critical path (a reassessment,
not a build target). Proposed +1, total 151 → 152.*
...
| worker-retention-and-revocation | Proposed | **Research / reassessment** of the daemon's kill-on-sweep revocation stance against SwingSet/`ocap-kernel`, E, and Hofman's no-orthogonal-persistence worker model. Recommends building the **batch-flush retention root** as a specialization of CapTP question/answer refcounting (a `question`-labeled edge refining the captp-bounded transient pin from connection-lifetime to single-question-lifetime), bounded by session-abort + per-root lease + per-session admission cap — needing nothing from OCapN for the two-party case; defers multi-party (three-party-handoff embargo) explicitly. Takes positions on worker-type-as-constraint (an explicit per-worker `persistence`/`upgrade`/`identity` constraint on the incarnation formula) and durable pending-promises (scoped, not solved). No formula identifier is guest-observable on any proposed path. |
...
| worker-retention-and-revocation | S | research doc | 4 | Research/reassessment (no build); lands recommendations on kill-the-worker revocation, worker-type-as-constraint, durable pending-promises, and the batch-flush retention root (a `question`-labeled edge refining the landed refcounted transient pin from single-operation to single-question lifetime, released on resolution). The buildable batch-flush root is a follow-up over `daemon-retention-paths` + the Proposed `makeRetainedValue` surface; multi-party scope deferred behind three-party-handoff embargo |
