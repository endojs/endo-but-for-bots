...
(single-call `defineAgent` and generic `makeCodeModeAgent`) landed in #517;
`prepareCodeMode` now makes host and repository access independent setup axes,
while general discrete-tool attenuation/wire/compaction remains aspirational),
...
| agentry-agent-builder | In Progress | `@endo/agentry` agent builder for constructing a caller's own lal; core `defineAgent` and the generic `makeCodeModeAgent` landed in #517; `prepareCodeMode` independently selects in-process or daemon evaluation and `inspect`, `edit`, or `rewriteHistory` repository authority; repository-specific agents stay internal to evals; general discrete-tool selection, attenuation, wire derivation, compaction, steering, discovery, and per-harness bundles remain aspirational |
