...
**Current totals (2026-09-04, reconciliation refresh):** 49 Complete/Implemented, 39 In Progress, 49 Not Started, 38 Proposed, 2 Active, 13 Reference, 3 Deprecated, 1 Draft, 4 Superseded, 1 Approved (`ironhorse-engine`), plus [cbor-codec](cbor-codec.md) at *Phase 1 implemented* and [genie-integration](genie-integration.md) at *Largely realized* (201 designs). This supersedes every count above, including the prior 2026-08-29 block; the buckets are reproducible by tallying the Status column of the summary table. This pass reconciles the drift the layered per-rebase delta notes (2026-08-25 through 2026-09-03) had accumulated against the stale 2026-08-29 header: those notes each incremented a running design count that had already fallen behind the table, so the header buckets and the delta lineage no longer agreed. Re-tallying the Status column of the summary table yields 201 rows against the header's 196 — Complete/Implemented 48 -> 49, In Progress 36 -> 39, Proposed 37 -> 38, the other buckets unchanged — accounting for every design the delta notes added since (hardener-indexed-cardinality, genie-integration re-add, exo-git-follow-root-advancement, npm-dev-publisher-attenuation, endo-workflow with its Proposed -> In Progress flip, and daemon-secret-manager as *Implemented (local backend)*). Per the index's single-current-totals-block rule those superseded notes are folded into this block and moved to [`ARCHIVE.md`](ARCHIVE.md).
