...
**Current totals (2026-09-04, reconciliation refresh):** 49 Complete/Implemented, 39 In Progress, 49 Not Started, 38 Proposed, 2 Active, 13 Reference, 3 Deprecated, 1 Draft, 4 Superseded, 1 Approved (`ironhorse-engine`), plus [cbor-codec](cbor-codec.md) at *Phase 1 implemented* and [genie-integration](genie-integration.md) at *Largely realized* (201 designs). This supersedes every count above, including the prior 2026-08-29 block; the buckets are reproducible by tallying the Status column of the summary table, which is how this pass re-derived them. The 2026-08-29 header was never a function of that table: it claimed 196 designs while the table beneath it already held 198 rows, and the per-rebase delta notes that followed each incremented a running count off that already-stale header instead of re-tallying. The prior 2026-08-29 block and its folded delta lineage now live in [`ARCHIVE.md`](ARCHIVE.md).
