...
| [endo-daemon-aws-storage](endo-daemon-aws-storage.md) | 2026-07-08 | 2026-07-08 | Proposed |
...
| [endo-daemon-aws-storage](endo-daemon-aws-storage.md) | Proposed | Daemon-level AWS storage platform: DynamoDB behind the synchronous `DaemonDatabase` engine seam (in-memory mirror, write-behind flush), S3 behind the `ContentStore` contract (`size`/`readRange`); scaffold, emulators, and SQLite-parity tests land with the design; complements gateway-aws-attuned's gateway-level substitutions |
...
| endo-daemon-aws-storage | M | 4-5 days | 5 | Daemon storage on DynamoDB + S3; engines and parity tests scaffolded with the design, remaining effort is flavour wiring, emulator-fidelity runs, and the reference deployment |
