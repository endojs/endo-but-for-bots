...
| [hex-native-dispatch-opt](hex-native-dispatch-opt.md) | 2026-07-10 | 2026-07-10 | Proposed |
...
| hex-native-dispatch-opt | Proposed | Post-completion follow-up (does not reopen the M2 exit criterion): platform-conditional `@endo/hex` dispatch per the approving review of PR #580 — native `Uint8Array.fromHex`/`toHex` first everywhere, char-code default pure-JS fallback, char-pair-`Map` fallback behind the `xs` exports condition (2.2–2.6× cheaper metered compute on XS per `benchmarks/hex-decode-codec-comparison/REPORT.md`) |
...
| hex-native-dispatch-opt | S | 1-2 days | 2 | Post-completion M2 follow-up, timeline-neutral: two `src/*-xs.js` modules + `exports` conditions + second hex-test pass; encode fallback gated on a metered-xsnap measurement |
