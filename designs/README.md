...
| reminder-integration-chat | S-M | 3-4 days | 7 | Integrate `@endo/reminder` into Chat: reminder-courier caplet (`notify` -> `send('@host', ...)`), `setup-reminder.js` two-principal provisioning + attenuation, `getReminder(id)`/`limits()` scheduler getters, `/remind-every` + `/reminders` chat-bar affordances; surfacing is free via `followMessages`. Blockers: the `EndoMount`-vs-extended-fs `Directory` store contract (`fs-interface-reconciliation` In Progress) plus a shared `@endo/reminder` specifier + courier archive with the Familiar/minion.town sibling plans |
