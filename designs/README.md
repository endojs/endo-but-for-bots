...
| [ironhorse-engine](ironhorse-engine.md) | 2026-07-02 | 2026-09-08 | Approved |
...
| [endo-claude](endo-claude.md) | 2026-08-16 | 2026-09-08 | In Progress |
...
- **The confined in-guest agent** — an agent running *inside* a guest,
  ...
  *is* an Endo guest's inference engine, its Claude tool surface replaced by
  ...
  narrowing pending maintainer confirmation, not yet decided; `endo-claude.md` DD6).
  Tool-surface-substitution core in flight as PR
...
| **endo-claude** | In Progress | **Client-side bridge, top priority (carved 2026-09-03; moved here from M6).** An unconfined host caplet that launches fresh `claude -p --bare` processes and replaces Claude's default tool surface with one guest's local MCP tools. The candidate stack is `--bare` + `--strict-mcp-config` + `--setting-sources ""` + `--tools ""` + `--disable-slash-commands`, literal catalog-derived `mcp__<server>__<tool>` allows, and never resume; current CLI documentation also supplies `--permission-prompts none` and `--no-session-persistence`. Draft PR [#1015](https://github.com/endojs/endo-but-for-bots/pull/1015) contains the dependency-injected core but no production spawn or live substitution test. The one-guest stdio MCP pieces landed in #1206 and are now the explicit one-guest MCP surface interface #1015 must consume. This narrowing of the boundary to tool-surface substitution (OS sandboxing as optional hardening, not a prerequisite) is proposed, pending maintainer confirmation, not yet decided (`endo-claude.md` DD6, § Open questions). |
...
| endo-claude | M | 1-1.5 weeks | 6 | `@endo/claude`: an unconfined caplet spawning fresh `claude -p --bare` children whose Claude tool surface is replaced by one guest's local MCP catalog. #1015 implements the injected argv/env/catalog/pool spine but still lacks production spawn effects and a live positive-and-negative test. It consumes the one-guest stdio MCP lifecycle now present on `llm` after #1206; it does not carry a second shim or wait on the gateway. Optional `@endo/claude-sandbox` wrapping adds OS confinement separately. Load-bearing residuals are the subscription-through-`apiKeyHelper` mechanism, entitlement, and live proof that MCP survives while built-ins are denied. |
