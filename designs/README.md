...
| [ironhorse-panic](ironhorse-panic.md) | 2026-08-17 | 2026-08-29 | Proposed |
...
| ironhorse-panic | Proposed | Names and extends the **panic**: an uncatchable vat/worker termination no `try`/`catch`, promise handler, or engine path can intercept. `Halt::StackOverflow` and `Halt::MeterAbort` are already panics; the formal category retains diagnostic variants, adds `is_panic()` plus `Halt::Panic(PanicKind)`, and surfaces `CrankOutcome` at the prospective Ironhorse `Machine` seam. Adds the missing recovery foundation: one SQLite WAL database per worker records snapshot watermarks, inbound/outbound messages, crank commits, host-call requests/replies, and logical host-handle reconstruction descriptors. Outbound messages remain embargoed until their crank transaction commits; replay restores the last snapshot, suppresses recorded sends, returns recorded host replies, re-seats reconstructible handles, and stops on divergence or a non-replayable effect. Debugger panics use a distinct `<panic>` message, orthogonal to `setExceptionBreakMode`. The off-by-default reference-error Coda captures the fault PC before unwind. |
