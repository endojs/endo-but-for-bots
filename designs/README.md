...
`test262:ironhorse`, only start. Steps 1, 2 and 5 landed; measured to agree
with XS's own `fx_lockdown` across 6053 files under `endot-ih -l`, with
byte-identical failure sets. Closes the reach where
`({}).constructor.constructor('return 1+1')()` evaluated to 2 under
`global_names: Some(vec![])`. Carries the measured starting state rather than a
...
| [ironhorse-native-lockdown](ironhorse-native-lockdown.md) | 2026-09-16 | 2026-09-16 | **Complete** |
