...
2026-08-06; a durable daemon session retains one canonical workspace and
independent workspace, Git, package-manager, and Web grants, while safe
fixed-argv install, mediated public research, and brokered HTTPS Git run on the
existing host path without executing untrusted package code),
...
| session-execution-capabilities | L | 3-5 weeks | 3 | Six useful cuts: portable package-manager predecessors, canonical session grants plus safe host install, mediated Web research, public HTTPS Git remotes, authenticated Git operations, and endo-pi acceptance. Builds on the existing platform filesystem, mount, native Git, HTTP confinement, agent-tools, and agentry seams; the optional sandbox backend is deliberately off the critical path. |
