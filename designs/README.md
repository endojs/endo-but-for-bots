...
| [gateway-bearer-token-auth](gateway-bearer-token-auth.md) | 2026-03-02 | 2026-07-06 | In Progress |
...
| Design | Status | Notes |
|--------|--------|-------|
| gateway-bearer-token-auth | In Progress | Agent ID as bearer token; the `fetch(token)` gate + rate limiter shipped in `ws-gateway.js`, but the `ENDO_GATEWAY` remote/CIDR admission wiring is unbuilt (`cidr.js` exists, tested, unimported). Status corrected 2026-07-06 from Implemented; remaining Phase A is the M3 remote-control keystone and unblocks PR [#608](https://github.com/endojs/endo-but-for-bots/pull/608)'s remote-access follow-up |
...
| gateway-bearer-token-auth | S | 1-2 days | 3 | In Progress (corrected 2026-07-06 from ✅ Implemented); remaining Phase A wires the existing tested `cidr.js` admission checker into `ws-gateway.js` plus remote-mode tests |
