...
| [endo-daemon-cloudflare-storage](endo-daemon-cloudflare-storage.md) | Proposed | Daemon storage powers on Cloudflare: DO SQLite as a third `daemon-database.js` engine (`better-sqlite3-do.js` shim), R2 content store via `makeR2FilePowers`, toy scaffold + tests on branch `design/endo-daemon-cloudflare-storage`. Sibling of the AWS storage design (`endo-daemon-aws-storage`); estimate ~1-2 weeks for build phases 1-2 (miniflare rig + large-blob path), the DO-hosted daemon and control powers are separate later work. |
