...
| [ironhorse-guest-compartment](ironhorse-guest-compartment.md) | 2026-09-18 | 2026-09-18 | Not Started |
...
flowchart TD
    ...
    subgraph Ironhorse Snapshot Schema
        ...
        ihses --> ihlock
        ihengine --> ihlock
        ihcompart[ironhorse-guest-compartment<br/><i>NOT STARTED</i>]
        ihlock --> ihcompart
        ihses --> ihcompart
    end
...
| ironhorse-guest-compartment | L | not estimated pending three decisions | M11 | Sized L-XL and not estimable until the note's open decisions are taken: the instance-ownership shape, template versus creation-time attenuation, and whether phase 2's intrinsics are minted in phase 1's boot change so the fingerprint moves once rather than twice. Phase 1 is 12 corpus files and independently shippable; phase 2 is 56 plus `test/modules/`'s 7 and carries both prerequisites (D2 property-override enablement, and referrer threading through `ModuleGraph::resolve`, which takes one argument today while `descriptors/source/specifier.js` asserts the referrer directly), so sizing phase 1 and calling it "the Compartment" would under-report by roughly 4x. This is the remainder `ironhorse-native-lockdown` fenced off and is already counted under `ironhorse-engine` roadmap stage 4; no milestone total or timeline change is assigned. |
