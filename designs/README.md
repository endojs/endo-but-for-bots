...
| **daemon-guest-bot-incarnation** | Not Started | **Prerequisite of the in-guest agent (carved 2026-09-08).** A general daemon primitive binding an Endo guest to a bot capability, so accepting a message for that guest makes a best effort to keep exactly one bot incarnation running without letting the message's durable acceptance depend on whether the bot starts. Optional `GuestFormula.bot` edge + `provideGuest(..., { bot })`, the guarded `EndoBot` start/result protocol, a single-flight incarnation supervisor woken by a post-commit mailbox hook, lazy restart recovery for retained mail, crash backoff with a rolling-window crash-loop breaker + typed credential/admission/operator blockers, `getBotStatus`/`stopBot`/`retryBot` host methods, and retained-child quota invariants. Daemon-only surface with no Claude/credential/minion.town policy; **`endo-claude` (#1015) composes with it** and it defers idle suspension to [daemon-xs-worker-snapshot](daemon-xs-worker-snapshot.md). Design in flight as PR [#1227](https://github.com/endojs/endo-but-for-bots/pull/1227). |
...
| daemon-guest-bot-incarnation | M | 1-1.5 weeks | 3 | Daemon primitive binding a guest to an `EndoBot`: optional `GuestFormula.bot` edge + guest-only `provideGuest` option (splitting the shared `normalizeHostOrGuestOptions` helper), the guarded start/result protocol, a single-flight incarnation supervisor woken by a post-commit `mail.js` hook, lazy restart scan over retained mail, full-jitter backoff + rolling-window crash-loop breaker (one persisted boolean per guest) + typed credential/admission/operator blockers, and `getBotStatus`/`stopBot`/`retryBot` host methods with lifecycle diagnostics. Testable with a fake `EndoBot` (no build dependency). Bulk is the supervisor state machine and the breaker/restart accounting; prerequisite `endo-claude` composes with it, so it lands ahead of #1015 on the same critical path rather than adding to it. |
...
| M3: Remote Access & Tools (was M1) | 20 (`gateway-package`, `daemon-docker-selfhost`, `daemon-agent-tools`, `endo-agent-tools`, `agentry-agent-builder`, `agentry-git-verb-gaps`, `agentry-git-eval-scenarios`, `exo-git-follow-root-advancement`, `daemon-mount`, `daemon-worker-import-from-mount`, `npm-registry-as-directory-tree`, `mvs-resolver`, `snapshot-mapper`, `filesystem-watchers`, `daemon-locator-terminology`, `daemon-rename-to-manager`, `daemon-xs-worker-snapshot`, `daemon-guest-bot-incarnation`, `endoclaw-timer`, `endoclaw-network-fetch`) | 9-13 weeks | 11-15 weeks |
