...
| hosted-agent-sandbox-unification | In Progress | Revocable inference grants and shared Claude/OpenCode MCP protocol/framing/relay with simultaneous tool admission and interruption fencing, plus shared cleanup/session ownership and generic public egress services with one listener image landed. Codex now selects external turn sandboxing and disables its managed proxy; the proxy now binds loopback and the privileged address helper is removed; the shared network contract and OpenCode public broker grants are available. Podman now retains failed removals and bwrap retains children until stdio closes, both fencing pending acquisition through the common ownership registry; factory disposal now permanently fences admission and retains failed cleanup for retry without rewriting process history. A host-only factory kit tracks pending construction/probes, retains late driver contexts, and stops existing slices while draining acquisitions; runtime storage-owner composition remains pending. The literal generated-file contract and destination checks are available; Podman supports them when given a host allocator, staging individual read-only binds until all container users are removed; bwrap still refuses them. The host-only allocator now retains aggregate byte/entry charges through cleanup failures and refuses stale roots; runtime-owner integration remains pending. Live generated-file acceptance, hosted resolver integration, pinned CLI acceptance, hung-control recovery, supervisor behavior, and aggregate transport bounds remain pending. Shared session supervisor and revocable inference, one guest authority domain, retained runtime across turns, bounded host transports, and small Claude/Codex/OpenCode adapters; refines the existing sandbox envelope. |
