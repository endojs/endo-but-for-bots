...
accept a sturdy reference as a value in a tool call, without assigning it a
pet name. The value is the first-class `'sturdyref'` pass-style value defined
by the sturdyref work. It is inert data, not a remotable, and it is enlivened
only by a closely held capability of the relevant CapTP network.

This revision deliberately removes the proposed `SturdyRefToken` remotable.
There is one reference representation at this boundary: `SturdyRef`. The
daemon holds the capability that associates a sturdyref with its locator and
can obtain a sturdyref for a locator. Confined code receives neither that
capability nor a locator. It can pass a sturdyref back to a daemon method that
...
The pass-style implementation defines how a sturdyref is recognised and the
CapTP-network layer constructs it. The latter keeps the locator association.
The interface available to a confined worker has this shape conceptually:
...
argument, but no worker receives either of the first object's operations.
...
not a serialisation, not an authority-bearing string, and not a second kind of
reference.
...
- A worker may not obtain a locator, formula identifier, swiss number, or a
...
boundary: the sturdyref passable value is the intended designation.
...
pass-style recogniser and the facet resolves the value through the closely held
capability.

The final method list must be derived from authority, not from input shape:
...
| `list` when its existing path argument identifies the directory to list | To confirm | The implementation must verify that the result does not reveal locator authority. |
...
An explicit user-authorised namespace write remains a separate operation.
...
sturdyref must keep its referenced formula alive. There are two distinct cases:
...
- Every admitted daemon method has an authority review proving that it does not
  disclose a locator or stable naming information.
...
2. Audit daemon methods and add only the admitted `SturdyRef` inputs, with
   confinement tests for each method.
...
|---|---|
| SturdyRefs on demand design (PR #539) | Defines the sturdyref pass style and closely held enlivenment capability this surface consumes. Its guest-token conclusion must be revised to match this document. |
| PR #521 | Implements the first-class `'sturdyref'` pass-style work that this design assumes. |
| PR #541 | Provides daemon-side sturdyref resolution at the facet boundary. |
| [daemon-retention-paths](daemon-retention-paths.md) | Candidate basis for showing the user the workers that retain a formula. |
...
- Does holding a sturdyref across a worker turn require a formula-graph
  retention edge, and, if so, what is its explicit lifecycle?
- Which existing or new UI exposes worker-specific retention and performs the
  user-authorised revocation?
