# Snapshot schema requirements from the surgery perspective
...
The recipes live in [upgrades.rs](../rust/endo/ironhorse-store-sqlite/examples/vat_surgery/upgrades.rs).
...
For the complementary collector perspective, see
[snapshot schema requirements for efficient GC](ironhorse-snapshot-schema-gc.md).
For snapshot inspection and debugger interoperability, see
...
> explore a wider range of upgrades, in increasing complexity. identify whats easy and whats hard
> with the current schema, and what would address them. continue the review loop
>
> a key one to test is replacing a function implementation. should test all of anonymous,
> name bound, or on a prototype.
>
> im researching snapshot schema design from different perspectives and this is one of them.
> i want to be sure to clearly capture whats desired from the schema design for the sake of
> snapshot surgery. where is it documented?
