...
OpenClaw grants agents ambient authority — any tool the agent calls operates
with the user's full permissions.
Endo's object-capability model means agents hold only the specific `Dir`,
`Shell`, `Git`, and other capabilities explicitly granted to them. This is
Endo's primary differentiator for security.
...
- The Chat UI runs inside the Familiar (Electron) or can be served by a
  self-hosted daemon ([daemon-docker-selfhost](daemon-docker-selfhost.md)).
- External platform bridges could be built as guest plugins but are not on the
  current roadmap.
...
- A future design could add a Telegram or Signal bridge as a guest plugin.
...
| Llama.cpp (Meta)         | Via Ollama or OpenAI-compatible API | **Available**          |
...
- so any OpenAI-compatible endpoint (LM Studio, vLLM, etc.) works by providing
  its URL.
...
- Endo's [daemon-agent-tools](daemon-agent-tools.md) design provides
  ...
  allowlists.
- Unlike OpenClaw's ambient `system.run`, Endo's shell capability validates
  commands against an allowlist and passes arguments as arrays (no shell
  injection).
- The [daemon-capability-filesystem](daemon-capability-filesystem.md) design
  provides structural confinement — the agent cannot access paths outside its
  granted `Dir` root.
...
- Location, camera, and screen capture are device-specific features that would
  live in the Familiar or mobile companion apps rather than the daemon.
...
- A future design could provide a `Browser` capability backed by Puppeteer or
  Playwright, with structural confinement: the agent receives a browser context
  scoped to specific allowed domains, with the host controlling which sites are
  accessible.
...
- For example, a Gmail plugin would receive an `OAuth` capability that provides
  authenticated access to the Gmail API without exposing the raw OAuth token to
  the agent.
...
- These could be community-contributed once the capability filesystem and agent
  tools are in place.
...
- Each agent is a guest with its own formula identity, pet-name directory,
  inbox, and capability set.
- Agents can message each other via `send()`.
- The [lal-fae-form-provisioning](lal-fae-form-provisioning.md) design replaces
  OpenClaw's `AGENTS.md` with form-based provisioning: the root user fills a
...
- A `Timer` capability is listed in the
  ...
  designed.
- Webhooks could be a gateway feature.
...
- The Chat UI could potentially integrate Web Speech API for browser-based
  voice input, but this is speculative.
...
- Endo's object-capability model makes these attacks structurally impossible:
...
- The Chat UI served by a self-hosted daemon
  ...
  via browser once [gateway-bearer-token-auth](gateway-bearer-token-auth.md) is
  implemented.
...
- A skills directory could index available plugins.
