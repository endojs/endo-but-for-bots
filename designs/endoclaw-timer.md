...
- **Power downstream scheduling:** Higher-level scheduling policies
  ...
  _by the agent_ on top of its heartbeat — the heartbeat itself carries
...
- the next tick fires at a fixed offset from when _this_ tick was scheduled to
...
is scheduled relative to the previous tick's _scheduled_ time, not its
...
2. **Start-to-start timing, not end-to-start.** Each tick is scheduled
   at a fixed offset from the previous tick's _scheduled_ time. This
