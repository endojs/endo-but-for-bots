...
oracle. Read as a *parity* assertion ("IH must match XS here") it deserved
demolition. But read mechanically it also said "this load costs a specific amount",
and when it is relaxed to advisory drift, nothing catches that same load silently
...
debug/release by Linux/macOS lanes `golden_computrons.rs` already runs on, where the
...
`ironhorse-262/baseline/` provenance discipline.

**The record is a single artifact: a new `rust/engine/benches/computron-baseline.json`,
and only that.** An earlier draft floated *also* extending the `computrons.tsv`
corpus that `golden_computrons.rs` consumes, so that the exact per-size pins would
live in the TSV. This design rejects that split. The deterministic exact pins and
the (host-tied) confirming wall-clock medians have different volatility and
different readers, but keeping them in one JSON keyed by load id is simpler than a
schema change to the consumed TSV format, and gate 1 reads the pins directly from
the JSON. `golden_computrons.rs`'s single-size TSV corpus is left untouched; gate 1
is a *new* input-parameterized harness that reads `computron-baseline.json`, not an
edit to the TSV schema (§ Relationship to existing infrastructure restates this).
...
1. **Exact pins (PR lane, deterministic, cheap).** At each committed ladder size,
   `C(n)` must equal the recorded value. This is a new input-parameterized harness
   in the spirit of `golden_computrons.rs`, reading the per-size pins from
   `computron-baseline.json` for the loads that lost their constraint (it does not
   modify the 52 single-size families in `computrons.tsv`). Catches *any* change to
...
2. **Growth-envelope gate (PR lane, deterministic, cheap).** Run each parameterized
   ...
   the load's declared growth class (see the class-band table below); (b) `C(n)` for
   an **off-ladder** size lies within `C_model(n) +/- epsilon`, catching a
   regression that only manifests between or beyond the pinned points. Because it
...
Gate 2's per-doubling computron ratio bands, one per member of the closed growth-basis set:

| growth class `f(n)` | per-doubling computron ratio band (gate 2, two-sided) |
|---|---|
| `1` (constant) | `[1.00, 1.15]` |
| `log n` | `[1.05, 1.35]` (ratio approaches `1 + 1/log2(n)`; widen at small `n`) |
| `n` (linear) | `[1.80, 2.20]` |
| `n*log n` | `[2.00, 2.60]` (linear ratio scaled by `1 + 1/log2(n)`) |
| `n^2` (quadratic) | `[3.60, 4.40]` |

Every member of the declared closed set has a worked band here, including `log n`
and `n*log n` (the log-class bands are wider and size-dependent because a
per-doubling ratio for a logarithmic term is itself a slowly-moving function of `n`;
the builder confirms the exact widths against measured ladders and records the
chosen widths in the baseline record's tolerance fields). Gate 3's single-sided
time ceilings are derived per class from the same table (upper edge, loosened for
timing noise per the section above).
...
- When the underlying meter defect (F4) is fixed, the load's computrons stop being
  ...
  recalibration), clears `known_divergent`, and re-enables gate 3's class-match. The
  seeding step (§ Phased execution step 5) marks the F4 trio `known_divergent` from
  the start rather than seeding them into an immediately-red gate 3.
...
- `golden_computrons.rs` + `computrons.tsv` (52 exact single-size families): gate 1
  ...
  pins from the new `computron-baseline.json` and leaves the 52-family TSV corpus
...
2. **Define the baseline record format**: the single
   `rust/engine/benches/computron-baseline.json` schema (load id, `f(n)` basis,
   fitted coefficient/intercept, per-size exact `computrons`/`meter_raw`, confirming
   wall-clock medians, tolerance bands, `known_divergent` flag, `COST_TABLE_VERSION`
   plus provenance) and the growth-class band table (§ class-band table).
3. **Build the harness**: a `computron_baseline` test crate/module that (gate 1)
   asserts exact pins at committed ladder sizes reading `computron-baseline.json`,
   (gate 2) asserts per-doubling class bands and off-ladder `C_model(n) +/- epsilon`,
   both deterministic and PR-runnable, sharing the one failure-message helper; and a
...
5. **Seed the roster** (§ polynomial built-ins) plus every load from step 1's audit;
   ...
   `known_divergent` per § F4 exception; record all baselines with `--write-baseline`
   on a controlled host and commit.
...
6. **One committed artifact.** The baseline is a single `computron-baseline.json`,
