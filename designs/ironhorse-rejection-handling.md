...
This is a rationale/analysis document, not an implementation spec. It argues why
an Ironhorse option to **panic on a reference error** (freeze the world at the
fault site so a heap snapshot can be taken with the program counter still there)
is worth building, and it situates that option against the other tools that
claim to catch *silently dropped* failures — chiefly Node.js's
escalate-on-unhandled-rejection behavior. It ends in a concrete recommendation
for a future design or build job to act on.
...
Dependencies/related-designs table (see § Dependencies and § Open Questions).

## 1. Why panic-on-reference-error matters

A reference error — reading an undefined variable, or touching a
still-uninitialized (temporal-dead-zone) binding — is a **bug in the program**,
not an expected control-flow event. In Ironhorse these are engine-raised errors:
`ironhorse-vm`'s `XS_CODE_GET_VARIABLE` ("undefined variable") and
`XS_CODE_GET_LOCAL` ("get: not initialized yet") build a `Halt::Throw(...)`
today (see [ironhorse-debugger-recovery-and-uncaught](ironhorse-debugger-recovery-and-uncaught.md)
§ Prerequisite). Because they are ordinary catchable throws, the surrounding
program — deliberately, or far more often by accident — can swallow them:
...
into a retry loop. By the time anyone notices — minutes, or many messages,
later — the interpreter has unwound through the `catch`, the activation that
raised the error is gone, and the heap has moved on. **Unwinding through a
`catch` (or a promise rejection handler) before anyone inspects the failure
destroys the single most useful piece of evidence: the exact program counter and
heap state at the moment of the fault.** A post-hoc log line or a re-thrown
wrapper is a photograph of the scene after the body has been moved.
...
catchable `RangeError` — a deliberate, consensus-relevant choice in the xsnap
lineage (`interp.rs`, the `Halt::StackOverflow` doc comment: *"an abort to the
host, not a catchable `RangeError`"*; and see
...
across the CapTP boundary. It has no way to distinguish:

- a promise **legitimately handed off** to a peer that owns its resolution, from
- a promise **genuinely dropped** on the floor locally.
...
"nobody subscribed yet ⇒ abandoned" timing guess, wired to process death. It is
...
**Endo/Ironhorse should not adopt Node's escalate-on-unobserved-rejection
...
3. **For genuinely-abandoned rejections, prefer *visibility* over *escalation*.**
   ...
   handoff — that is the closest an engine can honestly get to "abandoned." The
   right response is to **report** it (a diagnostic, a debugger panel — see §
   Debugger note), the way XS accumulates unhandled rejections in a weak list and
   reports them at drain/exit (`fxAddUnhandledRejection` /
   `fxCheckUnhandledRejections`; see
   [ironhorse-debugger-recovery-and-uncaught](ironhorse-debugger-recovery-and-uncaught.md)
...
## Debugger note — reactive visualization panels
...
- **As-yet-unhandled ("unwatched") rejections**, shown **live, until each is
  handled**. Critically, this panel is *reactive*: a rejection that later gets a
  handler attached must **visibly leave the panel** rather than stay flagged.
  That behavior is exactly the "can still be handled later" property § 2 argues
  Node's heuristic ignores — encoded as UI. The panel shows current unwatched
  status as a fact, and retracts it the instant the fact changes, instead of
  freezing a timing guess into a persistent accusation. Over CapTP, a rejection
  whose obligation was handed off should likewise be distinguishable from one
  that is genuinely local-and-unobserved, so the panel does not cry wolf over the
  normal distributed shape (see § Open Questions).
...
1. **A rejection tracker for Ironhorse — what is its terminal boundary and its
   ...
   shape. Is the terminal boundary end-of-crank, drain, or worker exit?
...
2. **Data plumbing for the two debugger panels.** The pending-promises and
   ...
   (to be filed against the recovered debugger row) that extends the `DebugHook` /
   `DebugCtx` seam from
   [ironhorse-debugger-recovery-and-uncaught](ironhorse-debugger-recovery-and-uncaught.md)
