...
the method _names and signatures_ across the three live tree surfaces) and on
...
- **C3 (done) / C4 (done for the daemon blob).** `EndoReadableTree` is now
  ...
  `EndoBlob` is aligned _up_ to the richer `BlobRef` range-I/O shape: it carries
...
- **A "tree of named things"** is declared three times in live code: the
  ...
  `makeDirectory` / `remove` by hand. The daemon already _imports_
...
F7/D3, is a _symptom_ of two shapes for one concept), and it multiplies the
surface a viewer must learn to dispatch over.

## The load-bearing constraint: signatures diverge, names do not

The reconciliation aligned method _names_. It did **not** make the argument
_shapes_ identical, because the live surfaces accept genuinely different
...
therefore requires a maintainer decision on the _canonical_ shape per method,
...
the daemon's `Mount` _shape_ — `help`, `has`, `list`, `lookup`, `maybeLookup`,
...
- `assertIsMountCap` is a _shape_ gate checking for the presence of
  `['readText','writeText','makeDirectory','has','list']` — all still present.
- `provideHostPath` is an _identity_ gate keyed on a `WeakSet`/`WeakMap` of
...
`NameOrPathShape` and have the mount accept entries through a _separate_
...
_Blast radius:_ `daemon/src/interfaces.js`, `mount.js`, and the consumers that
...
_shared-shape_ methods into exported **method-guard records** (not just
...
_Blast radius:_ `platform/src/fs/interfaces.js` (export records),
...
_Blast radius:_ `platform/src/fs/interfaces.js`, `daemon/src/interfaces.js`, and
...
**Direction resolved (maintainer decision):** align _up_ to the **richer
...
layered _on top of_ this; the unified rich blob keeps them. The earlier "retire
...
1. _Powers / content store_ (`daemon-persistence-powers.js`): the store's
...
2. _Hash format reconciliation:_ `BlobRef.getInfo().hash` is **base64**; the
   ...
   recommended _hex_ as the canonical encoding to match the store filenames;
   that was reversed — base64 is the portable on-the-wire spelling and the store
   key is an internal detail.)
3. _Blob exos:_ `getInfo` + `fetch` were added to every content-addressed blob
...
4. _Interface:_ the range-I/O surface is the shared `rangeReadMethodGuards`
...
_Blast radius:_ the widest — content-store powers, the CAS cache, every blob
...
Once C2 makes the lite `Directory` / `File` the _source_ of the shared method
...
C1–C5 aligned method _names and argument shapes_. A separate audit found a
...
- **Immutable bytes:** one rich range-I/O shape (C4). The convergence went the
  other way from the early "retire `BlobRef`" framing: `BlobRef` is the _richest_
...
- **Records, not inheritance.** `M.interface` has no native `extends`; the
  mechanism throughout is _exported method-guard records spread into multiple
  `M.interface()` calls_, with per-surface overrides where the divergence table
...
| [namehub-interface-unification.md](namehub-interface-unification.md) | C1 _is_ this design's accepted plan; folded in and unblocked here. |
...
- [x] C4: align the daemon `EndoBlob` _up_ to the richer `BlobRef` range-I/O
...
- [x] C4: extend the daemon mount-file `readOnly()` view — `FilePowers` gains
      ...
      `getInfo`/`fetch` over the _live_ file (a write-disabled face, not a
