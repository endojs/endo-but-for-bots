...
aborted in-flight model and CLI work. That contradicts the product model:
**sessions run on the daemon; the UI is only a view.**

## Principle

The UI **displays state** that authority elsewhere already owns (daemon,
session guest, persistence). It **sends actions** (start turn, cancel turn,
navigate) as explicit commands.
...
**Losing observation must not change intent.** Only an action (or session or
factory teardown) may cancel work.
...
state and sends actions. Apply to Floot session lifecycle vs browser CapTP.
