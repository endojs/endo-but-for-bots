...
six steps into one hard-coded implementation order (rate `take()` →
method/header normalization → origin check → `fetch` → redirect →
byte-cap).
...
- The reconciliation that makes **metering and the Phase 1 byte-cap one
  ...
  (reserve worst-case, refuse-before-read, bill actual).
...
| `ctx` mutable context | An **immutable `RequestContext` record** threaded by value plus far-refs for effectful shared state (meter, breaker). No stage mutates another's fields. |
...
// Immutable, passed by value down the chain; far-refs inside it
// carry effectful shared state a stage may consult.
const RequestContextShape = M.splitRecord(
  { origin: M.string(), attempt: M.number() },
  {
    deadline: M.number(),         // absolute ms; the wall-clock budget
    reservation: M.remotable('MeterReservation'), // set by the meter stage
  },
);
...
export const HttpStageInterface = M.interface('EndoHttpStage', {
  ...
  // substitutable for the whole client to the stage above it.
  request: M.call(RequestShape, M.promise())
    .optional(RequestContextShape)
    .returns(M.promise()),
...
request: (req, cancellation, ctx) => {
  // ...before: consult/mutate this stage's own scoped state...
...
chain inner-to-outer (transport first, then meter, rate, retry, breaker
...
- Stages are **never smuggled in by the client**. The `RequestContext`
  the client may pass carries only a caller-supplied `deadline` proposal
  (clamped down, never up, by the meter/timeout stages); it cannot carry
  a stage or a purse.
...
**Onion (effectful, outer → inner):**

| # | Stage | Why here |
|---|---|---|
| 1 | **Circuit breaker** | Outermost gate *and* outermost observer. An `open` breaker fast-rejects before any resource is spent; on the way out it records each attempt's outcome. It wraps retry so it sees every attempt and can trip mid-loop. |
| 2 | **Retry** | Re-invokes the inner chain up to N times for idempotent methods. Each attempt is a real request, so it must sit *above* rate + meter (every attempt is separately rate-limited and billed) and *below* the breaker (a tripped origin aborts the loop). |
| 3 | **Rate limiter** | Spend a window token per attempt. Above the meter so a rate refusal costs no fund reservation; below retry so each retried attempt is independently throttled. |
| 4 | **Meter (reserve → settle)** | Reserve the worst-case cost up front and **refuse before reading any bytes** if the purse cannot cover it; after the read, settle to actual usage and refund the remainder. |
...
This is a deliberate generalization of http-confine's fixed order. The
one substantive reordering: http-confine spends the rate token (step 1)
*before* the origin check (step 3); this pipeline moves origin/method/
header validation into the pure **pre-flight ahead of everything**, so a
structurally invalid request never consumes a rate token or a fund
reservation. Redirect resolution and byte-cap stay inside the transport
stage exactly as http-confine defines them. Where a deployment omits a
concern (no fees, no breaker), that stage is simply absent from the
composed chain and the onion collapses toward the Phase 1 behavior.
...
1. **Reserve.** Compute `cost_max`; call `reserve(operationId, cost_max)`
   ...
   `available → reserved` and returns a `MeterReservation` capability, or
   — if the balance cannot cover the pessimal case — **rejects now** with
...
export const MeterReservationInterface = M.interface('MeterReservation', {
  ...
  release: M.call().returns(M.promise()),            // abort: refund the whole hold
});
...
export const ChargeAccountInterface = M.interface('EndoChargeAccount', {
  ...
  // exceed `limit`.
  reserve: M.call(M.string(), M.number()).returns(M.promise()), // Promise<MeterReservation>
...
- **Retryable classes.** Retry fires on the same error classes the
  circuit breaker counts: connection errors, timeouts, and 5xx / 429
  responses. It does **not** retry 4xx (client error, deterministic) or
  an origin/method/header rejection (never admissible) or an
  `InsufficientFunds` (funds do not improve by retrying). A `429` with a
  `Retry-After` honors that hint as the backoff floor.
...
- **Interaction with cancellation/timeout.** There is **one**
  ...
  attempt. Each attempt increments `ctx.attempt` (visible to the meter,
  which bills every attempt, and the breaker, which records every
  attempt).
...
The breaker is the outermost stage: a per-origin state machine that trips
on error classes to stop hammering a failing upstream.

- **States.** `closed` (pass through, count failures) → `open` (fast-
  reject every request with `CircuitOpenError` for a cooldown window) →
  `half-open` (admit a single probe; success → `closed`, failure →
  `open`). Standard three-state breaker.
- **Keyed on error classes, not all failures.** The breaker counts
  **server-side and transport failures** — 5xx, 429, connection refused,
  DNS failure, TLS failure, timeout — as evidence the *origin* is
  unhealthy. It does **not** count 4xx (the request was wrong, the origin
  is fine), origin/method/header rejections, `InsufficientFunds`, or a
  caller-side cancellation. Counting client-fault errors would let a guest
  trip the breaker for every other guest by sending garbage.
...
- **Position and interaction with retry.** The breaker wraps retry, so it
  observes **every attempt** (each retried failure is fresh evidence) and
  can trip **mid-retry-loop**: once the breaker opens, the retry stage's
  next `E(next).request` fast-rejects with `CircuitOpenError`, which retry
  treats as terminal (an open circuit is not itself retryable), ending the
  loop immediately. This is the desired coupling — a retry storm against a
  dying origin trips the breaker, which then halts the storm.
...
- **Stage composition:** a controller composing `[transport]`,
  `[rate, transport]`, and the full five-stage chain each yields a client
...
- **Retries:** GET retries on 5xx/timeout/connection error up to
  ...
  `InsufficientFunds` are not retried.
- **Circuit breaker:** N failures within the window trip `open`; an open
  ...
  the loop; 4xx does not count toward the threshold.
...
- **Metering unit.** Is the billed unit an ERTP token (mintable, per
  ...
  currency is attractive (an agent's total resource spend — compute +
...
- **Deadline as controller policy vs caller proposal.** The caller may
  propose a `deadline` in `RequestContext`; the meter/timeout stages clamp
  it **down** to the controller's `timeoutMs` (never up). Should a caller
  be allowed to request a *shorter* deadline to lower its own worst-case
  cost? Proposed: yes — a shorter caller deadline lowers `cost_max`, which
  is incentive-correct and cannot widen authority.
