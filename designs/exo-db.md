...
implemented by DynamoDB. This is API portability, not file-format portability:
moving a deployment between providers requires a row-copy migration, but does
not require application code or schemas to change.
...
- the three-export pattern in [Platform Filesystem
  Capabilities](platform-fs.md): a condition-selected module, a portable
  `lite` contract, and explicit platform modules;
- the Node/Endor SQLite parity surface in [SQLite Host Functions for the Endor
  Daemon](daemon-endo-rust-sqlite.md) and [Streaming SQLite Rows for
...
```js
export const DatabaseReadI = M.interface('DatabaseRead', {
...
shape before touching a backend. The static interfaces show the capability
boundary; `makeTableGuardKit(schema)` supplies the narrower per-instance
guards.
...
- `replace(row)` atomically fails if it does not exist;
- `put(row)` atomically inserts or replaces;
- `delete(key)` returns whether a row existed; and
- an acknowledged mutation is durable, while two concurrent writes to one key
  take some serial order.
...
|---|---|---|---|---|
| Narrow | `boolean`, signed 64-bit `bigint`, finite binary64 `number`, well-formed `string`, passable bytes, and explicit `null` | `INTEGER`, `REAL`, `TEXT`, `BLOB`, with `STRICT` tables and checks | `BOOL`, `N`, `S`, `B`, `NULL` | Exact validation, keys, comparisons, projection |
...
| Passable | Every durably passable value, including copy data, errors, promises, remotables, and references nested in copy data | Smallcaps body plus formula-id slots | Opaque body plus formula-id slot list | Whole-cell read/write only; a `Key` value may additionally serve as an encoded key |
...
identifier by the supplied `ReferenceCodec`; an ephemeral reference that the
host cannot formulate causes the write to reject before mutation. The body is
opaque to the database. This is how the broad band covers all durably passable
values without pretending that a database can natively index capability
graphs.
...
`ReferenceCodec` can assign stable identities.
...
type Query = {
  ...
  start?: Bound;                  // bounds apply to the sort key
  end?: Bound;
  reverse?: boolean;
  where?: Predicate;
  project?: Readonly<Record<string, Selector>>;
...
selected whole. Predicate paths must select narrow cells or JSON scalars.
Adapters compile the AST to parameterized native expressions and reject a
query they cannot compile; they do not fetch opaque rows and silently emulate
...
an unbounded result array. Early return closes the local SQLite statement or
abandons the DynamoDB continuation through the established exo-stream reader
protocol.

## Compact ordered key encoding

`@endo/exo-db` exports a versioned `makeOrderedKeyCodec(referenceCodec)`. It
...
`makeEncodePassable` preserves the repository's JavaScript rank order,
including UTF-16 code-unit string order. Durable identifiers provide the
otherwise-missing order between distinct remotables. SQLite `BINARY` text
collation and DynamoDB string keys compare their stored encodings, so storing a
binary key avoids a disagreement for astral versus high-BMP strings. Bytewise
comparison of the versioned UTF-16BE result is the same as JavaScript code-unit
...
Both modules provide exactly this synchronous low-level power:
...
The Node provision wraps the existing `better-sqlite3` dependency. The Endor
provision moves the existing `makeXsSqlitePowers` adapter over the Rust
...
part of the platform contract. Transactions use `exec('BEGIN IMMEDIATE')`,
`COMMIT`, and `ROLLBACK`; no provider object escapes the adapter.
...
- `p BLOB NOT NULL` and optional `s BLOB NOT NULL` ordered primary-key bytes;
...
- materialized binary partition/sort columns for every secondary index.

The primary key is `(p, s)` or `(p)`. Every secondary index begins with its
encoded partition and sort columns. JSON index sources are extracted and type
checked during the write, then encoded into those materialized columns; query
planning never depends on SQLite and DynamoDB having identical JSON collation.
Prepared statements bind every value. Schema ordinals are the only source of
SQL identifiers.

`query` repeatedly executes a bounded `LIMIT` statement using the last encoded
key as continuation, applies the compiled JSON/narrow predicate in SQLite, and
yields through `PassableReader`. This avoids requiring an Endor cursor handle
for the first implementation; `StatementSync.iterate` can replace the bounded
page loop after the streaming-SQLite design lands without changing exo-db.
...
the daemon's configured `DatabaseStorageProvider.open(formulaId)` reconstructs
the admin/read/write exo kit. A new `makeDatabase(petName)` host/guest method
first assigns the formula identifier, creates the provider storage under that
identifier, writes the formula, binds it in the caller's pet store, and returns
the admin facet. The ordinary formula and pet-name graph therefore controls
who can rediscover the database capability.
...
daemon supplies a `ReferenceCodec` with four responsibilities: map a reference
to a formula identifier, revive an identifier, retain an identifier on behalf
of the database formula, and release that retention. The SQLite sidecar and the
daemon's main `endo.sqlite` formula graph are separate files, so updates use a
crash-safe asymmetric protocol:

1. marshal the new row and compute old/new slot-count deltas;
2. add all new formula retentions in the main daemon database;
3. commit the row and its per-database reference-count ledger in the sidecar;
4. release removed retentions from the main daemon database; and
5. on startup, reconcile the main retention counts to the sidecar ledger before
   exposing the database exo.

A crash can therefore leave an extra retention, never a committed row with a
dangling reference. Reconciliation removes over-retention. Deleting the
database formula first stops new calls, then closes and removes provider
storage, then releases all ledgered references. All five steps run under the
formula-graph mutation queue so two row mutations cannot race retention
accounting.
...
- `ConflictError` for failed `insert`/`replace` existence conditions;
...
- `LimitExceededError` for a row, key, schema, or index over a versioned limit;
  and
- `DatabaseUnavailableError` for storage or throttling failures.
...
2. Add schema types, schema-to-pattern guards, exo interface kits, the backend
   ...
   suite.
3. Move the common SQLite types and Endor adapter into
   `@endo/platform/sqlite`; add the Node provision and make the daemon consume
   the condition-selected surface.
...
- Property-test `keyEQ`/encoded equality, `compareRank`/byte ordering, codec
  round trips, every PassStyle allowed by `Key`, Unicode boundary pairs, and
...
8. **Retain before row commit; release after.** Separate persistence domains may
   leak retention across a crash, but must never create dangling authority.
