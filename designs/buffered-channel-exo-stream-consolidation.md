...
**Coordination with PR #486:** `packages/claude-sandbox` exists only on
#486's head branch (`claude/claude-sandbox`). If #486 merges first, the build
PR edits both packages on `llm` directly (preferred). If the build lands
first, it covers exo-stream + floot, and #486 rebases to delete its copy and
import from `@endo/exo-stream` before un-drafting. Either order keeps the
invariant: no release of `llm` carries two copies.
