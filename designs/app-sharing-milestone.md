...
It is a _cut_, not a new bucket of unrelated work: it pulls forward and
...
installers and `electron-updater` auto-update are MVR _followups_ in that plan.
_Exit (this milestone's slice):_ a non-developer downloads, installs, and
...
_Exit:_ clicking an `endo://invite/…` link in another app opens Familiar, shows
...
weblet substrate. _Exit:_ a user runs an endo-fs/`make-from-tree` app, opens
...
phases 2–3) — no per-blob hashing; integrity is the transport's job. _Exit:_ a
...
require all of M1/M2/M3 to complete — only the slices listed under _Constituent
Work_.
...
> are 1) to be able to distribute the endo daemon chat app as a distributeable. 2) connect to other peers via a deep linking url format (needs a confirmation
