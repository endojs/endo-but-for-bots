...
>
> - Names the tree-backed methods by _source shape_ rather than
...
- [x] **Phase 4 — Worker (Rust / XS)**: the XS worker bootstrap
  ...
  _Open optimisation:_ the worker currently streams the archive
...
- [ ] **Phase 6 — `@node` special name, XS never runs
      `makeUnconfined`.** We explicitly decide _not_ to implement
...
source-only ZIPs, the last gap in the _confined_ surface is running
...
distinguishing axis is the _source shape_, so the name names the
...
_tree_ (CAS snapshot or mount) today have to stage it to disk
...
1. **Archive (ZIP)** or **readable tree** loaded in _any_ worker
...
1. **Phase 1 (additive)** — add `make-archive` alongside `make-bundle`:
   ...
   one passing end-to-end archive test. _Done._
2. **Phase 2 (migration)** — convert every existing `makeBundle` test
   to `makeArchive`. Confirm the full suite passes. _Done._
3. **Phase 3 (removal)** — delete `makeBundle`, `MakeBundleFormula`,
   ...
   external consumers exist. _Done._
...
5. **Phase 5 (Node-side wiring closure)** — Node worker `makeArchive`
   passes same env/context contract as `makeBundle` did. _Done._
...
7. **`makeFromTree` unifies the archive and tree paths.** Once
   ...
   tree-walk overhead. Naming is by _source shape_
...
- [ ] Whether `makeArchive` should accept a CAS root-hash reference
      _directly_ (skipping the readable-blob wrapper) for Rust workers,
