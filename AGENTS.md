...
- Linting: `eslint` with project-specific rules; run `pnpm lint` per-package
...
- Runtime tests: `pnpm test` (uses `ava`)
- Type tests: `pnpm lint:types` (uses `tsd` — test files are `test/types.test-d.ts`)
- Lint: `pnpm lint` (runs both `lint:types` and `lint:eslint`)

Always run `pnpm lint` in each package you've modified before committing.
...
```sh
pnpm build:types        # one-shot build
pnpm build:types:watch  # incremental watch (cold start: ~10-30s)
```

The config files are generated — do not edit them by hand:

```sh
pnpm build:types:gen    # regenerate tsconfig.composite.json files
pnpm build:types:check  # verify generated files are up to date (used in CI)
```

**When to regenerate:** run `pnpm build:types:gen` after adding, removing, or
changing the runtime `dependencies`/`peerDependencies`/`optionalDependencies`
of any workspace.

The generator (`scripts/generate-composite-tsconfigs.mjs`) reads the pnpm
recursive workspace listing and each package's `package.json`.
