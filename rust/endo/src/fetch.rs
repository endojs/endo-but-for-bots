...
impl std::fmt::Display for FetchError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            FetchError::Offline { url } => {
                write!(f, "offline: network access to {url} refused (run without --offline to populate the cache)")
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn ureq_client_with_config_carries_credentials() {
        // The request-building path must consult the config; the
        // actual header application is exercised against the live
        // registry only, so this pins the token lookup wiring.
        let mut config = crate::npmrc::NpmConfig::new();
        config.apply_npmrc("//registry.npmjs.org/:_authToken=tok\n");
        let client = UreqClient::with_config(config);
        assert_eq!(
            client
                .config
                .as_ref()
                .unwrap()
                .token_for("https://registry.npmjs.org/x"),
            Some("tok")
        );
        let anonymous = UreqClient::new();
        assert!(anonymous.config.is_none());
    }
