...
//!    into the CAS via [`NpmResolver`] (Go-like MVS, registry-table
//!    fast path, offline replay),
...
//!
//! The compartment map is the artifact the XS-hosted compartment
//! mapper consumes to execute the program (the remaining half of
//! Phase 4, gated on the XS boot bundles): one compartment per
//! resolved `(name, version)`, each `location` a `cas:sha256:<tree>`
//! URI, each dependency edge bound to the compartment that MVS
//! selected for it. Everything here is deterministic: assembling the
//! same package twice yields byte-identical maps and hence the same
//! CAS hash.
...
use crate::resolver::{NpmResolver, Resolution, ResolveError, ResolvedPackage};
...
/// The outcome of [`assemble_entry`]: everything the XS execution
/// half of Phase 4 needs, already in the CAS.
#[derive(Debug)]
...
/// dependencies); a malformed document or non-string range is an
/// error rather than a silently empty graph.
fn parse_manifest(text: &str) -> Result<(String, BTreeMap<String, String>), AssembleError> {
    ...
    if let Some(deps) = doc.get("dependencies") {
        let obj = deps.as_object().ok_or_else(|| {
            AssembleError::BadPackageJson("\"dependencies\" is not an object".to_string())
        })?;
        for (dep, range) in obj {
            let range = range.as_str().ok_or_else(|| {
                AssembleError::BadPackageJson(format!("dependency {dep} has a non-string range"))
            })?;
...
fn compartment_key(package: &ResolvedPackage) -> String {
...
/// the mapper's business — it reads the target's `package.json` from
/// the CAS).
...
/// Offline mode is the HTTP client's property, as in
/// [`NpmResolver::with_config`]: pass
/// [`crate::fetch::OfflineClient`] and only cached packages resolve.
pub fn assemble_entry<H: HttpClient>(
    ...
    let resolver = NpmResolver::with_config(http, cas, registry_table, config);
    let resolution = resolver.resolve_transitive(&root_dependencies)?;
...
mod tests {
    use super::*;
    use crate::fetch::{FetchError, OfflineClient};
    use std::cell::RefCell;
    use std::collections::HashMap;
    use std::io::Write;

    // Minimal mock registry, mirroring the fixtures in
    // `resolver::tests`. Duplicated rather than shared: the fixtures
    // live inside that module's `#[cfg(test)]` block on an open PR
    // (#772), and reorganising it from this stacked branch would
    // manufacture rebase conflicts. A follow-up can hoist a common
    // test-support module once the stack lands.
    struct MockHttp {
    ...
    fn assembly_is_deterministic_across_runs() {
        ...
        let first = assemble_entry(&graph_http(), &cas, &registry, NpmConfig::new(), &entry)
            .unwrap();
        let second = assemble_entry(&graph_http(), &cas, &registry, NpmConfig::new(), &entry)
            .unwrap();
    ...
    #[test]
    fn offline_replay_reassembles_identically_with_zero_network() {
        let (_cas_tmp, cas) = fresh_cas();
        let registry = RegistryTable::open_in_memory().unwrap();
        let app = tempfile::tempdir().unwrap();
        entry_package(app.path());
        ...
        let online = assemble_entry(&graph_http(), &cas, &registry, NpmConfig::new(), &entry)
            .unwrap();
        // Replay against the populated CAS + registry table with a
        // client that hard-fails every request: the registry table
        // is the lock file.
        let offline = assemble_entry(&OfflineClient, &cas, &registry, NpmConfig::new(), &entry)
            .unwrap();
        assert_eq!(online.compartment_map_hash, offline.compartment_map_hash);
    }
    ...
    fn live_registry_entry_assembly() {
        ...
        // Offline replay from the same CAS + table must reproduce
        ...
        eprintln!("offline replay reproduced the map with zero HTTP");
    }
}
