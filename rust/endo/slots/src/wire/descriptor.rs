...
impl Descriptor {
    pub fn new(direction: Direction, kind: Kind, position: u64) -> Self {
        Descriptor { direction, kind, position }
    }
    ...
    pub fn from_kind_byte(b: u8, position: u64) -> Result<Self> {
        ...
        let direction = if b & 0b1 == 0 { Direction::Local } else { Direction::Remote };
        ...
        Ok(Descriptor { direction, kind, position })
    }

    pub fn to_vref(self) -> Vref {
        Vref {
            ty: self.kind.to_vref_type(),
            direction: self.direction.to_vref_dir(),
    ...
    pub fn from_vref(v: &Vref) -> Self {
        Descriptor {
            direction: Direction::from_vref_dir(v.direction),
...
mod tests {
    ...
    fn kind_byte_roundtrip() {
        for direction in [Direction::Local, Direction::Remote] {
            for kind in [Kind::Object, Kind::Promise, Kind::Answer, Kind::Device] {
                let d = Descriptor::new(direction, kind, 42);
    ...
    fn vref_bridge() {
        ...
        assert_eq!(d.direction, Direction::Local);
