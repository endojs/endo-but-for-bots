...
class HeapDecodeTests(unittest.TestCase):
    ...
    def test_the_capture_is_at_the_current_container_format(self):
        # The point of capturing this container is that it shows what the
        # engine writes now. After a format bump, re-run
        # `cargo run --example capture-map-heap` from rust/engine.
        heap = model.heap_sample()
        current = model.const_value(model.FORMAT_SRC, "IRONHORSE_FORMAT_VERSION")
        self.assertEqual(str(heap["format_version"]), current,
                         "architecture-map-heap.container is stale; re-capture it")

    def test_the_capture_example_is_present(self):
