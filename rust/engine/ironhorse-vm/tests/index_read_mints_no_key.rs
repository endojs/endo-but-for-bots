...
/// Every index-keyed read path is charged against the native-recursion
/// budget, exactly as its id-keyed twin is.
///
/// Forwarding through a chain of untrapped proxies recurses once per level.
/// The engine's contract is that such a recursion is bounded by the budget so
/// a guest gets a catchable-looking `Halt::StackOverflow` rather than a real
/// stack overflow that ABORTS THE PROCESS. `uninterned_index_get` and
/// `uninterned_index_own_descriptor` were not charged, so at 20,000 levels
/// they ran past the limit their named spellings stopped at — a guest-
/// reachable process crash on a smaller stack.
#[test]
fn a_deep_proxy_chain_halts_on_the_budget_for_either_key_spelling() {
    let nest = |expr: &str| {
        format!(
            "var p = {{}}; for (var i = 0; i < 20000; i++) {{ p = new Proxy(p, {{}}); }} \
             var r = 'no-halt'; try {{ r = String({expr}); }} catch (e) {{ r = 'threw'; }} r"
        )
    };
    for (named, index) in [
        ("p.x", "p[0]"),
        ("'x' in p", "0 in p"),
        ("delete p.x", "delete p[0]"),
        (
            "Object.getOwnPropertyDescriptor(p, 'x')",
            "Object.getOwnPropertyDescriptor(p, 0)",
        ),
    ] {
        for source in [nest(named), nest(index)] {
            let out = run(&source);
            assert!(
                !out.completed
                    && matches!(out.halt, ironhorse_vm::Halt::StackOverflow(_)),
                "must halt on the budget, not run past it: {:?}\n  {source}",
                out.halt
            );
        }
    }
