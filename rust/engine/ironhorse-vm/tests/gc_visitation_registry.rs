...
fn every_slot_bearing_field_is_classified_and_the_classification_holds() {
    ...
    let declared: Vec<_> = fields
        ...
        .collect();
    let emitted: Vec<_> = ironhorse_vm::INTERP_FIELDS
        .iter()
...
fn sweep_sources(src: &str) -> (String, String) {
    ...
    (
        ironhorse_vm::gc_tables::FULL_SWEEP_SOURCE.join("\n"),
        ironhorse_vm::gc_tables::PARTIAL_SWEEP_SOURCE.join("\n"),
    )
}
...
fn chunk_source(src: &str) -> String {
    ...
    ironhorse_vm::gc_tables::CHUNK_WALK_SOURCE.join("\n")
}
...
fn edge_sources(src: &str) -> (String, String) {
    ...
    let full = expanded_row_edges(ironhorse_vm::gc_tables::FULL_EDGE_SOURCE, true);
    let partial = expanded_row_edges(ironhorse_vm::gc_tables::PARTIAL_EDGE_SOURCE, false);
    let tail = expanded_row_edges(ironhorse_vm::gc_tables::TAIL_EDGE_SOURCE, false);
...
fn expanded_row_edges(tables: &[&str], full: bool) -> String {
    let rows = ironhorse_vm::gc_tables::ROW_EDGE_SOURCE;
...
fn assert_tail_coverage(partial: &str, tail: &str) {
    for (field, _) in ironhorse_vm::INTERP_FIELDS {
...
fn row_checks_reject_a_disconnected_call_even_when_another_table_uses_the_policy() {
    let mut tables: Vec<String> = ironhorse_vm::gc_tables::PARTIAL_EDGE_SOURCE
        .iter()
...
fn weak_sources(src: &str) -> (String, String) {
    ...
    (
        ironhorse_vm::gc_tables::EPHEMERON_SOURCE.join("\n"),
        ironhorse_vm::gc_tables::WEAK_PRUNE_SOURCE.join("\n"),
    )
}
...
fn root_source(src: &str) -> String {
    ...
    let sources = ironhorse_vm::roots::ROOT_SOURCE;
