...
and a migration that ran ends with the first.
...
| `STORE_SCHEMA_VERSION` | Snapshot `store.rs`: 35 | Change paged-store/manifest/small-state representation with a schema bump and a migration step or explicit refusal. `migrate_store` advances monotonically, checks each step's input root until phase 13's stage 2 retires it, and ends a migration that ran with the metadata-scale `validate_store`; it does not translate old meter semantics. |
