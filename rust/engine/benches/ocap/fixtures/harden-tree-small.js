...
function makeGraph(seed) {
    ...
    for (var index = 0; index < 8; index += 1) wide.children.push({ value: seed + index });
    var deep = null;
    for (var depthIndex = 8 - 1; depthIndex >= 0; depthIndex -= 1) deep = { value: depthIndex, next: deep };
    var leaves = [];
    for (var leafIndex = 0; leafIndex < 4; leafIndex += 1) leaves.push({ value: leafIndex * 3 });
  ...
  var graphs = [];
  for (var copy = 0; copy < 4; copy += 1) graphs.push(makeGraph(0));
  var root = harden({ graphs: graphs });
  var result = census(root);
  return 'harden-tree:4:' + result[0] + ':' + result[1] + ':' + result[2];
}())
