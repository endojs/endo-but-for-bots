//! Full-run reporting for `endot-ih`: per-case records, run provenance,
...
impl RunReport {
    ...
    /// owns provenance). This is what `endot-ih --json` writes per batch (a
...
/// `endot-ih --batch-size`, keeping discovery and execution single-sourced.
...
pub fn to_html(report: &RunReport) -> String {
    ...
    s.push_str("<footer><p>Generated by <code>ironhorse-262-report</code> from per-case <code>endot-ih</code> output. This report is a snapshot; the machine-readable <code>report.json</code> alongside it carries every case.</p></footer>\n");
    s.push_str("</main>\n</body>\n</html>\n");
    s
}
...
"#,
        ...
        assert_eq!(provenance.runner, "endot-ih");
...
"#,
    ...
    fn proxy_is_observed_unimplemented_via_oracle_slice() {
        ...
        // the whole-tree Proxy sweep is the `endot-ih`/full-run path.
