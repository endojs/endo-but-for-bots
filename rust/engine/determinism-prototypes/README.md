...
|---|---|
| `fixed-widths-and-string-index.patch` | Scratch admission charges a declared width per element type (a `ScratchElement` trait) instead of `size_of::<T>()`. The widths are the audited 64-bit layouts, checked by a compile-time assertion on 64-bit targets, so native thresholds do not move. It also pins the RegExp matcher's `State` and `AssertionData` charges the same way, and stops `advance_string_index` from computing `index + 1` in `usize`. |
...
Each repro then ran on unpatched and patched probes for both targets:

| Repro | Unpatched native | Unpatched wasm32 | Patched native | Patched wasm32 |
|---|---|---|---|---|
| `/z/.test` on a 16M-unit subject with a 64M-unit filler | `HeapExhausted`, 20,972,860 | `Return false`, 37,750,079 | same | same |
| `'a'.repeat(30000000).replaceAll('a','c').length` | `HeapExhausted`, 14,013,699 | `Return 30000000`, 30,000,936 | same | same |
| `replace(/a/g, 'b')` after a 134.1M-unit filler | `HeapExhausted`, 33,552,805 | `Return 5000`, 33,579,763 | same | same |
| `JSON.stringify(new Array(1000))` after a 134.2M-unit filler | `HeapExhausted`, 33,553,334 | `Return 5001`, 33,553,337 | same | same |
| `JSON.parse` with a reviver after a 134.186M-unit filler | `HeapExhausted`, 33,549,846 | `Return 1000`, 33,563,123 | same | same |
| `new RegExp('()'.repeat(129)+'a*$').test('a'.repeat(62500))` | `HeapExhausted`, 202,597 | `Return true`, 203,696 | same | same |
| a custom `exec` that sets `lastIndex = 2**32 - 1`, then `@@replace` | `Return "Qx"`, 118 | trap (overflow panic) | same | same |
| `JSON.stringify` of an array nested 450 deep with indentation 10 | `Return "RangeError:result too large"`, 268,657,025 | `HeapExhausted`, 268,656,997 | same | same |
| *Control:* `new RegExp('a'.repeat(420000)).test('a')` (the RegExp compiler, not patched) | `HeapExhausted`, 288,772 | `Return false`, 321,730 | same | `Return false`, 321,730 |
...
With both patches applied, `cargo test --release -p ironhorse-vm -p ironhorse-regexp` passes
1,187 tests and fails 1.
