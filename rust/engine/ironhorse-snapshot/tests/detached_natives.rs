...
fn dot_call_rebinds_this_onto_a_native_method() {
    let (ok, r) = run1("var n = 0; var a = 0; a = [1, 2]; n = a.push; n.call(a, 9); a.length");
    assert!(ok, "halt: {r}");
    assert_eq!(
        r, "3",
        ".call on a detached native method lands with the rebound this"
    );
    let (ok, r) =
        run1("var n = 0; var o = 0; o = { a: 1 }; n = Object.keys; n.call(null, o).length");
    assert!(ok, "halt: {r}");
...
fn dot_apply_supports_the_dense_array_and_no_array_subsets() {
    let (ok, r) =
        run1("var n = 0; var a = 0; a = [1, 2]; n = a.push; n.apply(a, [7, 8]); a.length");
    assert!(ok, "halt: {r}");
    assert_eq!(r, "4", ".apply forwards a dense argument array");
    let (ok, r) = run1("var n = 0; var a = 0; a = [1, 2]; n = a.push; n.apply(a); a.length");
    assert!(ok, "halt: {r}");
...
fn bare_detached_method_with_wrong_receiver_stays_a_named_refusal() {
    ...
    assert!(
        !ok,
        "a bare detached push with this=undefined must not complete: {r}"
    );
    assert!(
