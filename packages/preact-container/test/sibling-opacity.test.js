...
describe('sibling opacity — multi-party inline composition', () => {
  ...
  const makeAlice = () =>
    confineComponent(({ h: ch }, props) => ch('p', { class: 'alice' }, props.text), {
      name: 'AliceRegion',
    });
  ...
  it('a sibling region cannot read the other party\'s props', () => {
    ...
    const Mallory = confineComponent(({ h: ch }, props) => {
    ...
    renderConfined(
      h('article', null, h(Alice, { text: ALICE_SECRET }), h(Mallory, { text: 'nice draft!' })),
      scratch,
    );
  ...
  it('a sibling cannot read the other party\'s painted DOM (no ambient document)', () => {
    ...
    const Mallory = confineComponent(({ h: ch }, props) => {
      ...
      try {
        record(String(globalThis.document && globalThis.document.body.textContent));
      } catch (e) {
    ...
    renderConfined(
      h('article', null, h(Alice, { text: ALICE_SECRET }), h(Mallory, { text: 'c' })),
      scratch,
    );
    expectNoLeak();
  });
  ...
  it('a container party cannot read the props of the region it wraps', () => {
    ...
    const Mallory = confineComponent(({ h: ch }, props) => {
      ...
      return ch('blockquote', { class: 'mallory' }, 'as I was saying — ', inner);
    }, { name: 'MalloryWrap' });
  ...
  it('a container party cannot INVOKE the region it wraps to get its subtree as a value', () => {
    ...
    renderConfined(h(Mallory, { children: h(Alice, { text: ALICE_SECRET }) }), scratch);
    expectNoLeak();
  });
  ...
  it('a container party cannot RE-PARAMETERIZE the region it wraps', () => {
    ...
    const Mallory = confineComponent(({ h: ch }, props) => {
      ...
      try {
        ...
        return ch('div', null, ch(inner.type, { text: 'ALICE-SAYS-MALLORY-IS-RIGHT' }));
      } catch (e) {
  ...
  it('two confined regions do not share mutable state through their endowments', () => {
    const Alice = confineComponent((endowments, props) => {
      try {
  ...
  it('the shared channel is not merely one level deep (h.prototype, __proto__)', () => {
    ...
    const Alice = confineComponent((endowments, props) => {
      for (const write of [
        ...
        try { write(); } catch (e) { /* frozen — the channel does not exist */ }
      }
      return endowments.h('p', null, props.text);
    }, { name: 'AliceDeepStash' });
    const Mallory = confineComponent(({ h: ch }, props) => {
    ...
    try { delete Object.prototype.__dropbox2; } catch { /* */ }
  ...
  it('a party cannot reach the other\'s content through a ref', () => {
    ...
    const Alice = makeAlice();
    const Mallory = confineComponent(({ h: ch }, props) => {
      const grab = el => record(el ? String(el.ownerDocument.body.textContent) : 'null-ref');
      return ch('div', { ref: grab }, ch('span', { ref: grab }, props.text), props.children);
    }, { name: 'MalloryRef' });
  ...
  it('a party cannot forge the frame\'s attribution mark', () => {
    ...
    const Attribution = sealComponent(
      ({ party }) => h('span', { class: 'attribution' }, marks[party] || '(unattributed)'),
      { params: ['party'] },
    );
    const Mallory = confineComponent(({ h: ch }, props) => {
  ...
  it('the frame\'s secret pattern is not readable by any party', () => {
    ...
    const Badge = sealComponent(() => h('span', { class: 'pattern' }, PATTERN), { params: [] });
    const Mallory = confineComponent(({ h: ch }, props) => {
      try {
  ...
  it('an unattributed region renders as unattributed — it does not inherit its container\'s mark', () => {
    ...
    const Attribution = sealComponent(
      ({ party }) => h('span', { class: 'attribution' }, party ? `✦ ${party}` : '(unattributed)'),
      { params: ['party'] },
    );
    const Region = confineComponent(({ h: ch }, props) =>
      ch('section', null, ch(props.Attribution, { party: props.party }), props.text),
    { name: 'Region' });
    ...
    const marks = [...scratch.querySelectorAll('.attribution')].map(n => n.textContent);
