// @ts-check
// Regression test for the "unhandled-rejection trap: empty-object
// reasons render as `{}`" symptom (Error own-property invisibility).
//
...
//
// 2. The receiver-side rendering: a diagnostic shaped like the inline
//    `console.error('CapTP', name, 'exception:', err?.message || err,
//    err?.stack || '')` pattern formerly used by daemon callers, when
//    fed the round-tripped reason, produces a string that does NOT
//    contain the original Error's message - only the literal
//    `'[object Object]'` or `'{}'` rendering of an empty object.
//
// Both assertions fail before the wire-format fix; after the fix that
// preserves Error structure across the wire, both pass.
