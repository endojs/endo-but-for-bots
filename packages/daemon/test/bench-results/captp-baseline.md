...
- **Pure Node.js is ~3x faster than Rust+XS on inter-worker.**  The
...
Speculative targets:
- `worker_to_worker_ping` Rust+XS: **1.4 -> 0.6–0.9 ms** (1.5–2x win).
- `worker_to_worker_echo_1kib` Rust+XS: **2.6 -> 1.0–1.5 ms**
  (~2x win, dominated by the skipped JSON encode/decode of the body).
