...
test('getMountBacking reports the effective denied set', async t => {
  const rootPath = makeDenyFixture(t);
  // The default set, the empty (denial disabled) set, and a replacement
  // set are the three shapes a host has to tell apart before handing a
  // mount's backing directory to anything outside the mount's own path
  // resolution.
