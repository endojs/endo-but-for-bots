...
test('caret ranges on 0.x packages stay within semver-compatible bounds', async t => {
  const resolver = makeResolver({
    a: {
      '0.2.3': packageJson('a', '0.2.3'),
      '0.2.4': packageJson('a', '0.2.4'),
      '0.3.0': packageJson('a', '0.3.0'),
      '0.9.0': packageJson('a', '0.9.0'),
    },
    b: {
      '0.0.3': packageJson('b', '0.0.3'),
      '0.0.4': packageJson('b', '0.0.4'),
    },
  });

  const resolution = await resolver.resolve(
    packageJson('entry', '1.0.0', {
      dependencies: { a: '^0.2.3', b: '^0.0.3' },
    }),
  );

  t.deepEqual(resolution.keys, ['a@0.2.4', 'b@0.0.3']);
});

test('wildcard ranges select one greatest satisfying package version', async t => {
  const resolver = makeResolver({
    a: {
      '1.5.0': packageJson('a', '1.5.0'),
      '2.3.0': packageJson('a', '2.3.0'),
  ...
  const resolution = await resolver.resolve(
    packageJson('entry', '1.0.0', { dependencies: { a: '*' } }),
  );

  t.deepEqual(resolution.keys, ['a@2.3.0']);
});
...
test('unsatisfied optionalDependencies do not constrain later required edges', async t => {
  const resolver = makeResolver({
    a: {
      '1.5.0': packageJson('a', '1.5.0'),
    },
  ...
  const resolution = await resolver.resolve(
    packageJson('entry', '1.0.0', {
      optionalDependencies: { a: '^1.9.0' },
      dependencies: { b: '^1.0.0' },
    }),
  );

  t.deepEqual(resolution.keys, ['a@1.5.0', 'b@1.0.0']);
  t.like(resolution.diagnostics.unmetOptionals[0], {
    importer: 'entry',
    name: 'a',
    range: '^1.9.0',
