...
export const helpTextEntries = harden([
  ...
  [
    'EndoMount',
    {
      ...
      glorp:
        'glorp(globPattern, grepPattern, options?) -> Promise<Array<{ file, line, text }>>\nGrep a pattern across exactly the files a glob matches — the grep-over-glob convenience\ncombinator, equivalent to grep(grepPattern, await glob(globPattern), options).\nglobPattern: string — A glob pattern, exactly as glob() takes it, selecting the files to search.\ngrepPattern: string — An ECMAScript RegExp source, exactly as grep() takes it.\noptions.maxResults: number — Cap on the number of match records (default 1000), forwarded to grep.\nglob and grep stay decoupled: glorp just globs, then greps the resulting paths — the same as\nchaining grep(grepPattern, glob(globPattern)) by hand. Returns the same { file, line, text }\nrecords as grep. Denied, escaping, directory, or unreadable paths are skipped silently.\nExample: glorp("src/**/*.js", "TODO") → every TODO line under src.',
      lookup:
