...
// Bus daemon entry point — the Node.js daemon process running under an
// external supervisor (Go or Rust). Receives configuration as command-line
// arguments and communicates with the supervisor via the envelope protocol
// on fd 3/4.
//
// fd 3: daemon writes envelopes to supervisor (child → parent)
// fd 4: daemon reads envelopes from supervisor (parent → child)

if (process.argv.length < 5) {
  throw new Error(
    `bus-manager-node.js requires arguments [sockPath] [statePath] [ephemeralStatePath] [cachePath], got ${process.argv.join(
      ', ',
    )}`,
  );
}
...
// Read the init envelope from the supervisor to learn our handle.
const readInitEnvelope = async () => {
  const frameData = await readFrameFromStream(envelopeReadStream);
  if (frameData === null) {
    throw new Error('bus-daemon: EOF before init envelope');
  }
  const env = decodeEnvelope(frameData);
  if (env.verb !== 'init') {
    throw new Error(`bus-daemon: expected init envelope, got ${env.verb}`);
  }
  return env.handle;
};
...
const main = async () => {
  ...
  // Read init envelope from supervisor.
  const selfHandle = await readInitEnvelope();
  console.log(`Endo daemon assigned handle ${selfHandle} by supervisor`);
  ...
  if (powers.control.startEnvelopeReader) {
