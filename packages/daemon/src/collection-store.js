...
export const makeCollectionStoreMaker = ({
  ...
  const makeIdentifiedMapStore = (storeId, formulaNumber) => {
     ...
     * @property {string | null} valueBody
    ...
    const readValue = entry => {
      entry.valueBody !== null || Fail`collection entry has no value`;
      return unmarshalFromCapData({
        body: /** @type {string} */ (entry.valueBody),
        slots: entry.valueSlots,
      });
    };
  ...
  /**
   * Construct (or reconstruct) a durable strong `SetStore` using the same
   * key index and retention machinery as `MapStore`, with null value columns.
   *
   * @param {FormulaIdentifier} storeId
   * @param {FormulaNumber} formulaNumber
   * @returns {SetStore}
  ...
  const makeIdentifiedSetStore = (storeId, formulaNumber) => {
    /** @typedef {{ keyBody: string, keySlots: string[], slotIds: FormulaIdentifier[] }} SetEntry */
    /** @type {Map<string, SetEntry>} */
    const entriesByRank = new Map();
    /** @type {Map<FormulaIdentifier, number>} */
    const slotCounts = new Map();
    ...
    for (const row of listCollectionEntries(formulaNumber)) {
      row.valueBody === null ||
        Fail`set entry unexpectedly has a value for rank ${row.keyRank}`;
      const keySlots = JSON.parse(row.keySlots);
    ...
    const setStore = {
      ...
      delete: async key => {
        ...
        deleteCollectionEntry(formulaNumber, rank);
        entriesByRank.delete(rank);
        await applyRetentionDelta([], /** @type {SetEntry} */ (entry).slotIds);
      },
      getSize: async () => entriesByRank.size,
      keys: async () => {
        await null;
        return harden(
          await Promise.all(
            sortedRanks().map(rank =>
              readKey(/** @type {SetEntry} */ (entriesByRank.get(rank))),
            ),
          ),
        );
      },
      ...
      entries: async () => {
        ...
        return harden(
          await Promise.all(
            sortedRanks().map(async rank => {
              const key = await readKey(
                /** @type {SetEntry} */ (entriesByRank.get(rank)),
              );
              return harden([key, key]);
            }),
          ),
        );
      },
