 ...
 * messages routed through the supervisor. The pipe layout:
 *
 *   fd 3: worker writes to supervisor (child → parent)
 *   fd 4: worker reads from supervisor (parent → child)
 *
 * The worker reads an init envelope on startup to learn its handle,
 * then wraps/unwraps CapTP traffic in envelopes with verb "deliver".
 * The supervisor routes these envelopes to/from the daemon using handle
 * rewriting.
 */
...
export const makePowers = ({ fs, url }) => {
  ...
  const startBufferedReading = () => {
    const loop = async () => {
      ...
      if (initFrame === null) {
        throw new Error('bus-worker: EOF before init envelope');
      }
    ...
    void loop().catch(err => {
      console.error('bus-worker: envelope reader error:', err);
      readerDone = true;
      if (readerResolve) {
        readerResolve({ done: true, value: undefined });
        readerResolve = null;
