/**
 * Bundle the thixotrope XS worker scripts for the `thixotrope-xs-worker` binary
 * (rust/thixotrope-xs-worker), and satisfy the xsnap crate's generated-file
 * build inputs.
 *
 * Produces:
 *   packages/thixotrope/dist-xs/boot.js        — pre-bundle boot script (a
 ...
 *     so the crate compiles; thixotrope-xs-worker never evaluates them. The
 *     real ones belong to the endor daemon's own bundler.
 *
 * Usage: node packages/thixotrope/scripts/bundle-xs-worker.mjs
 */
import '@endo/init';
import fs from 'fs';
import url from 'url';
...
for (const stub of [
  ...
  if (!fs.existsSync(stubPath)) {
    fs.writeFileSync(
      stubPath,
      `/* Stub generated by packages/thixotrope/scripts/bundle-xs-worker.mjs so
 * the xsnap crate compiles. thixotrope-xs-worker never evaluates this file.
