...
test('a closing paren the URL opened stays in the link', t => {
  const parts = linkify('read https://en.wikipedia.org/wiki/Foo_(bar) first');
  const [link] = anchors(parts);
  t.is(link.props.href, 'https://en.wikipedia.org/wiki/Foo_(bar)');
  // A paren the sentence closed does not.
  const wrapped = linkify('(see https://example.com/page)');
  t.is(anchors(wrapped)[0].props.href, 'https://example.com/page');
  t.is(wrapped[wrapped.length - 1], ')');
});

test('a long punctuation run is trimmed in one pass', t => {
  // A reply carrying a URL glued to thousands of periods must not stall the
  // view: the trim is linear in the run, not quadratic.
  const run = '.'.repeat(100_000);
  const parts = linkify(`https://example.com/x${run}`);
  const [link] = anchors(parts);
  t.is(link.props.href, 'https://example.com/x');
  t.is(parts[parts.length - 1], run);
});
