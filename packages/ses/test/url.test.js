...
// change reverted — it does NOT witness this PR's behavior. The load-bearing
// regression pin for the warning-suppression lives in
...
test('characterization: the blob statics keep their undeletable `.prototype` end state', t => {
  ...
  for (const name of ['createObjectURL', 'revokeObjectURL']) {
    ...
    // The undeletable own `.prototype` slot is a Node.js/V8 artifact. A
...
test('URL.prototype is shared between the start and shared compartments', t => {
  ...
  // The %InitialURL% and %SharedURL% constructors are distinct values but share one
...
test('URL.prototype.constructor points at the tamed URL, not the powered one', t => {
  ...
  // the tamed `%SharedURL%` (no blob statics), never the powered
  // `%InitialURL%` the start compartment may keep.
  // Without the constructor re-point in tame-url-constructor.js, a shared
  // compartment would regain the ambient blob-registry authority through
  // `URL.prototype.constructor.createObjectURL` even though its own `URL`
  // binding omits it.
  // This test reddens if that re-point is dropped, mirroring how
  // `%DatePrototype%.constructor` is pinned to `%SharedDate%`.
  const c = new Compartment();
