...
- **Breaking:** Passable byte readers and writers now use the generic `stream()`
  ...
  representation — the current hex marshal encoding is ~1.5x larger and, in
  ad-hoc measurement, several times slower on Node 22 — pending a compact
  byteArray marshal path; see `DESIGN.md` § Bytes Transport Decision.
