...
test('roundtrip across all kinds and directions', t => {
  for (const direction of [Direction.Local, Direction.Remote]) {
    for (const kind of [Kind.Object, Kind.Promise, Kind.Answer, Kind.Device]) {
      for (const position of [0, 1, 23, 24, 255, 256, 65_535, 65_536]) {
        const d = { direction, kind, position };
        const bytes = encodeDescriptor(d);
        const d2 = decodeDescriptor(bytes);
        t.is(d2.direction, direction);
        t.is(d2.kind, kind);
...
test('descriptorKey is stable and unique', t => {
  const a = { direction: Direction.Local, kind: Kind.Object, position: 5 };
  const b = { direction: Direction.Local, kind: Kind.Object, position: 5 };
  const c = { direction: Direction.Remote, kind: Kind.Object, position: 5 };
