  ...
  async t => {
    ...
    try {
      ...
      if (/slirp4netns or pasta/.test(/** @type {Error} */ (e).message)) {
        t.pass(`rootless network backend missing: ${/** @type {Error} */ (e).message}`);
        return;
      }
      throw e;
    }
    ...
    t.true(
      ifaces.includes('lo'),
      'loopback present inside private slice',
    );
    t.true(
      ifaces.some(i => i !== 'lo'),
...
test.serial(
  'apk update succeeds inside a private alpine slice',
  async t => {
    await null;
    if (!podmanAvailability.available || !podmanAvailability.imagePresent) {
      t.pass(
...
test.serial('podman driver rejects non-OCI rootfs at slice construction', async t => {
  if (!podmanAvailability.available) {
    t.pass(`podman not available: ${podmanAvailability.reason}`);
    return;
  }
