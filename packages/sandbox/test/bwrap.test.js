...
test.serial('listBackends() reports bwrap available via the factory', async t => {
  if (!bwrapAvailability.available) {
    t.pass(`bwrap not available: ${bwrapAvailability.reason}`);
    return;
  }
...
test.serial(
  ...
  async t => {
    ...
    const ifaces = stdout
      ...
      .filter(name => name !== '');
    t.true(ifaces.includes('lo'), 'host loopback visible inside host-net slice');
