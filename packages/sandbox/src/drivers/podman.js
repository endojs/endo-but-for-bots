...
const assembleCreateArgv = (spec, containerName, netBackend, extras) => {
  ...
  const seccompOpt = seccompSecurityOpt(spec.seccomp, extras.seccompProfilePath);
...
export const makePodmanDriver = ({
  ...
  const prepareSlice = async spec => {
    ...
    if (
      ...
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'endo-sandbox-seccomp-'));
      const file = path.join(dir, 'profile.json');
      const profile = /** @type {{ profile: unknown }} */ (spec.seccomp).profile;
      const body =
      ...
      rootlessNet: harden(
        netBackend === null
          ? {
            ...
            reason: 'no rootless network backend on PATH (slirp4netns / pasta)',
          }
          : { backend: netBackend },
      ),
    });
