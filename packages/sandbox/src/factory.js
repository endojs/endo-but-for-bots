 ...
 * `throw`).
 *
 * @param {AsyncIterable<Uint8Array> | undefined} iterable
 * @returns {object}
 */
...
export const makeSandboxFactory = ({ drivers, scratchProvider }) => {
  ...
  const resolveRootfs = async rootfs => {
    ...
    if (
      ...
      const ociSpec =
        /** @type {{ kind: 'oci'; ref: string }} */ (rootfs);
  ...
  const make = async opts => {
    ...
    const disposeSlice = async () => {
      ...
      await Promise.all(
        // eslint-disable-next-line no-empty-function
        [...liveMounts].map(m => E(m).unmount().catch(() => {})),
      );
  ...
  const help = methodName => {
    ...
    const text = METHOD_HELP[/** @type {keyof typeof METHOD_HELP} */ (methodName)];
    if (text === undefined) {
