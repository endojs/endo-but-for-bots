...
(endojs/endo-but-for-bots#125). Pass done: false to mark a partial
submission (recipient should show a progress indicator); pass done: true
(or omit) once the message has settled.`,
      parameters: {
        type: 'object',
        properties: {
          messageNumber: {
...
export const spawnWorkerLoop = async (powers, context, workerEnv) => {
  ...
  const runAgent = async () => {
    ...
    // sender called daemon `editMessage`
    // (endojs/endo-but-for-bots#125): a partial submission that has
