...
} from '../src/code-mode-globals/fs.js';
...
test('capability global factories preserve custom lexical names and pet paths', t => {
  const globals = [
    makeDaemonMountGlobal({ name: 'sourceTree', petName: ['repo', 'mount'] }),
    makeDaemonMountGlobal({
      name: 'referenceTree',
      petName: 'repo/read-only',
      readOnly: true,
    }),
    ...
    makeLocalFilesystemGlobal({ name: 'localFs', petName: 'local-fs' }),
  ];

  t.like(globals[0], {
    name: 'sourceTree',
    petName: ['repo', 'mount'],
    declaration: { body: 'DaemonMount' },
  });
  t.like(globals[1], {
    name: 'referenceTree',
    petName: 'repo/read-only',
    declaration: { body: 'DaemonMountReadOnly' },
  });
  t.like(globals[2], {
    name: 'builderShell',
  ...
  t.like(globals[5], {
    name: 'localFs',
    declaration: { body: 'Filesystem' },
  });

  const prompt = formatGlobalDeclarations(globals);
  t.true(prompt.includes('declare const sourceTree: DaemonMount;'));
  t.true(prompt.includes('declare const referenceTree: DaemonMountReadOnly;'));
  ...
  t.true(prompt.includes('declare const localFs: Filesystem;'));
});
...
test('a compartment can evaluate code against fake capability globals', async t => {
  ...
  const mountGlobal = makeDaemonMountGlobal({ name: 'mount' });
  const globals = [shellGlobal, httpGlobal, remoteGlobal, mountGlobal];
  ...
  t.true(prompt.includes('declare const mount: DaemonMount;'));
  ...
  return {
    ...
    text: await E(mount).readText('README.md'),
  };
})()`,
    globals,
  });
