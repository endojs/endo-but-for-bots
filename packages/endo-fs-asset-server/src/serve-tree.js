 ...
 * (`SnapshotBlobInterface` = `stream` / `text` / `json` / `sha256` /
 * `size`; `SnapshotTreeInterface` = `has` / `list` / `lookup` / `sha256` /
 * `size`). A leaf's bytes are read by driving its `stream` responder
 ...
 * confirmed to be a *file* by the shared `looksLikeReadableBlob` discriminator
 * over its method names (introspected via `__getMethodNames__`) — a
 * `SnapshotBlob` carries `stream`, a `SnapshotTree` does not. A path that
 * resolves to a
...
export const makeTreeRequestHandler = ({ tree, index = 'index.html' }) => {
  ...
  const handler = async request => {
    ...
    // `text`/`getInfo`/`readReturnPattern` marker), not by the shared
    // metadata methods alone (both blobs and trees expose those).
    let blob;
