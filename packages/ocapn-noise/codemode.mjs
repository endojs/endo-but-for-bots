...
export const runAgentCode = async ({ toolbox, manifest, userText, history = [], onStep = () => {}, signal, persona = '', attachments = [], model = 'default', llm, budgetLine = '', resumeMessages = null, callLLM = defaultCallLLM, buildUserContent = defaultBuildUserContent } = {}) => {
  ...
  const sys = [
    ...
    'NEW / UNFAMILIAR OBJECTS ARE SELF-DOCUMENTING. If you have ACCEPTED an Endo object into your inventory (the `objects` power: listObjects shows them, callObject(name, method, args) invokes them) and you do not recognise it or do not know its methods, do NOT give up or guess — FIRST call `callObject(name, "describe")` (or "help"), read the methods it reports, THEN call them. Every Endo object answers describe()/help(). The same goes for any live in-scope object via `methodsOf(obj)`. Introspect first; an object you hold is never "unusable" until you have asked it what it can do.',
    '',
