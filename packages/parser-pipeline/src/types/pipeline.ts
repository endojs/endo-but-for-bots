...
/**
 * Function to call with "Babel-recoverable" parsing errors.
 *
 * Some errors encountered during parsing are unrecoverable, and will cause the
 * parsing operation to throw an exception instead.
 *
 * @param errors - The array of parse errors.
 * @param location - The location of the module being parsed in `file://` URL
 * format.
 */
export type OnParseErrorFn = (
...
> {
  ...
  /**
   * Shared callback called if a parsing a module results in "Babel-recoverable" `ParseError`s.
   *
   * You may elect to throw from this function to abort the parsing operation even on recoverable errors.
   *
...
/**
 * In practice, {@link ParseError} is always an {@link Error}.
 */
export type BabelParseError = ParseError & Error;
