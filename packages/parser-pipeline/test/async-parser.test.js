...
test.serial('onParseError is called with recoverable errors', async t => {
  ...
  t.is(typeof err.pos, 'number', 'ParseError preserves its source position');
  t.is(typeof err.loc?.line, 'number', 'ParseError preserves its location');
  t.truthy(err.details, 'ParseError preserves its details');
});
