...
const float64Macro = test.macro({
  ...
  /**
   * @param {string | undefined} providedTitle
   * @param {any} inputValue
   * @param {string} [_expectedDiagnostic]
   */
  title(providedTitle, inputValue, _expectedDiagnostic) {
...
test('interop: op:deliver pattern', async t => {
  // Pattern: 27([280("op:deliver"), to-desc, args])
  const tag27Bytes = new Uint8Array([0xd8, 0x1b]);
  ...
  // Label
  innerWriter.writeSelectorFromString('op:deliver');
  ...
  t.is(value.tag, 27);
  t.is(value.value[0].tag, 280);
  t.is(value.value[0].value, 'op:deliver');
});
