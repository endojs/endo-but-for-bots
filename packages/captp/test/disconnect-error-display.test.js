// @ts-check
// Repro for the "unhandled-rejection trap: empty-object reasons render
// as `{}`" symptom (Error own-property invisibility).
//
// This file is the language-layer sentinel: it pins the underlying
// ECMA-262 fact that a host transport which naively `JSON.stringify`s
// a CapTP envelope containing an `Error` reason will emit `{}` for the
// reason, because `Error`'s own properties (`message`, `stack`, `name`)
// are spec-defined non-enumerable.
//
...
// transport is fixed lives in
// `@endo/daemon/test/disconnect-error-display.test.js`.
//
