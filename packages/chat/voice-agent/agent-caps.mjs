  ...
  const toolboxAndManifestFor = (powers, node, ctx = {}) => {
    ...
    if (node.isRoot && customView && typeof customView.register === 'function') {
      ...
      manifest.push({ name: 'customView', reversible: false, args: {
        ...
        source: 'string — the renderer: a SINGLE arrow function `(endowments, props) => vnode` (NO module, NO JSX). ⚠️ Build ONLY with RAW HTML tags: `endowments.h(\'div\'|\'button\'|\'input\'|\'textarea\'|\'span\'|\'select\'|…, props, ...children)`. Do NOT call component primitives like h(Card,…)/h(Btn,…)/h(TextField,…) — they render NOTHING in a confined renderer. Theme via CSS classes on raw tags: button→class:"mini" (or "mini primary"); text input→`h("input",{class:"kit-in",value,placeholder,onInput:e=>setV(e.target.value),onKeyDown:e=>{if(e.key==="Enter")go()}})`; card→class:"ncard"; chip/empty→class:"pill"; row→class:"kit-rowx"; stack→class:"kit-stack". Events are standard Preact (onClick/onInput/onChange/onKeyDown; read e.target.value). props.value = a SAMPLE of the object\'s data — render it legibly for THIS shape. props.methods = method names; props.call(method, argsArray) = INVOKE one (host-mediated; returns a Promise) — wire actions to it (e.g. a Send button onClick → props.call("send",[text]).then(props.refresh)); NEVER call a method not in props.methods. props.refresh() re-fetches. endowments.useState/useEffect = local UI state. CONFINEMENT: no DOM/network/fs/caps beyond props.call. Use theme CSS vars (var(--ink)/--mut/--edge/--acc/--panel/--bg), never hardcoded colours.',
