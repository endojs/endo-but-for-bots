...
export const makeFieldAgent = ({ outDir, baseUrl, autoConfirmFile, specialistsFile, fillSpecialist, wandPolicy, customView = {}, peerBridge = endoPeer, peerRedemption = process.env.FIELD_AGENT_PEER_REDEMPTION === '1' } = {}) => {
  ...
  const baseVerb = harden({
    ...
    bufferDraftAll: { reversible: true, args: { text: 'string — the post text to draft everywhere' }, description: 'Save the same DRAFT to EVERY connected channel (nothing published — each draft waits in Buffer for your approval). Safe; reversible by deleting the drafts. To publish to all at once, use proposeBufferBlast.',
      run: async ({ text }) => { const r = await aff.buffer.blast({ text: String(text || ''), draft: true }); return { ok: r.ok, drafted: r.posted, failed: r.failed, results: r.results }; } },
    proposeBufferBlast: { reversible: false, args: { text: 'string — the message to blast', mode: 'string — addToQueue (next slot, default) | shareNow (immediately) | customScheduled (needs dueAt)', dueAt: 'string — ISO-8601 UTC, required for customScheduled' },
      description: 'PROPOSE BLASTING one message to ALL your connected social channels at once via Buffer. Does NOT publish — you review the text + the full channel list + timing and confirm; only on confirm does it go live everywhere.',
      run: async ({ text, mode, dueAt }, agent) => { const chs = (await aff.buffer.channels()).filter(c => !c.isDisconnected); return propose({ type: 'buffer-blast', power: 'buffer', agent, title: `Blast to ALL ${chs.length} channels`, summary: String(text || '').slice(0, 140), detail: { channels: chs.map(c => `${c.service} · ${c.displayName || c.name}`).join(', '), text: String(text || ''), when: String(mode || 'addToQueue') + (dueAt ? ` @ ${dueAt}` : '') }, commit: () => aff.buffer.blast({ text: String(text || ''), mode: String(mode || 'addToQueue'), dueAt }) }); } },
  ...
  const toolboxAndManifestFor = (powers, node, ctx = {}) => {
    ...
    if (powers.has('notes') || powers.has('home') || powers.has('web')) {
      ...
      manifest.push({ name: 'readPdf', reversible: false,
        ...
        description: `Read a PDF document and extract its TEXT, per page. Source it from ${src} — give exactly one. Returns { pages:[{page,text}], text (joined with page markers), totalPages, truncatedPages, truncatedChars }. Honors least authority: a vault PDF is read through your notes folder, a home PDF through your home jail, a URL through the SSRF-guarded web power. Scanned/image-only PDFs return empty text (no OCR). ⚠️ EXTRACT ONCE, REUSE THE STRUCTURE: if this document will inform MULTIPLE turns (a schedule, a table, a spec), do NOT re-read + re-parse the raw PDF every prompt. On the FIRST pass, parse the relevant facts into a DURABLE DATA STRUCTURE and operate on THAT afterwards — save structured JSON to your home folder (fileWrite) and fileRead it later, or (best) proposeTool/forgeTool a small grain-backed tool that ingests it once + exposes query methods. Re-reading the same doc each turn is slow, costs tokens, and re-derives the same facts.` });
    }
    ...
    toolbox.showComponent = harden({ run: async ({ source, cells, height, uses, writableCells } = {}) => {
      ...
      const granted = (Array.isArray(writableCells) ? writableCells : []).map(String).filter(c => c === 'theme'); // only the SAFE `theme` propagator cell today (CSS vars, reversible)
      if (granted.length) widget.writableCells = granted; // → the component may ui.grain('theme').set({name,vars}) to drive the global style propagator
      const reused = Object.keys(refs).length;
      // FEEDBACK LOOP — a UI meant to ACT ON an inventory object cannot be built here: showComponent only
      // reaches LOCAL grains + tools, never an accepted capability. If this looks like an object/peer chat
      // while we hold inventory objects, the component is talking to a local stand-in — steer to customView,
      // which hands the component a real props.call to the OBJECT ITSELF (designation by reference).
      const callableObjs = acceptedObjects.filter(o => isHttpCallable(o) || isIrohCallable(o) || isPeerCallable(o));
      const looksLikeObjUI = /\bsendMessage\b|message[-\s]?box|\bmessageBox\b|chat[-\s]?box|\binbox\b|relay|\bsend\b[^]{0,24}\bmessage\b|\bmessage\b[^]{0,24}\bsend\b/i.test(src);
      const objSteer = (callableObjs.length && looksLikeObjUI)
        ? ` ⚠️ This looks like a UI to MESSAGE/act on something — but a showComponent CANNOT reach an inventory object (it only has local grains + tools), so it is only talking to a LOCAL stand-in (a tool that appends to itself). To make a UI that genuinely acts on one of your objects (${callableObjs.slice(0, 4).map(o => o.name).join(', ')}) — e.g. send to a peer — call \`customView({ kind: 'object', objectName: '<the object>', methods: [...the methods you need...] })\` instead: the component then receives a real \`props.call(method, args)\` to THAT object. Rebuild it that way. (Read-only? Pass only the object's READ methods — the component is read-only because of the handle it holds, not a type.)`
        : '';
      return { ok: true, widget, note: `Rendered a custom confined component${reused ? ` (reused ${reused} saved component(s) via ui.use)` : ''}${dropped.length ? ` (dropped unreachable cells: ${dropped.join(', ')} — haFind them first)` : ''}.${objSteer}` };
    } });
    ...
    toolbox.showThemePreview = harden({ run: async ({ name, vars, mode, themes } = {}) => {
      const cleanVars = v => { if (!v || typeof v !== 'object' || Array.isArray(v)) return null; const c = {}; for (const k of Object.keys(v)) { if (/^--[\w-]+$/.test(k) && typeof v[k] === 'string' && v[k].length <= 60) c[k] = v[k]; } return Object.keys(c).length ? c : null; };
      // GALLERY: `themes:[{name,vars,mode}]` → a try-on strip (the user flips between them live). Single `vars`
      // → one before/after (legacy). Either way it's pure style data, never authority.
      const list = (Array.isArray(themes) && themes.length ? themes : [{ name, vars, mode }])
        .map(t => ({ name: String((t && t.name) || 'custom').slice(0, 40), mode: (t && (t.mode === 'light' || t.mode === 'dark')) ? t.mode : undefined, vars: cleanVars(t && t.vars) }))
        .filter(t => t.vars).slice(0, 12);
      if (!list.length) return { ok: false, error: 'provide vars (or themes:[{name,vars,mode}]): CSS custom properties, e.g. {"--bg":"#1a1410","--panel":"#241c14","--edge":"#3a2e20","--ink":"#f0e6d2","--mut":"#a8957a","--acc":"#c98a3a","--acc2":"#6a8a3a","--bad":"#cf5a3a","--you":"#3a6a9a"}.' };
      const widget = list.length > 1 ? { type: 'theme-preview', themes: list } : { type: 'theme-preview', name: list[0].name, mode: list[0].mode, vars: list[0].vars };
      return { ok: true, widget, note: list.length > 1 ? `Showed a try-on strip of ${list.length} themes — the user flips between them live and keeps one.` : 'Showed a before/after theme preview — the user decides whether to apply it globally.' };
    } });
    manifest.push(
      { name: 'showThemePreview', reversible: false, args: { name: 'string — a short name for the theme', vars: 'object — CSS custom properties to preview. Themeable keys: --bg (page) --panel (cards) --edge (borders) --ink (text) --mut (muted text) --acc (accent/buttons) --acc2 (good) --bad (errors) --you (your bubble). Give the FULL set for a coherent look, e.g. a warm theme: {"--bg":"#1a1410","--panel":"#241c14","--edge":"#3a2e20","--ink":"#f0e6d2","--mut":"#a8957a","--acc":"#c98a3a","--acc2":"#6a8a3a","--bad":"#cf5a3a","--you":"#3a6a9a"}', mode: 'string — OPTIONAL "light" or "dark" (else inferred from --bg brightness)', themes: 'array — OPTIONAL: propose SEVERAL variants to TRY ON, [{name, vars, mode}, …]. The user flips between them live and keeps one. Use this when the user wants to compare/try options.' }, description: 'Propose a theme (the user\'s global style). Pass a single `vars` for a before/after, OR `themes:[{name,vars,mode}]` for a TRY-ON strip the user flips between live (use when they want to compare options). On keep it becomes their live style EVERYWHERE. Use whenever the user asks to change colors / restyle / make a theme / try theme options.' },
      ...
      { name: 'showComponent', reversible: false, args: { source: 'string — a function (ui) => ui.create(...). ui.create(tag) → element with .text(s)/.attr(k,v)/.class(c)/.style({...})/.on(event,fn)/.push(children)/.follow(grain, v=>text). ui.island(name, props, children) → a THEMED design-system primitive so it matches the app — name ∈ [Card, Btn, Chip, Badge, Banner, Meta, Stack, Row, Divider, EmptyState, ProgressBar, Field, TextField]; PREFER these over raw create() for a consistent look. ⚠️ The CONTENT primitives take their text as a NAMED PROP, not children: Badge/Chip/Btn use {label}, Banner/EmptyState use {text}, Meta uses {parts:[...]}, Card uses {title}/{body} — e.g. a time chip is ui.island("Badge",{label:"2:30 PM"}), NOT ui.island("Badge",{},[ui.create("span").text("2:30 PM")]). LAYOUT primitives (Card/Stack/Row/Field) take children. (Children passed to a content primitive now fall back to rendering, but use the named prop.) ui.use(alias) → render a PRIOR saved component you referenced in `uses` (e.g. ui.use("door")). ui.grain(cellId) → a LIVE server cell (e.g. "ha:<handle>" — follow it). ui.local(initial) → client state.', cells: 'array — the server cell ids your component will follow (e.g. ["ha:<handle>"]); declare them so the live data is brokered + cap-gated', uses: 'object — OPTIONAL {alias: componentId} map (ids from listComponents) to REUSE saved components live; call ui.use(alias) in your source. Up to 4. Their data still re-gates by what you hold.', height: 'number — initial px height (it auto-grows)', writableCells: 'array — OPTIONAL cells your component may DRIVE (write), only "theme" today. With writableCells:["theme"] read the live theme as a grain — const t = ui.grain("theme"); t.subscribe(v => …) — and DRIVE the whole app\'s style by writing it: t.set({name, vars}) (pure CSS vars; propagates everywhere). Revert = write the prior value back. This is a propagator (shared state), not a command.' }, description: 'Render a CUSTOM component you write — confined + safe (sandboxed; no DOM/network/authority) and ALWAYS available regardless of your powers (building UI is a free right; only the DATA it draws via ui.grain is gated by what you hold). Compose the themed kit with ui.island, and REUSE a saved component live with `uses` + ui.use(alias) (listComponents to find ids). Use when status/countdown/choices don\'t fit. It can FOLLOW live grains and be broken out + shared. To build a THEME switcher that applies on click, pass writableCells:["theme"]: ui.grain("theme") is the live theme cell — read it with .subscribe and DRIVE the app by writing ui.grain("theme").set({name,vars}) (revert = write the prior value). ⚠️ NOT for a UI that ACTS ON an inventory object (chat with a peer, control a device, use an accepted capability): showComponent only reaches LOCAL grains + tools — it CANNOT call an object in your inventory, so a "chat box for Kumavis" built here only talks to a local stand-in. For that, use `customView` for the object instead — it hands the component a real props.call(method,args) to the object itself (authority by reference).' },
    );
    ...
    if (node.isRoot && customView && typeof customView.register === 'function') {
      ...
      manifest.push({ name: 'customView', reversible: false, args: {
        ...
        source: 'string — the renderer: a SINGLE arrow function `(endowments, props) => vnode` (NO module, NO JSX). Build with `endowments.h(type, props, ...children)` where type is a raw HTML tag string OR a themed UI-KIT component (preferred). ⚠️ Kit components take NAMED props, NOT React children/events: `h(Btn,{label:"Send",onClick,variant:"primary",disabled})` (label is the text); `h(TextField,{value,placeholder,onInput:v=>setV(v),onEnter:v=>go(v)})` (onInput/onEnter receive the VALUE string, not an event); `h(Chip,{label})`/`h(Badge,{label})`/`h(EmptyState,{text})`; LAYOUT takes positional children: `h(Stack,null,a,b)`/`h(Row,null,a,b)`/`h(Banner,{kind:"info"},text)`/`h(Divider)`; `h(Card,{title,body,footer})` uses props NOT children (custom card → h("div",{class:"ncard"},…children)); also List/Table/Select/Textarea/ProgressBar/Spinner. Raw-tag fallback uses CSS classes (button→"mini", input→"kit-in", card→"ncard", chip→"pill", row→"kit-rowx", stack→"kit-stack") + standard Preact events (e.target.value). props.value = a SAMPLE of the object\'s data — render legibly for THIS shape. props.methods = method names; props.call(method, argsArray) = INVOKE one (host-mediated; returns a Promise) — wire actions to it (e.g. a Send button → props.call("send",[text]).then(props.refresh)); NEVER call a method not in props.methods. props.refresh() re-fetches. endowments.useState/useEffect = local UI state. CONFINEMENT: no DOM/network/fs/caps beyond props.call. Theme CSS vars (var(--ink)/--mut/--edge/--acc/--panel/--bg), never hardcoded colours.',
      }, description: 'Author or REVISE the custom view (a confined renderer) for a KIND of object — a contact, an HA entity, an inventory capability, a peer. You write the (endowments,props)=>vnode yourself; this registers it so it renders wherever that kind appears. THIS is how you build a UI that ACTS ON an object (a chat box for a peer, a controller for a device): the component receives a live mediated `props.call(method,args)` to the object — authority flows BY REFERENCE to the real object, NOT through a local tool stand-in (which is why showComponent is wrong for this). READ-ONLY IS BY CONSTRUCTION, NOT A TYPE: scope `methods` to only the object\'s READ methods (omit send/act) and the component literally cannot call the others — it is read-only because of the attenuated handle it holds. Use it to fulfil a "custom view" / "chat with X" / "make a UI for this object" task; if the task includes a current source, iterate THAT rather than rewriting. The chat IS the studio — this turn is visible in the trace.' });
    }
