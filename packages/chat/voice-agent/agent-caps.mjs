...
export const makeFieldAgent = ({ outDir, baseUrl, autoConfirmFile, specialistsFile } = {}) => {
  ...
  const toolboxAndManifestFor = (powers, node, ctx = {}) => {
    ...
    toolbox.proposeTool = harden({ run: async ({ name: tname, description, code, args, kind, files, entry } = {}) => {
      ...
      if (!multi && !String(code || '').trim()) return { ok: false, error: 'provide `code` (a `make(powers)` body) OR `files` (a multi-file class: {"tool.js": "export const make = ...", ...}). Use powers.state.get/set to persist; confined (no fs/network).' };
    ...
    manifest.push({ name: 'proposeTool', reversible: false,
      ...
      description: 'PROPOSE a new STATEFUL tool. Pure JS (`make(powers)` → a persistent object, NOT a one-shot function): persist via powers.state across calls + restarts. Single-file → `code` (a make body). MULTI-FILE class → `files` (entry tool.js exports make + imports siblings; bundled as a real multi-module Endo SMR bundle, shareable). Queued for the owner to REVIEW + admit — never auto-injected.' });
