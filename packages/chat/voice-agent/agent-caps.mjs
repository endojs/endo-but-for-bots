...
export const makeFieldAgent = ({ outDir, baseUrl, autoConfirmFile, specialistsFile, fillSpecialist, wandPolicy, customView = {}, peerBridge = endoPeer, peerRedemption = process.env.FIELD_AGENT_PEER_REDEMPTION === '1' } = {}) => {
  ...
  const toolboxAndManifestFor = (powers, node, ctx = {}) => {
    ...
    toolbox.showComponent = harden({ run: async ({ source, cells, height, uses, effects } = {}) => {
      ...
      const grantedEffects = (Array.isArray(effects) ? effects : []).map(String).filter(e => e === 'theme'); // only the SAFE `theme` effect today (CSS vars, reversible)
      if (grantedEffects.length) widget.effects = grantedEffects; // → the component may ui.effect('theme:apply',{vars}) to restyle the app directly
