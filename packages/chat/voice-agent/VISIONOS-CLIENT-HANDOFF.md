...
**Security headers (the `SEC` object, applied to every response — grep `const SEC =` in server.mjs; line numbers in this doc have drifted, prefer the symbol):** `x-content-type-options: nosniff`, `x-frame-options: DENY`, `referrer-policy: no-referrer`, `permissions-policy: payment=(),interest-cohort=(),browsing-topics=()`, and a CSP. The **current full CSP** is (note it now carries a `frame-src` allow-list — the app frames its **own** widgets plus inline fleet SPWAs on the loopback/tailnet origins):
```
default-src 'self'; img-src 'self' data:; media-src 'self' blob:; connect-src 'self'; style-src 'self' 'unsafe-inline'; frame-src 'self' http://localhost:* http://127.0.0.1:* http://100.83.80.102:* http://*.taildd002.ts.net:* https://*.taildd002.ts.net; base-uri 'none'; form-action 'self'
```
(Note it permits `img-src data:`, `style-src 'unsafe-inline'`, and the `frame-src` list above; don't assume the whole CSP is just `connect-src 'self'`. There is also a variant that adds `'unsafe-eval'` to `default-src` for the SES-shim page.) Every static route goes through `serveFile()`, which applies `SEC` verbatim → `x-frame-options: DENY`; the **only** exception is `/trace-app.html`, which explicitly overrides to `SAMEORIGIN` (grep `'x-frame-options': 'SAMEORIGIN'`) so the SPA can iframe the trace viewer. (A native client doesn't care about XFO, but get this right when reasoning about the web client's framing/trusted-path story.) Request bodies are destroyed past **25MB** (`/chats/save` additionally caps JSON at 6MB).
