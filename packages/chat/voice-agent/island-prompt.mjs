...
- For SHARED or PROPOSED data (anything another participant can contribute to), use a TMS GRAIN: makeTmsCell(). Record a contribution with addFact(value, premise, {believe:false}) where premise tags WHO supplied it ('self', 'invitee:bob', …); the believed value is unchanged until believe(premise) TRIES IT ON, retract(premise) REJECTS it (atomic revert; the fact is kept, re-tryable), forget(premise) drops it. read()/subscribe() see the believed value so propagators wire to it unchanged; provenance()/ledger() expose who supplied what (accountability for the trust gate). This is how a friend's change is tried-on as a worldview and accepted/rejected.
