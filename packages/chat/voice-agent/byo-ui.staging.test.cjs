...
(async()=>{
  ...
  const invite=await new Promise((res,rej)=>{const r=http.request(`${srv.base}/invite`,{method:'POST',headers:{'content-type':'application/json'}},x=>{let d='';x.on('data',c=>d+=c);x.on('end',()=>res(JSON.parse(d)))});r.on('error',rej);r.end(JSON.stringify({cap,powers:['reference','home'],label:'ByoUiGuest'}))});
