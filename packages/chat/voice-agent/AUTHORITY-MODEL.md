...
| --- | --- | --- | --- |
| `read` | Observe only, no side effect | none (free) | `searchNotes` `readNote` `searchDietNotes` `readDietNote` `consult` `fetchUrl` `haFind` `haTree` `haState` `agentsList` `agentStatus` `fileList` `fileRead` `listTimers` `contactsSearch` `contactsGet` `listSpecialists` `listConnectors` `listCustomTools` `listScheduledTasks` `listChats` `readChatSanitized` `kazputerStatus` |
