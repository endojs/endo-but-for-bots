...
import { Btn, TextField } from './ui-kit.js';
...
export const DevTaskCard = (props = {}) => {
  ...
  return h('div', { class: 'msg', style: accent ? `border-color:${accent}` : undefined }, [
    ...
    h('div', { class: 'dev-thread' }, [
      h('button', { class: 'dev-thread-toggle', 'aria-expanded': String(!!expanded), style: accent ? `color:${accent}` : undefined, onClick: () => onToggle && onToggle() },
      ...
      expanded ? h('div', { class: 'dev-thread-body', style: accent ? `border-color:${accent}` : undefined }, [
        ...
        h('div', { class: 'dev-thread-row kit-rowx' }, [
          TextField({ value: draft, placeholder: `reply to ${who}…`, onInput: v => onReplyChange && onReplyChange(v), onEnter: () => onReplySend && onReplySend() }),
          Btn({ label: 'Send', variant: 'primary', onClick: () => onReplySend && onReplySend() }),
        ]),
      ]) : null,
    ]),
  ]);
