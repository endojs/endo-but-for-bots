...
const mountFileBrowser = async el => {
  ...
  const draw = () => renderInto('FileBrowser', el, { roots, root: st.root, path: st.path, entries: st.entries, file: st.file, busy: st.busy, error: st.error, editing: st.editing, draft: st.draft, dirty: st.dirty, onRoot, onOpen, onCrumb, onAdd, onDownload, onRemove, onCloseFile, onToggleEdit, onEdit, onSave });
  const list = async () => { st.busy = true; st.error = ''; st.file = null; st.editing = false; draw(); const r = await pf('/files/list', { root: st.root, path: st.path }); st.busy = false; if (r.error) { st.error = r.error; st.entries = []; } else st.entries = r.entries || []; draw(); };
  ...
  const onOpen = async (n, isDir) => { if (isDir) { st.path = rel(n); list(); return; } st.busy = true; st.editing = false; draw(); const r = await pf('/files/get', { root: st.root, path: rel(n) }); st.busy = false; if (r.error) st.error = r.error; else st.file = { name: r.name, text: r.text, size: r.size, b64: r.b64 }; draw(); };
  ...
  const onSave = async () => { if (!st.file || !st.dirty) return; st.busy = true; draw(); const b64 = await fileToB64(new Blob([st.draft], { type: 'text/plain' })); const r = await pf('/files/put', { root: st.root, path: rel(st.file.name), b64 }); st.busy = false; if (r.error) st.error = r.error; else { st.file = { ...st.file, text: st.draft }; st.dirty = false; } draw(); };
