    ...
    if (req.method === 'POST' && u.pathname === '/chat') {
      ...
      try {
        if (Date.now() - startedAt > 45000 || asks.length || proposals.length || r.asking || r.blocked) {
          ...
          notify({ title: `${icon} ${JSON.stringify(scrubCaps(t)).slice(1, 41)}… — ${summary}`, message: safeAnswer.slice(0, 160), click: `${WEB_URL}/#chat=${sid}`, tags: ['chat'] }).catch(() => {}); // cap-scrubbed (mirrors donePayload) — a swissnum must not reach feed.json/ntfy
