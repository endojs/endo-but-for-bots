...
The harness covers layers 3 to 8 on one machine with node-fs and in-memory
backings.
The rest of the investigation is still inference, and each gap has a concrete
way to close it.

| Gap                                                                | What would answer it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Where it can run                                                                                                                                                                                      |
| ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The kernel v9fs client (layer 2): cache modes, `msize`, `noxattr`, dentry and attribute behaviour, `mmap` | A `--mount` mode for the harness, or a `test:live`-style script: stand up `makeFsBridge9p` on a socket, `mount -t 9p -o trans=unix,version=9p2000.L,msize=…,cache=…` onto a temp directory, and run a fixed workload matrix: `git status` on a 10000-file checkout with and without `core.preloadIndex`, `git commit`, `find . -type f \| wc -l`, `tar cf /dev/null .`, `dd` at `bs=1M` and `bs=4k`, and a program that reads through `mmap`. Count 9P messages per type in the bridge (layer 3, last row) and syscalls with `strace -f -c`. Repeat per cache mode (`none`, `readahead` or `mmap`, `loose`), per `msize` (128 KiB, 1 MiB), with and without `noxattr` | A Linux host with `CONFIG_9P_FS` and `CAP_SYS_ADMIN` or `NINEP_SUDO`; not this VM. The opt-in `test:live` suite in `packages/claude-sandbox` (`test/live-daemon.test.js`, excluded from the default run at `package.json:67`) is the pattern to copy |
| The podman bind (layer 1)                                          | The same workload inside a rootless slice that binds the host mountpoint, either `podman run --rm --userns=keep-id -v <mountpoint>:/workspace …` or a slice minted through the sandbox factory, against the host-side run. The expectation is no difference; a difference is a finding. Alongside it, the per-session fixed costs that bound short sessions and are not 9P at all: `podman create` and `start`, and the policy anchor's attestation                                                       | The same host, with podman                                                                                                                                                                            |
| The remote-daemon topology (layer 6)                               | A `captp-tcp` topology in the harness: netstring CapTP over loopback TCP (`packages/daemon/src/networks/tcp-netstring.js`), with `tc qdisc add dev lo root netem delay 10ms` to inject a round trip. This is where the round-trip items (the pipelined stream call, `readAt`, the attribute prefetch on `Twalk`) become measurable instead of counted. Two daemons over iroh per `DEMO.md` part B is the end-to-end form. Profile the relay daemon with `node --cpu-prof` during the cross-worker run to see the JSON and netstring share of its loop | Any Linux box; `netem` needs root                                                                                                                                                                     |
| The attach backends (layer 7): `from-mount` over a daemon `Mount`, an `EndoGit` worktree | A `frommount` topology: `mountAsFilesystem(makeMount({ rootPath, readOnly: false, filePowers }))` (`@endo/daemon/src/mount.js:1953`) in process and then behind a hop, with the sequential-read and write rows run at 1, 8 and 64 MiB to expose the superlinear cost, and the `Treaddir` row for the N+1 `list`. The worktree case is the same topology over a git checkout                                                                                                                              | The harness, here                                                                                                                                                                                     |
| Whether the deployed workspace path is in process at all           | `ENDO_CAPTP_TRACE=1` on a running daemon while a `cat` runs in the slice: zero CapTP messages per 9P operation on the workspace mount confirms the same-worker topology; any at all means the cap was minted elsewhere. In the same session, `typeof Uint8Array.prototype.toBase64` in the daemon's Node decides whether base64 is native there                                                                                                                                                             | The deployment                                                                                                                                                                                        |
| The three in-process fixes                                         | Apply items 1 to 3 of the ranking on a branch and rerun the harness. Each has a predicted result: a 128 KiB `Tread` near 1 ms, a five-name `Twalk` at the cost of its syscalls, `Twrite` without the base64 share                                                                                                                                                                                                                                                                                             | The harness, here                                                                                                                                                                                     |
| Statistical confidence                                             | The tables are single runs on a 4-vCPU VM. Before any number decides a design, run five times and report medians with the p99 spread, on the deployment's Node version and hardware. A `--runs N` flag on the harness is the cheap way                                                                                                                                                                                                                                                                       | The harness, anywhere                                                                                                                                                                                 |
| The correctness items on a real mount                              | `git commit` and `git gc` under `strace -e trace=fsync,fdatasync` (the `Tfsync` `ENOSYS`), `cp -a` and `tar --xattrs` (the `Txattrwalk` chatter), a writable `mmap` under each cache mode, and a slow read interrupted with `SIGINT` (`Tflush`)                                                                                                                                                                                                                                                              | The same host as the kernel row                                                                                                                                                                       |

The cheapest next steps are the two harness-only rows, the attach backends
and the three fixes, which need nothing this VM lacks.
The most informative is the kernel row, because it is the only one that can
say what a real `git status` costs and which cache mode is safe for it.
