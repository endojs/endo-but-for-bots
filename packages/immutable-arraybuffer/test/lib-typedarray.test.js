...
// These tests exercise the property-record and emulated constructor machinery in
...
// makeEmulatedTypedArrayConstructor - wrapping an immutable ArrayBuffer

test('makeEmulatedTypedArrayConstructor wraps an immutable ArrayBuffer', t => {
  const ab = new ArrayBuffer(4);
  new Uint8Array(ab).set([1, 2, 3, 4]);
  const iab = sliceBufferToImmutable(ab);

  const EmulatedUint8Array = makeEmulatedTypedArrayConstructor(Uint8Array);
  const view = new EmulatedUint8Array(iab);
  ...
  // `emulatedTypedArrayBufferGetter` is exercised internally; we observe its
  // effect through the installed `view.buffer` accessor.
  t.is(view.buffer, iab);
  t.true(view.buffer.immutable);
});

// makeEmulatedTypedArrayConstructor - forwarding a non-immutable first arg

test('makeEmulatedTypedArrayConstructor forwards a non-immutable first arg', t => {
  const realAb = new ArrayBuffer(4);
  new Uint8Array(realAb).set([10, 20, 30, 40]);

  const EmulatedUint8Array = makeEmulatedTypedArrayConstructor(Uint8Array);
  const view = new EmulatedUint8Array(realAb);
...
test('buffer getter returns the real buffer for a genuine TypedArray', t => {
  ...
  // `emulatedTypedArrayBufferGetter` is installed on %TypedArrayPrototype%;
