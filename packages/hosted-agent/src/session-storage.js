...
/**
 * Durable storage removal for one recorded hosted session plan, shared by the
 * CLI adapters: each supplies its plan parser and the roots it was configured
 * with at setup.
 *
 * @module
 */
...
import { isNormalizedAbsolutePath } from './session-plan.js';

/**
 * The storage a recorded plan names, as the owner reads it: every path is a
 * normalized absolute host path; an operator-supplied workspace records no
 * `workspaceDir` and is never removed.
 * @typedef {object} StoragePlan
...
const StorageInterface = M.interface('SessionStorage', {
