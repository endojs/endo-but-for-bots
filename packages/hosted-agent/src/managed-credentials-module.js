...
/**
 * The per-credential caplet `provideManagedCredentials` mints over a delegated
 * SecretBlob read facet: issues single-shot per-session grants and never
 * exposes the catalog or mutation authority.
 *
 * @module
 */
...
const Credentials = M.interface('ManagedCredentials', {
 ...
 * @param {{ env?: { CREDENTIALS_KIND?: string, CREDENTIALS_LABEL?: string } }} [wrapper]
 */
export const make = (secret, _context, wrapper = {}) => {
  const kind = wrapper.env?.CREDENTIALS_KIND || 'apiKey';
  const label = wrapper.env?.CREDENTIALS_LABEL || 'Provider';
  ['apiKey', 'oauthToken'].includes(kind) || Fail`Invalid credential kind`;
  const grants = new Set();
  return makeExo('ManagedCredentials', Credentials, {
    kind: () => kind,
        ...
        async materialise() {
          ...
          try {
            ...
            try {
              ...
              throw Error(
                `${label} secret must contain a non-empty UTF-8 token of at most 8192 bytes`,
              );
            }
            return token;
          } finally {
    ...
    help: () =>
      `${label} credentials backed by Secrets. Rotate or revoke the token in the Secrets manager. Session grants are single-shot.`,
  });
};
harden(make);
