...
/**
 * Validate pass-by-copy data without admitting nested capabilities,
 * promises, errors, or accessors. Descriptors are inspected before
 * `passStyleOf` so an ordinary nested accessor is refused without evaluating
 * its getter. This uses the existing pass-style rules for frozen passable
 * containers; it is not a membrane for arbitrary local JavaScript proxies.
 *
 * The same rule the daemon applies to session protocol data
 * (`@endo/daemon/copy-data.js`), for the hosted-agent modules that read a
 * broker's attestation, a slice's policy or a recorded plan without
