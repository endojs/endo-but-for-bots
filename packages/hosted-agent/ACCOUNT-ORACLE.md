...
A session whose factory has an oracle also gets an `accountStatus` tool, so the
model can answer "how much quota is left?" and "what is this costing?" in the
conversation where the user asked.
