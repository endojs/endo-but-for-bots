...
import {
  provisionCodexSession,
  resolveSandboxConfig,
} from './provision-codex-session.js';
...
const CodexSessionProvisionerInterface = M.interface(
  'CodexSessionProvisioner',
  {
...
/**
 * Make a narrowly scoped service that can only provision Codex clients for
 ...
 *   provisionSession?: typeof provisionCodexSession,
 *   removeDirectory?: typeof rm,
 *   getFsMounter?: () => Promise<any>,
 * }} [powers]
 */
export const makeCodexSessionProvisioner = (hostAgent, config, powers = {}) => {
  const {
    flootDir,
    clientBase,
    codexHomeName,
    workspaceBaseDir,
    stateBaseDir = path.join(path.dirname(workspaceBaseDir), 'codex-sessions'),
    rootfs,
    network = 'private',
    sandboxNamespace = 'codex-sandbox',
  ...
  const provisionSession = powers.provisionSession || provisionCodexSession;
  ...
  const attachMountName = (/** @type {string} */ key) => `codex-attach-${key}`;
  ...
  const namesFor = sessionId => {
    ...
    return harden({
      ...
      filesystemName: `codex-workspace-${sessionId}`,
      workspaceDir: path.join(workspaceBaseDir, sessionId),
      stateDir: path.join(stateBaseDir, sessionId),
      threadStateFile: path.join(stateBaseDir, sessionId, 'thread-id'),
    });
  };
  ...
  const provisionOne = async (sessionId, options = {}) => {
    ...
    // Thread identity is metadata, kept outside both the shared CODEX_HOME and
    // the user-facing workspace. The CLI's auth and rollouts remain in the
    // shared writable home; this file only selects the right thread to resume.
    ...
    await provisionSession(
      hostAgent,
      {
        ...
        // Pin the CLI to the session's selected OpenAI model.
    ...
    if (!(await E(hostAgent).has(...clientPath))) {
      throw new Error(
        `Codex session provisioner did not store "${clientPath.join('/')}".`,
      );
    }
    return clientName;
  };

  return makeExo('CodexSessionProvisioner', CodexSessionProvisionerInterface, {
    ...
    async remove(sessionId) {
      const { clientPath, filesystemName, workspaceDir, stateDir } =
        namesFor(sessionId);
      ...
      await removeDirectory(stateDir, { recursive: true, force: true });
    },
    /**
    ...
    help: () =>
      'CodexSessionProvisioner: provision(flootSessionId) creates one isolated CodexClient and workspace; remove(flootSessionId) tears them down; provideContainerMountBridge({key, capId, mode})/releaseContainerMountBridge(key) bridge session-held caps over 9P for runtime container attaches.',
  });
};
...
export const make = (hostAgent, _context, { env = {} } = {}) => {
  ...
  const clientBase =
    env.CODEX_CLIENT_NAME ||
    process.env.ENDO_CODEX_CLIENT_NAME ||
    'codex-client';
  const codexHomeName =
    env.CODEX_HOME_NAME || process.env.ENDO_CODEX_HOME_NAME || 'codex-home';
  const workspaceBaseDir =
    env.CODEX_WORKSPACE_BASE_DIR ||
    process.env.ENDO_CODEX_WORKSPACE_DIR ||
    path.join(os.homedir(), 'codex-workspaces');
  const stateBaseDir =
    env.CODEX_SESSION_STATE_DIR ||
    process.env.ENDO_CODEX_SESSION_STATE_DIR ||
    path.join(path.dirname(workspaceBaseDir), 'codex-sessions');
  const rootfs =
    env.CODEX_SANDBOX_IMAGE ||
    process.env.CODEX_SANDBOX_IMAGE ||
    process.env.ENDO_CODEX_SANDBOX_IMAGE ||
    'oci:localhost/codex:latest';
  return makeCodexSessionProvisioner(hostAgent, {
    flootDir,
    clientBase,
    codexHomeName,
    workspaceBaseDir,
