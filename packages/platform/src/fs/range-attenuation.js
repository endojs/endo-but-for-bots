// @ts-check
/**
 * Shared attenuation logic for the `ReadableBlob` `byteRange` / `textRange`
 * surface (designs/readableblob-range-attenuation.md).
 *
 * `byteRange(start, end)` and `textRange(startLine, endLine)` do not read or
 ...
 * outside its parent), and the LF line-boundary math used by `textRange`.
 *
...
export const assertByteRange = (start, end) => {
  const startNumber = toSafeNumber(start, 'start');
  const endNumber = toSafeNumber(end, 'end');
  if (endNumber < startNumber) {
    throw makeError(X`EINVAL: range end ${q(end)} is before start ${q(start)}`);
  }
  return harden({ start: startNumber, end: endNumber });
};
harden(assertByteRange);

/**
 * Validate a line-range `[startLine, endLine)` request. Zero-based,
 * end-exclusive, plain non-negative safe-integer line indices; a negative,
 * fractional, non-safe, or
...
export const assertLineRange = (startLine, endLine) => {
  const startNumber = toSafeNumber(startLine, 'startLine');
  const endNumber = toSafeNumber(endLine, 'endLine');
  if (endNumber < startNumber) {
    throw makeError(
      X`EINVAL: textRange end ${q(endLine)} is before start ${q(startLine)}`,
    );
  }
  return harden({ startLine: startNumber, endLine: endNumber });
};
...
export const lineRangeToByteSlice = (bytes, startLine, endLine) => {
  ...
  const numberOfLines = line + 1; // the final line follows the last LF (maybe empty)
  if (startLine >= numberOfLines) {
