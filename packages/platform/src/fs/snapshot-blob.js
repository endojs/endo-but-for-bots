...
export const snapshotBlobMethods = (store, sha256) => {
  ...
  return harden({
    ...
    // encoding). The hex form is the internal
    // content-store address; callers that need it convert at the callsite.
    sha256: () => encodeBase64(decodeHex(sha256)),
    // `size` comes from the store's cheap stat when available, else from
    // draining the already in-memory bytes.
    ...
    help: method =>
      method === undefined
        ? 'SnapshotBlob: immutable content-addressed bytes (sha256, size, text, json, streamBase64).'
