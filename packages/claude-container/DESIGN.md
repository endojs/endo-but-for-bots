...
The system lives inside the `endo` monorepo (yarn 4 workspaces +
turbo) alongside the rest of `@endo/*`.
The earlier draft of this document proposed a standalone
`claude-sandbox/` monorepo with `packages/protocol`,
`packages/orchestrator`, `packages/broker`, `packages/client`, and
`packages/nine-p`; the design landed differently — packages are
co-located with the rest of Endo, the shared types live next to
their server in `@endo/claude-orch` rather than in a separate
`protocol` package, and the reference 9P server became a full
package (`@endo/9p-server`) rather than an optional helper.
The current placement is:

```
endo/
├── packages/
│   ├── claude-orch/                      # host orchestrator
│   │   ├── package.json
│   │   ├── protocol.types.d.ts           # shared types (API, bootstrap, agent, broker)
│   │   ├── bin/
│   │   │   ├── claude-orch               # API server entrypoint
│   │   │   └── claude-broker             # credential broker daemon
│   │   ├── src/
│   │   │   ├── main.js                   # wiring + restart reattach
│   │   │   ├── api/                      # HTTP/1.1 over UDS
│   │   │   ├── sessions/                 # session table, boot nonce, persistence
│   │   │   ├── qemu/                     # args builder + spawner
│   │   │   ├── network/                  # nftables (linux) + pf (darwin)
│   │   │   ├── bootstrap/                # Hello/BootConfig handler
│   │   │   ├── agent/                    # runtime-agent RPC link
│   │   │   ├── stdio/                    # per-session stdio mux
...
│   │       ├── claude-client.js
│   │       ├── fs-bridge-module.js
│   │       ├── claude-credentials-factory.js    # R3
│   │       ├── claude-credentials-module.js     # R3
│   │       └── orchestrator-client.js
│   ├── 9p-server/                        # 9P2000.L over UDS, wraps an endo-fs Filesystem
│   │   └── src/{server.js, wire.js, fs-bridge.js, types.js, index.js}
│   └── endo-fs/                          # pipelinable FS capability (R1 backing)
│       ├── DESIGN.md
│       └── src/{type-guards.js, in-memory.js, node-fs.js, from-mount.js,
│                  readonly.js, compose.js, layer.js, ...}
└── rust/
    └── claude-orch/
        ├── bootstrap-init/               # PID 1 in the guest + 9P relay
        │   └── src/main.rs
        └── runtime-agent/                # claude-code wrapper + seccomp
...
this branch — a NixOS module is M6 work (see `claude-orch/README.md`).

**Coding style** follows the Endo monorepo conventions:

- JavaScript source files (`.js`) with `// @ts-check` and JSDoc
  type annotations.
- Shared protocol types live in `packages/claude-orch/protocol.types.d.ts`
  (consumed by `claude-container/src/orchestrator-client.js` via
  `@import` JSDoc tags); there is no separate `protocol` package.
- `ava` for tests, not `node --test`.
- ESM modules (`"type": "module"`).
- All exports `harden()`-ed per `@endo/harden-exports`; `makeExo`
  with `M.interface` guards rather than bare `Far`.
