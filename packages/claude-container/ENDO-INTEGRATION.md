...
9P-over-UDS server for any endo-fs Filesystem).
...
`@endo/endo-fs` `Filesystem` to a 9P client can depend on it
directly).
...
|---|---|
| Where do credentials enter the picture? | **Resolved by R3.** A `ClaudeCredentials` factory caplet on `@host` mints per-credential exos; the `Create Claude Container` form takes a `credentials` pet name and the factory threads the issued record through `BootConfig`. The original broker config file remains the v1 fallback. |
...
| Should `send()` accept tool capabilities? | v1: no. Tools come from inside the guest (Claude Code's own tool set). R5 will thread Endo capabilities through as MCP-like tools once the in-guest CapTP bridge lands. |
| Does `ClaudeClient` survive daemon restart? | **Resolved by R4.** The `ClaudeClient` is a per-session formula whose `env` carries the orchestrator socket and session id; it reincarnates with the same identity. The 9P bridge is likewise a formula and re-binds the same UDS. The orchestrator separately persists session state across its own restarts. |
...
| Item | Status |
|---|---|
| R1 — Remote-friendly Filesystem capability | **Done** (`@endo/endo-fs` + 9P bridge consumes it) |
| R2 — Native 9P server (Rust) | Open |
| R2a — 9P-over-virtio-serial relay | **Done** |
| R3 — Credential capability | **Done** (`src/claude-credentials-factory.js` + per-cap module wired through the form to the orchestrator's BootConfig) |
