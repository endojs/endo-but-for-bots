...
/** @import { Session } from '@endo/claude-orch/types' */
...
/**
 * Subset of `Session` that the client constructs on reincarnation
 * from env vars. The orchestrator's full `Session` (with `state`
 * and `controlSocketPath`) is a supertype.
 *
 ...
 * @property {(session: SessionRef, prompt: string, opts?: { model?: string }) => Promise<AsyncIterable<any>>} sendPrompt
 */
