...
#   npm run install-engines --workspace @endo/benchmark
#
# Usage:
...
if [ ! -x "$ESHOST" ] || [ ! -x "$ROLLUP" ]; then
  ...
  echo "Run \`npm run install --workspace @endo/benchmark\` first." >&2
  exit 1
fi

if [ ! -f "$HOME/.engines/bin/xs" ] || [ ! -f "$HOME/.engines/bin/v8" ]; then
  echo "xs and/or v8 not found in \$HOME/.engines/bin." >&2
  echo "Run \`npm run install-engines --workspace @endo/benchmark\` first." >&2
  exit 127
fi
