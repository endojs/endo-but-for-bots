...
#   npm run --workspace=@endo/benchmark install-engines
#
# Usage:
...
if [ ! -x "$ESHOST" ] || [ ! -x "$ROLLUP" ]; then
  ...
  echo "Run \`npm run --workspace=@endo/benchmark install\` first." >&2
  exit 1
fi

if [ ! -f "$HOME/.engines/bin/xs" ] || [ ! -f "$HOME/.engines/bin/v8" ]; then
  echo "xs and/or v8 not found in \$HOME/.engines/bin." >&2
  echo "Run \`npm run --workspace=@endo/benchmark install-engines\` first." >&2
  exit 127
fi
