...
//
// opencode launches an MCP "local" server by spawning a command and talking
...
// reach — lives OUTSIDE the container, in the hosted backend's daemon worker on
// the far end of the socket. Nothing but JSON crosses this pipe.
//
// Usage inside the slice (wired by the host-generated opencode config):
...
// Bidirectional relay. Ending either side tears the other down so opencode sees
// a clean EOF and the daemon-side connection handler cleans up.
