...
test.serial('endo trace --stats reports an aggregator on a fresh daemon', async t => {
...
test.serial('endo trace --recent shows a worker emission after a failing eval', async t => {
  const execa = $({ cwd: dirname });
  await execa`endo purge -f`;
  await execa`endo start`;
  try {
...
test.serial('endo trace <unknownId> exits non-zero with a friendly message', async t => {
  const execa = $({ cwd: dirname });
  await execa`endo purge -f`;
  await execa`endo start`;
  try {
