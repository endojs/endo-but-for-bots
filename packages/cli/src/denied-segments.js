 ...
 * `--allow` flags, which surface the daemon's `deniedSegments` mount
 ...
 * - **`--allow`** — every segment is allowed: denial is disabled with an
 *   empty set. `--allow` is the opposite of `--deny` and cannot be
 *   combined with it.
 */
...
/**
 * Resolve the parsed `--deny` / `--allow` options into the daemon's
 * `deniedSegments` mount option.
 *
 * Commander yields an array when `--deny` was given, `true` when `--allow`
 * was given, and `undefined` for either option that did not appear.
 *
 * @param {string[] | undefined} deny
 * @param {boolean | undefined} allow
 * @returns {string[] | undefined} The replacement set (an empty array
 *   disables denial), or `undefined` to keep the mount's default set.
 */
export const resolveDeniedSegments = (deny, allow) => {
  if (allow) {
    if (deny !== undefined) {
      throw new Error('--deny and --allow cannot be used together');
    }
    // `--allow` — allow all segments: disable denial with an empty set.
    return [];
  }
