 ...
 * `Ctor[Symbol.for('freezableConstructor')]`. The installed
 * constructor accepts an emulated immutable `ArrayBuffer` as its sole
 * argument and returns a freezable view whose mutators throw; for
 * non-immutable arguments it delegates to the original `Ctor`.
 *
 ...
 * Consumers of `@endo/bytes` reach the installed constructors via the
 * registered symbol on each TypedArray family, not by importing the
 * maker.
 */
...
export { symFreezableConstructor };
