 ...
 *
 * Calls through to the realm's `ArrayBuffer.prototype.sliceToImmutable`
 * slot installed by `@endo/bytes` (see `./install-to-immutable.js`).
 * When the immutable-`ArrayBuffer` shim or a native implementation
 * already provides `ArrayBuffer.prototype.sliceToImmutable` at the
 * same rendezvous, `installOrAdopt` adopts it and this function
 * prefers the existing value. The resulting buffer carries the
 * `'byteArray'` passStyle and is safe to share across vat boundaries.
 * The result is hardened so it is passable.
 *
