...
// credential-bearing container — and speaks newline-delimited JSON-RPC 2.0:
...
/**
 * Bound on one JSON-RPC frame (a single newline-terminated line), in UTF-16
 * code units. Any process in the slice can connect to the socket — it is the
 * model's own Bash tool's filesystem — and stream bytes with no newline; without
 * a cap that buffer would grow until the factory worker, which also hosts every
 * other unconfined caplet, ran out of memory. Claude Code's requests are tool
 * calls with their arguments, far below this.
 */
...
export const startMcpSocketServer = async ({
  ...
  const server = netModule.createServer(connection => {
    ...
    connection.on('data', chunk => {
      ...
      if (buffer.length > maxFrameLength) {
        // No newline within the cap: not a JSON-RPC peer. Drop it before the
        // buffer can grow further.
        buffer = '';
        connection.destroy();
        return;
      }
  ...
  server.on('error', error => {
    console.error(
      `[claude-sandbox mcp] socket server error (${socketPath}):`,
