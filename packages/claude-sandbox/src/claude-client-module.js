...
export const make = (powers, context, contextWrapper = {}) => {
  ...
  const provision = async () => {
    ...
    // The projection chain lives in `@endo/9p-server`; what stays here is
    // session shape, including the lazy detach a busy workspace needs.
    ...
    // Materialise the credential, project the workspace, then mint the
    // slice; on any failure release everything this attempt created.
    // The 9P branch always carries a mount handle, so take the type from
    // `projectFilesystem` rather than the wider `MountProjection` union.
    ...
    try {
      ...
      if (projection) {
        // `release()` is best-effort, so it cannot mask the original cause.
