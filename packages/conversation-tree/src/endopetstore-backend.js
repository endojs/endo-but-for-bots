...
export const makeEndoPetstoreBackend = powers => {
  ...
  let loadP;
  /**
   * Nodes written while a load is in flight. That load's `list()` snapshot can
   * predate such a write, and `index` is not assigned until the load settles,
   * so `putNode` has nowhere to put them; the load merges them in (append-last)
   * before publishing the index. Without this a concurrent write would be
   * absent from the index for the life of the backend — nothing re-lists — and
   * `getChildren` would never report it.
   *
   * @type {Map<string, ConversationNode>}
   */
  const writtenDuringLoad = new Map();
  ...
  const load = () => {
    ...
    if (!loadP) {
      loadP = (async () => {
        ...
        // it and one it missed lands last.
        for (const [id, node] of writtenDuringLoad) {
          map.set(id, node);
        }
        writtenDuringLoad.clear();
        index = map;
        return map;
      })().catch(error => {
  ...
  const backend = {
    async putNode(node) {
      ...
      if (index) {
        ...
        // Hand the node to the in-flight load rather than waiting on it: the
        // write is already durable, so a read in progress must not delay it.
        writtenDuringLoad.set(node.id, node);
      }
    },
