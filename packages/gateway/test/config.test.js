...
test('mergeGatewayConfig accepts adminDaemon without sockBootstrap', t => {
  ...
  //
  // Other features (notably `captpRelay` and a future
  // `ocapnWebSocket` once Feature 8 lands) may still depend on
  // sockBootstrap for their own reasons; those dependencies are
  // independent of the admin facet and are exercised by their own
  // tests. The shape this test pins is solely the admin-without-
  // bootstrap path, with everything else either independent of
  // sockBootstrap or explicitly disabled.
