...
export const randomInt = (source, lo, hi) => {
  ...
  } else if (range === 0x10000 || range <= 0x8000) {
    draw = randomUint16;
    limit = 0x10000 - (0x10000 % range);
  } else if (range === 0x1000000 || range <= 0x800000) {
    draw = randomUint24;
    limit = 0x1000000 - (0x1000000 % range);
  } else if (range === 0x100000000 || range <= 0x80000000) {
    draw = randomUint32;
    limit = 0x100000000 - (0x100000000 % range);
  } else {
    draw = randomUint53;
