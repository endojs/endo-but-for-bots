...
interop('endo MessagePort client → capnweb server: calls and special values', async t => {
  t.timeout(20_000);
  ...
  t.is(await E(r).echo(123_456_789_012_345_678_901n), 123_456_789_012_345_678_901n);
...
interop('capnweb MessagePort client → endo server: calls and special values', async t => {
  t.timeout(20_000);
  const { port1, port2 } = new MessageChannel();
  const endo = makeCapnWebSession(makeMessagePortTransport(port1), {
    localMain: Far('main', {
...
interop('endo MessagePort ↔ capnweb: capability passing and pipelining', async t => {
  t.timeout(20_000);
  const { port1, port2 } = new MessageChannel();
